...
export type StreamingAccountPlaylistOwnership = 'created' | 'favorited' | 'unknown';

export type StreamingAccountPlaylist = StreamingPlaylist & {
  ownership: StreamingAccountPlaylistOwnership;
  webUrl: string;
};

export type StreamingAccountPlaylistsResult = {
  provider: Extract<StreamingProviderName, 'netease' | 'qqmusic'>;
  playlists: StreamingAccountPlaylist[];
  fetchedAt: string;
};
...
export type StreamingLiveProvider = 'direct' | 'bilibili';

export type StreamingLiveResolveRequest = {
  url: string;
};

export type StreamingLiveResolveResult = {
  provider: StreamingLiveProvider;
  sourceUrl: string;
  pageUrl: string;
  playbackUrl: string;
