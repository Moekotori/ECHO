...
export type LibraryAlbumGroupingRefreshResult = {
...
export type LibraryDiagnostics = {
  ...
  performance?: LibraryPerformanceDiagnostics;
};

export type LibraryPerformanceStallDiagnostics = {
  timestamp: string;
  ...
  route: string | null;
  windowKind: 'main' | 'desktopLyrics' | 'miniPlayer' | 'unknown' | null;
};

export type LibraryPerformanceDiagnostics = {
  lastSlowIpc: {
    channel: string;
    durationMs: number;
    ageMs: number;
  ...
  recentStalls: LibraryPerformanceStallDiagnostics[];
};
...
export type ImportOsuFolderResult = {
