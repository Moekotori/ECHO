...
export const MiniPlayerApp = (): JSX.Element => {
    ...
    if (seekAnchor) {
      if (seekAnchor.trackKey && progressTrackKey && seekAnchor.trackKey !== progressTrackKey) {
        ...
        const expectedSeekPosition = Math.min(
          seekAnchor.positionSeconds + (visualState === 'playing' ? elapsedSeconds * playbackRate : 0),
          durationLimit,
        );
  ...
  const commitSeek = useCallback(
    async (nextPositionSeconds: number): Promise<void> => {
      ...
      const optimisticPlaybackStatus: PlaybackStatus = {
        state: activeStates.has(visualState) ? 'playing' : visualState === 'paused' ? 'paused' : playbackStatus?.state ?? 'playing',
      ...
      beginPlaybackSeekSnapshot(optimisticPlaybackStatus);
    ...
    [currentTrack, durationSeconds, filePath, isSpotifyCurrentTrack, playbackStatus?.state, progressTrackKey, queue.hqPlayerTakeoverEnabled, runPlaybackAction, trackId, visualState],
  );
