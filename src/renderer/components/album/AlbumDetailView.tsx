...
const isAlbumDetailTestRuntime = typeof process !== 'undefined' && process.env.NODE_ENV === 'test';
const albumDetailPlaybackPriorityTrackDelayMs = isAlbumDetailTestRuntime ? 10 : 500;
const isAlbumDetailPlaybackPriorityActive = (state: string | null | undefined): boolean =>
...
const albumDetailDeferredLoadDelayMs = isAlbumDetailTestRuntime ? 0 : 350;

const scheduleAlbumDetailIdleTask = (callback: () => void, delayMs = albumDetailDeferredLoadDelayMs): (() => void) => {
  ...
  if (idleWindow.requestIdleCallback && idleWindow.cancelIdleCallback) {
    const idleId = idleWindow.requestIdleCallback(callback, { timeout: Math.max(delayMs, 1) + 600 });
...
export const AlbumDetailView = ({ album, onBack }: AlbumDetailViewProps): JSX.Element => {
  ...
  const albumTrackLoadStartedRef = useRef<string | null>(null);
  const onlineInfoLoadStartedRef = useRef<string | null>(null);
  const relatedAlbumsLoadStartedRef = useRef<string | null>(null);
  const albumCoverLoadStartedRef = useRef<string | null>(null);
  ...
  const relatedAlbumsLoadKey = `${album.id}\n${albumArtistLookupName?.trim() ?? ''}`;
  const shouldPrepareAlbumDna = activeTab === 'sources' || activeTab === 'information';
  const shouldPrepareExpandedAlbumDna = shouldPrepareAlbumDna && isAlbumDnaExpanded;
  const albumDnaTrackKey = useMemo(
    () => (shouldPrepareExpandedAlbumDna ? loadedTracks.map((track) => `${track.id}:${track.title}:${track.album}:${track.artist}`).join('\n') : ''),
    [loadedTracks, shouldPrepareExpandedAlbumDna],
  );
  const albumDnaArtists = useMemo(
    () => (shouldPrepareExpandedAlbumDna ? buildAlbumDnaArtists(loadedTracks, albumArtistDisplay) : []),
    [albumArtistDisplay, loadedTracks, shouldPrepareExpandedAlbumDna],
  );
  ...
  const albumDnaFormatFacts = useMemo<AlbumDnaFact[]>(
    () =>
      shouldPrepareExpandedAlbumDna
        ...
        : [],
    [albumDnaKnownText, albumDnaLoadedTrackTotal, loadedTracks, shouldPrepareExpandedAlbumDna, t],
  );
  const albumDnaQualityFacts = useMemo<AlbumDnaFact[]>(() => {
    if (!shouldPrepareAlbumDna) {
      return [];
    }
  ...
  }, [albumDnaKnownText, loadedTracks, shouldPrepareAlbumDna, t]);
  const albumDnaIntegrityFacts = useMemo<AlbumDnaFact[]>(() => {
    ...
    return facts;
  }, [album.trackCount, loadedTotal, loadedTracks, shouldPrepareAlbumDna, t]);
  const albumDnaMemoryFacts = useMemo<AlbumDnaFact[]>(() => {
    if (!shouldPrepareExpandedAlbumDna) {
      return [];
    }
    ...
    return facts;
  }, [album.id, albumDnaHistoryState.loadedForAlbumId, albumDnaHistoryState.loading, albumDnaMemoryStats, likedTrackIds, loadedTracks, shouldPrepareExpandedAlbumDna, t]);
  ...
  useEffect(() => {
    ...
    onlineInfoLoadStartedRef.current = null;
  }, [album.id]);

  useEffect(() => {
    if (onlineInfoLoadStartedRef.current === album.id) {
      return undefined;
    }

    onlineInfoLoadStartedRef.current = album.id;
    return scheduleAlbumDetailIdleTask(() => void loadOnlineInfo(false));
  }, [album.id, loadOnlineInfo]);

  useEffect(() => {
    setRelatedAlbumsState(emptyRelatedAlbumsState());
    setFailedRelatedCoverUrls({});
    relatedAlbumsLoadStartedRef.current = null;
  }, [album.id]);

  useEffect(() => {
    if (relatedAlbumsLoadStartedRef.current === relatedAlbumsLoadKey || playbackPriorityBlocking) {
      return undefined;
    }

    relatedAlbumsLoadStartedRef.current = relatedAlbumsLoadKey;
    return scheduleAlbumDetailIdleTask(() => void loadRelatedAlbums());
  }, [loadRelatedAlbums, playbackPriorityBlocking, relatedAlbumsLoadKey]);
  ...
  useEffect(() => {
    ...
    if (!shouldPrepareExpandedAlbumDna || !library?.getPlaybackHistory || loadedTracks.length === 0) {
    ...
    let isCancelled = false;
    const cancelIdleTask = scheduleAlbumDetailIdleTask(() => {
      if (isCancelled) {
        return;
      }
  ...
  }, [album.id, album.title, albumDnaTrackKey, loadedTracks.length, shouldPrepareExpandedAlbumDna]);
  ...
  useEffect(() => {
    ...
    albumCoverLoadStartedRef.current = null;
  }, [album.id]);

  useEffect(() => {
    if (albumCoverLoadStartedRef.current === album.id || playbackPriorityBlocking) {
      return undefined;
    }

    const requestAlbumId = album.id;
    albumCoverLoadStartedRef.current = album.id;

    const loadAlbumCover = (): void => {
      window.echo.library
        .getAlbum(album.id)
        .then((detail) => {
          if (albumCoverLoadStartedRef.current === requestAlbumId) {
    ...
    return scheduleAlbumDetailIdleTask(loadAlbumCover);
  }, [album.id, playbackPriorityBlocking]);
  ...
  const handleFirstTrackChange = useCallback((track: LibraryTrack | null, isLoading: boolean): void => {
    if (track || isLoading) {
      albumTrackLoadStartedRef.current = album.id;
    }
    setFirstTrack(track);
  ...
  const handlePlayTrack = useCallback(
    async (track: LibraryTrack): Promise<void> => {
      try {
        setPlayError(null);
        const playableTracks = loadedTotal > 0 && loadedTracks.length < loadedTotal
          ? await getAllAlbumTracks()
          : (loadedTracks.length > 0 ? loadedTracks : [track]).map(withAlbumCoverFallback);
    ...
    [albumSource, getAllAlbumTracks, loadedTotal, loadedTracks, playTrack, withAlbumCoverFallback],
  );
  ...
  const handlePlayStreamingTrack = useCallback(async (track: StreamingTrack): Promise<void> => {
    ...
    try {
      ...
      const playableTracks = detail?.tracks.filter((candidate) => candidate.playable).map(streamingTrackToLibraryTrack) ?? [];
      const source = {
        type: 'streaming' as const,
        label: `${detail?.title ?? track.album} / ${detail?.provider ?? track.provider}`,
        provider: detail?.provider ?? track.provider,
      };

      await playTrack(streamingTrackToLibraryTrack(track), {
        replaceQueueWith: playableTracks.length > 0 ? playableTracks : undefined,
        source,
        forceNewQueueItem: playableTracks.length === 0,
      });
    } catch (error) {
  ...
  }, [playTrack, resolvingStreamingTrackKey, selectedStreamingAlbumDetail]);
