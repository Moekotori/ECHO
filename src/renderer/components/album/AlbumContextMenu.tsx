...
import { ChevronRight, Copy, Download, FileImage, FolderOpen, Heart, ListEnd, ListMusic, Play, Plus, Tag, Trash2 } from 'lucide-react';
...
const remoteHiddenActions = new Set<AlbumMenuAction>(['show-in-folder', 'edit-tags', 'delete-album']);
...
export const AlbumContextMenu = ({ album, position, liked = false, onAction, onClose }: AlbumContextMenuProps): JSX.Element => {
  ...
  const [copySubmenuPosition, setCopySubmenuPosition] = useState(() => ({ x: position.x + menuWidth + submenuGap, y: position.y }));
  ...
  const resolveSubmenuPosition = (target: HTMLElement): { x: number; y: number } => {
  ...
  const openPlaylistSubmenu = (target: HTMLElement): void => {
    setPlaylistSubmenuPosition(resolveSubmenuPosition(target));
    setPlaylistSubmenuOpen(true);
    setCopySubmenuOpen(false);
    loadPlaylists();
  };

  const openCopySubmenu = (target: HTMLElement): void => {
    setCopySubmenuPosition(resolveSubmenuPosition(target));
  ...
  const allItems: MenuItem[] = [
    ...
    { action: 'play-next', labelKey: 'albumMenu.action.playNext', icon: Play },
    ...
    { action: 'show-in-folder', labelKey: 'albumMenu.action.showInFolder', icon: FolderOpen },
  ...
  const copyItems: MenuItem[] = [
    { action: 'copy-info', labelKey: 'albumMenu.action.copyInfo', icon: Copy },
    { action: 'copy-title', labelKey: 'albumMenu.action.copyTitle', icon: Copy },
    { action: 'copy-artist', labelKey: 'albumMenu.action.copyArtist', icon: Copy },
    { action: 'copy-path', labelKey: 'albumMenu.action.copyPath', icon: FolderOpen },
    { action: 'copy-track-list', labelKey: 'albumMenu.action.copyTrackList', icon: ListMusic },
  ];
    ...
    <div className="album-menu-layer" role="presentation" onMouseDown={onClose}>
          ...
          if (item.action === 'copy-info') {
            return (
              <button
                ...
                role="menuitem"
                type="button"
                onClick={(event) => openCopySubmenu(event.currentTarget)}
                onMouseEnter={(event) => openCopySubmenu(event.currentTarget)}
              >
                <Icon size={16} />
                <span>{t(item.labelKey)}</span>
                <ChevronRight className="album-menu-branch-icon" size={15} />
      ...
      {copySubmenuOpen ? (
        <div
          className="album-playlist-submenu"
          role="menu"
          aria-label={t('albumMenu.copySubmenu.aria')}
          style={{ left: copySubmenuPosition.x, top: copySubmenuPosition.y }}
          onMouseDown={(event) => event.stopPropagation()}
        >
          {copyItems.map((item) => {
            const Icon = item.icon;
