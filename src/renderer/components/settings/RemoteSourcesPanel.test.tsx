...
describe('RemoteSourcesPanel', () => {
  ...
  it('opens Baidu OAuth and stores the exchanged token secret for testing', async () => {
    ...
    expect(await screen.findByText('已换取 Token，并已保存为百度网盘来源。之后可以直接切回这个来源。')).toBeTruthy();
  ...
  it('logs into Baidu through the loopback OAuth flow and fills the token secret', async () => {
    ...
    expect(await screen.findByText('登录完成，已保存为百度网盘来源。之后可以直接切回这个来源。')).toBeTruthy();
    expect(remoteApiMocks.create).toHaveBeenCalledWith(expect.objectContaining({
      provider: 'baidu',
      authType: 'token',
