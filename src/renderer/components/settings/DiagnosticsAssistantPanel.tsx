...
const snapshotFromStatus = (status: AudioStatus): AudioDiagnosticSnapshot => ({
  ...
  activeOutputBackendLabel: status.activeOutputBackendLabel,
  nativeOutputFormat: status.nativeOutputFormat,
  useNativeOutputRequested: status.useNativeOutputRequested,
  useMiniaudioOutputRequested: status.useMiniaudioOutputRequested,
  useLibavDecodeRequested: status.useLibavDecodeRequested,
  activeDecodeBackendLabel: status.activeDecodeBackendLabel,
...
export const buildAudioDiagnosticFindings = (diagnostics: AudioDiagnosticSnapshot): AssistantFinding[] => {
  ...
  if (diagnostics.host === 'error' || diagnostics.host === 'unavailable') {
    ...
    findings.push({
      severity: 'warn',
      title: '音频还在启动',
      detail: '如果长期停在 loading/starting，重点检查设备占用、采样率和 ready timeout。',
    });
  }
...
export const buildAudioRecommendations = (diagnostics: AudioDiagnosticSnapshot): string[] => {
  ...
  if (diagnostics.host === 'starting' || diagnostics.state === 'loading') {
    recommendations.push('如果一直停在 starting/loading，优先检查设备是否被独占、采样率/缓冲是否被驱动拒绝。');
  }

  if (hasUnderrun(diagnostics) || hasLowBuffer(diagnostics)) {
    recommendations.push('出现 underrun 或缓冲偏低时，优先切到稳定延迟档、增大 native buffer、关闭重负载后台任务，再观察是否还卡顿。');
  }
...
export const formatDiagnosticAssistantText = (
  ...
  const rows: Array<[string, unknown]> = [
    ...
    ['activeOutputBackendLabel', diagnostics.activeOutputBackendLabel],
    ['nativeOutputFormat', diagnostics.nativeOutputFormat],
    ['activeDecodeBackendLabel', diagnostics.activeDecodeBackendLabel],
    ['activeDecodeBackendImpl', diagnostics.activeDecodeBackendImpl],
    ['useNativeOutputRequested', diagnostics.useNativeOutputRequested],
    ['useMiniaudioOutputRequested', diagnostics.useMiniaudioOutputRequested],
    ['useLibavDecodeRequested', diagnostics.useLibavDecodeRequested],
  ...
  const deviceRows = devices.map((device) =>
    `- ${device.outputMode}#${device.index}: ${device.name}; default=${formatValue(device.isDefault)}; sampleRate=${formatValue(device.sampleRate)}; sharedRate=${formatValue(device.sharedDeviceSampleRate)}`,
  );
