...
const routes: AppRoute[] = [
  ...
  },
  {
    ...
    icon: Download,
    placement: 'main',
    element: <div>osu downloader</div>,
  },
  {
    id: 'audio-settings',
...
describe('Sidebar direct import actions', () => {
  ...
  it('hides osu downloader from the route context menu', async () => {
    ...
    fireEvent.contextMenu(screen.getByRole('button', { name: 'osu downloader' }));
    fireEvent.click(await screen.findByRole('menuitem', { name: /隐藏|闅愯棌/u }));

    expect(onHideRoute).toHaveBeenCalledWith('osu-downloader');
  });
});
