...
let cachedPluginMenuItems: PluginMenuItem[] | null = null;
let cachedExtraActionsEnabled: boolean | null = null;
...
if (typeof window !== 'undefined') {
  window.addEventListener('settings:changed', (event: Event) => {
    ...
    if (Object.prototype.hasOwnProperty.call(detail ?? {}, 'trackContextMenuExtraActionsEnabled')) {
      cachedExtraActionsEnabled = detail.trackContextMenuExtraActionsEnabled === true;
    }
  });
}

export const clearTrackContextMenuPluginCacheForTests = (): void => {
  cachedPluginMenuItems = null;
  cachedExtraActionsEnabled = null;
};
...
export const TrackContextMenu = ({
  ...
  showOpenOsuBeatmapset = false,
  onAction,
  onClose,
}: TrackContextMenuProps): JSX.Element => {
  const { t } = useI18n();
  ...
  const [pluginMenuItems, setPluginMenuItems] = useState<PluginMenuItem[]>(() =>
    cachedPluginMenuItems?.filter((item) => !track.mediaType || track.mediaType === 'local' || item.localOnly !== true) ?? [],
  );
  const [pluginMenuReady, setPluginMenuReady] = useState(() => cachedPluginMenuItems !== null || !window.echo?.plugins || selectionCount > 1);
  ...
  const [extraActionsEnabled, setExtraActionsEnabled] = useState(() => cachedExtraActionsEnabled === true);
  const [extraActionsReady, setExtraActionsReady] = useState(() => cachedExtraActionsEnabled !== null || !window.echo?.app?.getSettings);
  ...
  }, [enabledActions, extraActionsEnabled, liked, pluginMenuItems.length, position.x, position.y, selectionCount, showOpenOsuBeatmapset, showRemoveFromPlaylist, track.mediaType]);
  ...
  useEffect(() => {
    ...
    const applySettings = (settings: Partial<AppSettings> | null | undefined): void => {
      if (!cancelled) {
        const nextExtraActionsEnabled = settings?.trackContextMenuExtraActionsEnabled === true;
        cachedExtraActionsEnabled = nextExtraActionsEnabled;
        setExtraActionsEnabled(nextExtraActionsEnabled);
        setExtraActionsReady(true);
      }
    };
    ...
    if (!window.echo?.app?.getSettings) {
      setExtraActionsReady(true);
    } else {
      setExtraActionsReady(cachedExtraActionsEnabled !== null);
  ...
  useEffect(() => {
    ...
    if (cachedPluginMenuItems) {
      setPluginMenuReady(true);
      setPluginMenuItems(cachedPluginMenuItems.filter((item) => isLocalFileTrack || item.localOnly !== true));
    } else {
      setPluginMenuReady(false);
    }
    ...
    void plugins.list()
      .then((result) => {
        ...
        cachedPluginMenuItems = nextItems;
        setPluginMenuItems(nextItems.filter((item) => isLocalFileTrack || item.localOnly !== true));
        setPluginMenuReady(true);
      })
      .catch(() => {
        if (!cancelled) {
  ...
  const allItems: MenuItem[] = [
    ...
    { action: 'open-osu-beatmapset', labelKey: 'trackMenu.action.openOsuBeatmapset', icon: ExternalLink },
  ...
  const items = allItems.filter((item) => {
    ...
    if (item.action === 'open-osu-beatmapset') {
      return !isBatch && showOpenOsuBeatmapset;
    }
  ...
  const menuReady = pluginMenuReady && extraActionsReady;
  ...
  return createPortal(
    <div className="track-menu-layer" role="presentation" onMouseDown={requestClose}>
      <div
        ...
        role="menu"
        style={{ left: menuPosition.x, top: menuPosition.y, maxHeight: menuMaxHeight, visibility: menuReady ? 'visible' : 'hidden' }}
