...
const renderPanel = (
  ...
  return render(
    <MvPanel
      ...
      renderPressureReduced={renderPressureReduced}
...
describe('MvPanel', () => {
  ...
  it('keeps foreground MV playback but removes the duplicate background video in render pressure mode', async () => {
    ...
    const foregroundVideo = await waitFor(() => {
      const element = container.querySelector('.lyrics-mv-video') as HTMLVideoElement | null;
      expect(element?.getAttribute('src')).toBe('echo-video://mv/video-1');
      return element!;
    });

    expect(foregroundVideo.preload).toBe('metadata');
    expect(container.querySelector('.lyrics-mv-background-video')).toBeNull();
    expect(container.querySelector('.lyrics-mv-panel')?.getAttribute('data-render-pressure-reduced')).toBe('true');
    expect(container.querySelector('.lyrics-mv-panel')?.getAttribute('data-immersive-active')).toBe('false');
  });

  it('releases foreground MV video resources when the panel unmounts', async () => {
    const loadSpy = vi.spyOn(window.HTMLMediaElement.prototype, 'load').mockImplementation(() => undefined);
    const { container, unmount } = renderPanel(makeVideo(), true, {
      ...defaultMvSettings,
    ...
    expect(foregroundVideo.getAttribute('src')).toBeNull();
  });
  ...
  it('keeps the foreground YouTube iframe but removes the duplicate background iframe in render pressure mode', async () => {
    ...
    expect(container.querySelector('.lyrics-mv-panel')?.getAttribute('data-immersive-active')).toBe('false');
    expect(container.querySelector('.lyrics-mv-panel')?.getAttribute('data-render-pressure-reduced')).toBe('true');
    expect(container.querySelector('iframe.lyrics-mv-background-video--youtube')).toBeNull();
  });
  ...
  it('uses Ctrl+wheel to adjust immersive MV background scale without saving each wheel tick', async () => {
    const initialSettings: MvSettings = {
      ...defaultMvSettings,
      immersiveBackground: true,
      immersiveBackgroundAutoScale: false,
      immersiveBackgroundScalePercent: 115,
    };
    const setSettings = vi.fn(async (patch: Partial<MvSettings>) => ({
      ...initialSettings,
      ...patch,
    }));
    const { container } = renderPanel(makeVideo(), true, initialSettings);
    window.echo!.mv!.setSettings = setSettings;

    const background = await waitFor(() => {
      const element = container.querySelector('.lyrics-mv-background') as HTMLElement | null;
      expect(element).toBeTruthy();
      expect(element!.style.getPropertyValue('--mv-immersive-scale')).toBe('1.15');
      return element!;
    });

    vi.useFakeTimers();
    fireEvent.wheel(background, { deltaY: -100 });
    expect(background.style.getPropertyValue('--mv-immersive-scale')).toBe('1.15');

    fireEvent.wheel(background, { ctrlKey: true, deltaY: -100 });
    fireEvent.wheel(background, { ctrlKey: true, deltaY: -100 });

    expect(background.style.getPropertyValue('--mv-immersive-scale')).toBe('1.25');
    ...
    expect(setSettings).toHaveBeenLastCalledWith({ immersiveBackgroundScalePercent: 125 });

    for (let index = 0; index < 7; index += 1) {
      fireEvent.wheel(background, { ctrlKey: true, deltaY: 100 });
    }
    expect(background.style.getPropertyValue('--mv-immersive-scale')).toBe('0.90');
    ...
    expect(setSettings).toHaveBeenLastCalledWith({ immersiveBackgroundScalePercent: 90 });
  });
  ...
  it('keeps the current MV visible while a manual MV switch is loading', async () => {
    ...
    await waitFor(() => expect(getSelected).toHaveBeenCalledTimes(2));
    expect(container.querySelector('.lyrics-mv-video')?.getAttribute('src')).toBe('echo-video://mv/video-1');
    ...
    await waitFor(() => expect(container.querySelector('.lyrics-mv-video')?.getAttribute('src')).toBe('echo-video://mv/video-2'));
  });
