...
describe('LyricsSettingsDrawer', () => {
  ...
  it('keeps page-provided streaming lyrics title when the bridge cache has no applied lyrics', async () => {
    ...
    expect(screen.getByText('QQ Music')).toBeTruthy();
  ...
  it('shows low scoring lyric search results in the drawer', async () => {
    ...
    expect(searchCandidates).toHaveBeenCalledWith('track-1', 'rough query', 'local', 'manual');
    expect(searchCandidates).toHaveBeenCalledWith('track-1', 'rough query', 'lrclib', 'manual');
    expect(searchCandidates).toHaveBeenCalledWith('track-1', 'rough query', 'netease', 'manual');
    expect(searchCandidates).toHaveBeenCalledWith('track-1', 'rough query', 'qqmusic', 'manual');
  });

  it('keeps drawer search results selectable instead of auto-applying them', async () => {
    ...
    expect(await screen.findByText('Auto Applied Song')).toBeTruthy();
    expect(window.echo?.lyrics.applyCandidate).not.toHaveBeenCalled();
    expect(appliedListener).not.toHaveBeenCalled();

    window.removeEventListener('lyrics:candidate-applied', appliedListener);
  });

  it('keeps newer drawer search results when a previous apply finishes late', async () => {
    const pendingApply = deferred<TrackLyrics>();
    const searchCandidates = vi.fn().mockImplementation(
      (_trackId: string, searchText: string | undefined) => Promise.resolve([
        makeLyricsCandidate({
          id: searchText === 'fresh query' ? 'fresh-candidate' : 'old-candidate',
          title: searchText === 'fresh query' ? 'Fresh Search Result' : 'Old Search Result',
          score: 0.42,
        }),
      ]),
    );
    ...
    fireEvent.change(searchbox, { target: { value: 'fresh query' } });
    fireEvent.submit(searchbox.closest('form')!);
  ...
  it('persists the current-track lyrics auto-apply toggle', async () => {
    ...
    const toggleLabel = await screen.findByText('自动应用歌词');
    fireEvent.click(toggleLabel.closest('label')!.querySelector('input')!);

    await waitFor(() => expect(setSettings).toHaveBeenCalledWith({ lyricsAutoApplyEnabled: false }));
  });
