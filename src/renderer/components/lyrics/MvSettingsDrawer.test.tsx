...
describe('MvSettingsDrawer', () => {
  ...
  it('toggles MV preload, restart sync, and replay on change from the drawer', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /Search MV by title only/ }));
    fireEvent.click(await screen.findByRole('button', { name: /Match by views/ }));
    ...
    await waitFor(() => expect(window.echo.mv.setSettings).toHaveBeenCalledWith({ titleOnlySearch: false }));
    await waitFor(() => expect(window.echo.mv.setSettings).toHaveBeenCalledWith({ preferHighestViewCount: false }));
