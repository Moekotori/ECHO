...
type AudioDrawerCopy = {
  ...
  eqOn: string;
  exclusive: string;
  nativeDirectPlayback: string;
  nativeDirectPlaybackStandby: string;
  nativeDirectPlaybackOff: string;
  nativeDirectPlaybackPending: string;
  miniaudioOutput: string;
  nativeRate: string;
...
const defaultAdvancedOutputGroupOpen: Record<AdvancedOutputGroupKey, boolean> = {
  outputPath: true,
  decodePath: false,
  dsdTransport: false,
  protection: false,
  bufferAndDevice: false,
  behavior: false,
};

const AdvancedOutputGroup = ({
  children,
  description,
  icon: Icon,
  isOpen,
  title,
  onToggle,
}: AdvancedOutputGroupProps): JSX.Element => (
  <div className={`audio-advanced-group${isOpen ? ' audio-advanced-group--open' : ''}`}>
    <button
      className="audio-advanced-group__toggle"
      type="button"
    ...
    <div className="audio-advanced-group__content" aria-hidden={!isOpen}>
      <div className="audio-advanced-group__body">{children}</div>
    </div>
  </div>
);
...
const defaultDrawerRememberedAudioOutput: RememberedAudioOutput = {
  enabled: true,
  outputMode: 'shared',
  sharedBackend: 'auto',
...
const isActiveMiniaudioBackend = (status: AudioStatus | null): boolean =>
  typeof status?.activeOutputBackendImpl === 'string' && status.activeOutputBackendImpl.startsWith('miniaudio-');

const isActiveNativeDirectLocalPlayback = (status: AudioStatus | null): boolean =>
  (typeof status?.activeDecodeBackendLabel === 'string' &&
    status.activeDecodeBackendLabel.startsWith('native-direct-libav-audio-format'));

const isNativeDirectLocalPlaybackStandbyVisible = (status: AudioStatus | null): boolean =>
  Boolean(status?.nativeDirectLocalPlaybackRequested && !isActiveNativeDirectLocalPlayback(status));

const getNativeDirectLocalPlaybackText = (
  status: AudioStatus | null,
  settingEnabled: boolean,
  copy: AudioDrawerCopy,
): string => {
  if (isActiveNativeDirectLocalPlayback(status)) {
    return copy.nativeDirectPlayback;
  }

  if (isNativeDirectLocalPlaybackStandbyVisible(status)) {
    const fallbackReason = status?.nativeDirectLocalPlaybackFallbackReason;
    return fallbackReason && fallbackReason !== 'disabled'
      ? `${copy.nativeDirectPlaybackStandby}: ${fallbackReason}`
      : copy.nativeDirectPlaybackStandby;
  }

  return settingEnabled ? copy.nativeDirectPlaybackPending : copy.nativeDirectPlaybackOff;
};
...
const getPlaybackChainText = (status: AudioStatus | null, copy: AudioDrawerCopy): string => {
  if (isActiveDsdDop(status)) {
    return `${copy.dsdDop} -> ${copy.exclusive}`;
  }

  const decodeText = isActiveNativeDirectLocalPlayback(status)
    ? copy.nativeDirectPlayback
    : isDsdDopFallbackVisible(status)
        ? copy.dsdDopFallback
  ...
  if (isActiveMiniaudioBackend(status)) {
    return `${decodeText} -> ${copy.miniaudioOutput}`;
  }


  if (
    status?.useMiniaudioOutputRequested &&
    status.sharedBackend !== 'directsound' &&
    status.outputBackend !== 'directsound-shared'
  ) {
    return `${decodeText} -> ${copy.miniaudioOutput}`;
  }
...
const getCurrentBackend = (status: AudioStatus | null, copy: AudioDrawerCopy): string => {
  if (isActiveMiniaudioBackend(status)) {
    return copy.miniaudioOutput;
  }

  return status?.outputBackend || status?.outputDeviceType || copy.systemAudio;
};
...
export const AudioSettingsDrawer = ({
  ...
  const copy = useMemo<AudioDrawerCopy>(
    () => ({
      ...
      nativeDirectPlayback: t('audioDrawer.signal.nativeDirectPlayback'),
      nativeDirectPlaybackStandby: t('audioDrawer.signal.nativeDirectPlaybackStandby'),
      nativeDirectPlaybackOff: t('audioDrawer.signal.nativeDirectPlaybackOff'),
      nativeDirectPlaybackPending: t('audioDrawer.signal.nativeDirectPlaybackPending'),
      miniaudioOutput: t('audioDrawer.signal.miniaudioOutput'),
  ...
  const [advancedOutputGroupOpen, setAdvancedOutputGroupOpen] = useState(() => defaultAdvancedOutputGroupOpen);
  ...
  const [resetAllAudioSettingsBusy, setResetAllAudioSettingsBusy] = useState(false);
  const [resetAllAudioSettingsMessage, setResetAllAudioSettingsMessage] = useState<string | null>(null);
  ...
  const engineBadges = useMemo(() => {
    ...
    if (isActiveNativeDirectLocalPlayback(status)) {
      badges.push({ label: copy.nativeDirectPlayback, tone: 'ready' });
    } else if (isNativeDirectLocalPlaybackStandbyVisible(status)) {
      badges.push({ label: copy.nativeDirectPlaybackStandby, tone: 'neutral' });
    } else if (nativeDirectLocalPlaybackEnabled) {
      badges.push({ label: copy.nativeDirectPlaybackPending, tone: 'neutral' });
    }
    ...
    if (isActiveMiniaudioBackend(status)) {
      badges.push({ label: copy.miniaudioOutput, tone: 'ready' });
    } else if (
      status?.useMiniaudioOutputRequested &&
      status.sharedBackend !== 'directsound' &&
      status.outputBackend !== 'directsound-shared'
    ) {
      badges.push({ label: copy.miniaudioOutput, tone: 'neutral' });
    }
    ...
    return badges;
  }, [copy, effectiveSharedSampleRate, hqPlayerTakeoverEnabled, nativeDirectLocalPlaybackEnabled, status]);
  ...
  const engineSignalDetails = useMemo(
    () => hqPlayerTakeoverEnabled
      ...
      : [
          ...
          { label: t('audioDrawer.meter.nativeDirect'), value: getNativeDirectLocalPlaybackText(status, nativeDirectLocalPlaybackEnabled, copy) },
    ...
    [copy, effectiveSharedSampleRate, hqPlayerTakeoverEnabled, hqPlayerTrack, nativeDirectLocalPlaybackEnabled, status, t],
  );
  ...
  const applyLatencyProfile = useCallback(
    (requestedLatencyProfile: AudioLatencyProfile): void => {
      ...
      const currentBufferSizeFrames = rememberedBufferSizeFrames;
    ...
    [applyOutput, outputMode, status?.outputMode],
  );
  ...
  const toggleAdvancedOutputGroup = (group: AdvancedOutputGroupKey): void => {
    setAdvancedOutputGroupOpen((current) => ({
      ...current,
      [group]: !current[group],
    }));
  };
  ...
  const toggleMiniaudioOutput = (enabled: boolean): void => {
    const previous = useMiniaudioOutput;
    setUseMiniaudioOutput(enabled);
    void window.echo?.app
      .setSettings({
        audioUseNativeOutput: enabled,
        audioUseMiniaudioOutput: enabled,
        audioMiniaudioOutputExperimentalEnabled: enabled,
      })
      .catch((settingsError) => {
  ...
  const resetAllAudioSettings = useCallback(async (): Promise<void> => {
    const app = window.echo?.app;
    const audio = window.echo?.audio;
    ...
    setResetAllAudioSettingsBusy(true);
    ...
    try {
      ...
      setReleaseExclusiveOnPauseExperimentalEnabled(false);
      setFixedVolumeEnabled(false);
      setLowLoadPlaybackModeEnabled(false);
      setLowLoadPlaybackEnhancementsEnabled(false);
      setHiddenDeviceKeys([]);
      setOutputMode('shared');
      ...
      const detail = nextSettings && typeof nextSettings === 'object'
        ? { ...nextSettings, ...resetAllAudioSettingsPatch }
        : resetAllAudioSettingsPatch;
      window.dispatchEvent(new CustomEvent('settings:changed', { detail }));
      setResetAllAudioSettingsMessage(t('audioDrawer.resetAll.done'));
    ...
    <div className="audio-drawer-root no-drag" role="presentation" data-open={isMotionOpen}>
      ...
      <aside className="audio-drawer" aria-label={t('audioDrawer.title')}>
        ...
        <section
          ...
          data-search-keywords="advanced output engine miniaudio native libav decode ffmpeg dsd dop volume lock exclusive pause fallback soxr directsound shared backend latency buffer fixed volume remember output bit-perfect"
        >
          <button
          ...
          {isAdvancedOutputOpen ? (
            <div className="audio-drawer-options-body">
              {advancedNativeOutputAvailable ? (
                <AdvancedOutputGroup
                  icon={AudioLines}
                  title={t('audioDrawer.advancedGroup.outputPath.title')}
                  description={t('audioDrawer.advancedGroup.outputPath.description')}
                  isOpen={advancedOutputGroupOpen.outputPath}
                  onToggle={() => toggleAdvancedOutputGroup('outputPath')}
                >
              <label className="audio-toggle-row">
                <span>
                  <FlaskConical size={17} />
                  <strong>{t('audioDrawer.option.miniaudioOutput')}</strong>
                </span>
                <input
                  type="checkbox"
                  checked={useMiniaudioOutput}
                  disabled={isBusy}
                  onChange={(event) => toggleMiniaudioOutput(event.currentTarget.checked)}
                />
              </label>
              <p>{t('audioDrawer.note.miniaudioOutput')}</p>
              ...
              {advancedNativeOutputAvailable ? (
                <AdvancedOutputGroup
                  icon={ShieldAlert}
                  title={t('audioDrawer.advancedGroup.protection.title')}
                  description={t('audioDrawer.advancedGroup.protection.description')}
                  isOpen={advancedOutputGroupOpen.protection}
                  onToggle={() => toggleAdvancedOutputGroup('protection')}
                >
              ...
              <AdvancedOutputGroup
                icon={Gauge}
                title={t('audioDrawer.advancedGroup.bufferAndDevice.title')}
                description={t('audioDrawer.advancedGroup.bufferAndDevice.description')}
                isOpen={advancedOutputGroupOpen.bufferAndDevice}
                onToggle={() => toggleAdvancedOutputGroup('bufferAndDevice')}
              >
          ...
          <div className={`audio-buffer-collapse${isBufferOptionsOpen ? ' audio-buffer-collapse--open' : ''}`}>
            ...
            {isBufferOptionsOpen ? (
              ...
              <AdvancedOutputGroup
                ...
                title={t('audioDrawer.advancedGroup.behavior.title')}
                description={t('audioDrawer.advancedGroup.behavior.description')}
                isOpen={advancedOutputGroupOpen.behavior}
                onToggle={() => toggleAdvancedOutputGroup('behavior')}
              >
          <label className="audio-toggle-row">
            <span>
              <RefreshCw size={17} />
        ...
        {error ? <p className="audio-drawer-error">{error}</p> : null}
          ...
          <details className="audio-drawer-section audio-hidden-devices">
            ...
            {hiddenDevices.map((device) => {
              ...
              return (
                <div className="audio-hidden-device" key={getDeviceStorageKey(device)}>
                  <DeviceIcon size={15} />
                  <span>
                    <strong title={device.name}>{device.name}</strong>
                    <small>{t('audioDrawer.device.systemOutput')} / {sampleRate || t('audioDrawer.status.sampleRatePending')}</small>
                  </span>
          ...
          <section className="audio-drawer-section audio-drawer-reset-section">
            <div className="audio-drawer-section-title">
              <RefreshCw size={17} />
              <h3>{t('audioDrawer.resetAll.title')}</h3>
            </div>
            <p>{t('audioDrawer.resetAll.description')}</p>
            <button
              className="audio-diagnostics-copy-button"
              type="button"
              disabled={resetAllAudioSettingsBusy || resetBusy || isBusy}
              onClick={() => void resetAllAudioSettings()}
            >
              <RefreshCw size={16} />
              <span>{resetAllAudioSettingsBusy ? t('audioDrawer.resetAll.busy') : t('audioDrawer.resetAll.action')}</span>
            </button>
            {resetAllAudioSettingsMessage ? <p className="audio-drawer-reset-section__message">{resetAllAudioSettingsMessage}</p> : null}
          </section>
        </div>
      </aside>
      {hiddenDeviceMenu ? (
