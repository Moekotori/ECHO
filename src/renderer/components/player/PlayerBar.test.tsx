...
import { hqPlayerConnectDeviceId, type AirPlayReceiverStatus, type ConnectReceiverStatus, type ConnectSessionStatus } from '../../../shared/types/connect';
...
const QueueSeed = ({
  ...
  return (
    <I18nProvider>
      <PlayerBar lyricsMiniPlayer={lyricsMiniPlayer} showSignalPathControl={showSignalPathControl} />
    </I18nProvider>
  );
};
...
describe('PlayerBar', () => {
  it('marks only the opted-in lyrics mini player for compact-away drawer styling', async () => {
    ...
    rerender(
      <I18nProvider>
        <PlaybackQueueProvider>
          <PlayerBar lyricsMiniPlayer lyricsCompactOnIdle />
        </PlaybackQueueProvider>
      </I18nProvider>,
    );

    expect(container.querySelector('.player-bar')?.getAttribute('data-compact-away')).toBe('true');
    expect(container.querySelector('.player-compact-progress')).toBeTruthy();
  });
  ...
  it.each(['native-direct-juce-audio-format', 'native-direct-juce-audio-format-src-pcm'] as const)(
    'shows local direct decode with a friendly label in the signal path popover for %s',
    (activeDecodeBackendImpl) => {
      const track = makeTrack(45, { title: 'Direct Signal Track', codec: 'flac', sampleRate: 44100, bitDepth: 16 });
      const status = {
        ...audioStatus(track),
        activeDecodeBackendImpl,
      ...
      const dialog = screen.getByRole('dialog', { name: translateFallback('audioSignalPath.title') });
      expect(dialog.textContent).toContain(translateFallback('audioSignalPath.node.decode'));
      expect(dialog.textContent).toContain(translateFallback('audioSignalPath.decode.nativeDirect'));
      expect(dialog.textContent).not.toContain(activeDecodeBackendImpl);
    },
  );

  it('keeps native output errors human-readable in the signal path popover', () => {
    const track = makeTrack(44, { title: 'Output Timeout Track', codec: 'flac', sampleRate: 44100, bitDepth: 16 });
    const rawError =
    ...
    const dialog = screen.getByRole('dialog', { name: '信号路径' });
    expect(dialog.textContent).toContain('信号路径: 输出没接上');
    expect(dialog.textContent).toContain('音频输出启动超时');
    expect(dialog.textContent).not.toContain('timeout_waiting_for_ready');
    expect(dialog.textContent).not.toContain('echo-audio-host.exe');

    fireEvent.click(screen.getByRole('button', { name: translateFallback('audioSignalPath.doctor.expand') }));
    expect(dialog.textContent).toContain('音频输出没准备好');
    expect(dialog.textContent).toContain('先重试播放');
    expect(dialog.textContent).not.toContain('stderrTail');
  });

  it('explains output fallback and actual sample rate in the signal path stability panel', async () => {
    window.localStorage.setItem('echo-next.locale', 'en-US');
    const track = makeTrack(42, { title: 'Fallback Signal Track', codec: 'flac', sampleRate: 192000, bitDepth: 24 });
    const fallbackStatus = {
      ...
      sharedDeviceSampleRate: 48000,
      resampling: true,
      warnings: ['exclusive_output_fell_back_to_shared'],
    };
    ...
    const dialog = await screen.findByRole('dialog', { name: 'Signal path' });
    expect(dialog.textContent).toContain('Stability feedback');
    expect(dialog.textContent).toContain('Using a playable fallback path');
    expect(dialog.textContent).toContain('Requested mode');
    expect(dialog.textContent).toContain('Exclusive output');
    expect(dialog.textContent).toContain('Actual chain');
    expect(dialog.textContent).toContain('Shared output');
    expect(dialog.textContent).toContain('Fell back from the requested path');
    expect(dialog.textContent).toContain('192kHz -> 48kHz');
    expect(dialog.textContent).toContain('Sample rate is being converted');
  });

  it('shows safe mode and DSP involvement in the signal path stability panel', async () => {
    window.localStorage.setItem('echo-next.locale', 'en-US');
    const track = makeTrack(43, { title: 'Safe Mode Signal Track', codec: 'mp3', sampleRate: 44100, bitDepth: 16 });
    const systemStatus = {
      ...audioStatus(track),
      outputDeviceName: 'System default output',
      outputMode: 'system' as const,
      outputBackend: 'system-audio',
      activeOutputBackendImpl: 'electron-html-audio',
      actualDeviceSampleRate: 44100,
      sharedDeviceSampleRate: null,
      bitPerfectCandidate: false,
      bitPerfectDisabledReason: 'system_audio_compatibility_mode',
      dspActive: true,
      replayGainEnabled: true,
      replayGainAppliedDb: -3.5,
      warnings: ['system_audio_compatibility_mode'],
    };

    render(
      <I18nProvider>
        <AudioSignalPathPopover isOpen={true} status={systemStatus} track={track} onClose={vi.fn()} />
      </I18nProvider>,
    );

    const dialog = await screen.findByRole('dialog', { name: 'Signal path' });
    expect(dialog.textContent).toContain('Safe mode is outputting');
    expect(dialog.textContent).toContain('Safe mode is using the Electron / HTMLAudio compatibility chain.');
    expect(dialog.textContent).toContain('System output');
    expect(dialog.textContent).toContain('Safe mode uses the browser audio chain');
    expect(dialog.textContent).toContain('ReplayGain');
    expect(dialog.textContent).toContain('Signal is passing through the ECHO processing chain');
  });
  ...
  it('shows ECHO SRC as upsampling in the signal path', () => {
    ...
    const status = {
      ...
      echoSrcFilterProfile: 'poly-sinc-hb' as const,
      echoSrcComputeBackend: 'cpu' as const,
      ...
      echoSrcRuntime: {
        ...
        filterProfile: 'poly-sinc-hb' as const,
        filterSlot: '1x' as const,
        qualityProfile: 'transparent' as const,
        tapCount: 255,
        firStageCount: null,
        firStageTapCounts: null,
        firStageProfiles: null,
        firProcessingMode: 'realtime' as const,
        firBatchFrames: null,
        ...
        window: 'blackman-harris' as const,
    ...
    const dialog = screen.getByRole('dialog', { name: '信号路径' });
    expect(dialog.textContent).toContain('ECHO SRC CPU FIR');
    expect(dialog.textContent).toContain('44.1kHz -> ECHO SRC 352.8kHz / CPU FIR / AMD Ryzen 7 9800X3D 8-Core Processor');
    expect(dialog.textContent).not.toContain('96kHz -> 48kHz');
  });

  it('labels CUDA FIR ECHO SRC in the signal path', () => {
    const track = makeTrack(37, { title: 'CUDA Upsampled Signal Track', codec: 'mp3', sampleRate: 44100, bitDepth: null });
    const status = {
      ...audioStatus(track),
      decoderOutputSampleRate: 352800,
      requestedOutputSampleRate: 352800,
      ...
      echoSrcQualityProfile: 'transparent' as const,
      echoSrcAdvancedModeEnabled: true,
      echoSrcFilterProfile: 'poly-sinc-gauss-long' as const,
      echoSrcComputeBackend: 'cuda' as const,
      echoSrcCudaActive: true,
      echoSrcCudaStatus: {
        available: true,
      ...
      echoSrcRuntime: {
        ...
        filterProfile: 'poly-sinc-gauss-long' as const,
        filterSlot: '1x' as const,
        qualityProfile: 'transparent' as const,
        tapCount: 1533,
        firStageCount: 3,
        firStageTapCounts: [1023, 255, 255],
        firStageProfiles: ['poly-sinc-gauss-long' as const, 'poly-sinc-hb' as const, 'poly-sinc-hb' as const],
        firProcessingMode: 'ultra' as const,
        ...
        firWorkerLastMs: null,
        firRealtimeRatio: null,
        window: 'gaussian' as const,
        phase: 'linear' as const,
        normalizedCutoff: 0.059375,
        transitionRatio: 0.05,
        stopbandAttenuationDb: 145,
        impulsePeakIndex: null,
        impulseEnergyCentroid: null,
        preRingingEnergyRatio: null,
        measuredStopbandPeakDb: null,
      ...
      resamplerEngine: 'default' as const,
      resamplerFallbackActive: false,
    };
    ...
    const dialog = screen.getByRole('dialog', { name: /信号路径|淇″彿璺緞/u });
    expect(dialog.textContent).toContain('ECHO SRC CUDA FIR');
    expect(dialog.textContent).toContain('44.1kHz -> ECHO SRC 352.8kHz / CUDA FIR / NVIDIA GeForce RTX 5070 Ti / 16GB VRAM / Filter 1x/Nx / poly-sinc-gauss-long -> poly-sinc-hb -> poly-sinc-hb / 3 stages / 1023+255+255 taps / ultra / batch 16384f / block 16384f / linear / Transparent');
    expect(dialog.textContent).not.toContain('SOXR / Transparent');
  });

  it('shows the actual ECHO SRC fallback backend in the signal path', () => {
    const track = makeTrack(38, { title: 'CUDA Fallback Track', codec: 'flac', sampleRate: 44100, bitDepth: 16 });
    const status = {
      ...audioStatus(track),
      decoderOutputSampleRate: 352800,
      requestedOutputSampleRate: 352800,
      ...
      echoSrcRuntime: {
        ...
        firStageTapCounts: null,
        firStageProfiles: null,
        firProcessingMode: null,
        firBatchFrames: null,
        firMaxBlockFrames: null,
        firLastInputFrames: null,
        firLastOutputFrames: null,
        firWorkerRequests: null,
        firWorkerAverageMs: null,
        ...
        measuredPassbandRippleDb: null,
        cudaActive: false,
        fallbackReason: 'src_cuda_worker_unavailable',
      },
      resamplerEngine: 'soxr' as const,
      resamplerFallbackActive: false,
    };
    ...
    expect(dialog.textContent).toContain('ECHO SRC fallback');
    expect(dialog.textContent).toContain('SOXR / Filter 1x / poly-sinc-gauss-long / 1023 taps / linear / fallback: src cuda worker unavailable / Transparent');
  ...
  it('keeps volume and playback speed popovers mutually exclusive', async () => {
    ...
    fireEvent.mouseEnter(screen.getByRole('button', { name: /Volume 100%/u }).parentElement!);
  ...
  it('keeps the pause button committed while a slow native pause still emits playing status', async () => {
    const track = makeTrack(1);
    const audioStatusHandlers: Array<(status: AudioStatus) => void> = [];
  ...
  it('queues play when the user clicks again before native pause settles', async () => {
    const track = makeTrack(1);
    let nativePlaybackStatus: PlaybackStatus = {
      state: 'playing',
      currentTrackId: track.id,
      positionMs: 4000,
      durationMs: track.duration * 1000,
    ...
    const pause = vi.fn(() =>
      new Promise<typeof nativePlaybackStatus>((resolve) => {
        resolvePause = () => {
          nativePlaybackStatus = {
            ...nativePlaybackStatus,
            state: 'paused',
          };
          resolve(nativePlaybackStatus);
        };
      }),
    );
    ...
    window.echo = {
      playback: {
        getStatus: vi.fn(async () => nativePlaybackStatus),
        playLocalFile: vi.fn(),
        play,
        pause,
        stop: vi.fn(),
  ...
  it('handles library, queue, search, playback-mode, mute, and mini-player global shortcut commands', async () => {
    const track = makeTrack(1);
    const globalShortcutHandlers: Array<(command: GlobalShortcutAction) => void> = [];
    ...
    await screen.findByText('Song 1');
    globalShortcutHandlers[0]?.('toggleCurrentTrackLiked');
    await waitFor(() => expect(toggleTrackLiked).toHaveBeenCalledWith(track.id));

    globalShortcutHandlers[0]?.('openPlaybackQueue');
    globalShortcutHandlers[0]?.('openSearch');
    expect(navigateQueue).toHaveBeenCalledTimes(1);
    expect(navigateRoute).toHaveBeenCalledWith(expect.objectContaining({ detail: 'search' }));

    globalShortcutHandlers[0]?.('toggleMute');
    await waitFor(() => expect(setOutput).toHaveBeenCalledWith({ volume: 0 }));
    ...
    globalShortcutHandlers[0]?.('cycleRepeatMode');
    await waitFor(() => expect(window.localStorage.getItem('echo-next:playback-mode')).toContain('"repeatMode":"one"'));

    window.removeEventListener('app:navigate:queue', navigateQueue);
  ...
  it('shows a loading animation for streaming tracks while playback is preparing', async () => {
    ...
    expect(container.querySelector('.player-cover')?.getAttribute('data-loading')).toBe('true');
    expect(container.querySelector('.player-bar')?.getAttribute('aria-busy')).toBe('true');
    expect(container.querySelector('.player-bar')?.getAttribute('data-network-loading')).toBe('true');
    expect(container.querySelector('.player-bar')?.getAttribute('data-playback-loading')).toBe('true');
    expect(container.querySelector('.progress-track')?.getAttribute('data-loading')).toBe('true');
  });

  it('keeps direct local playback out of preparing feedback while the audio engine opens the file', async () => {
    const track = makeTrack(1);
    const loadingAudioStatus = { ...audioStatus(track), state: 'loading' as const, positionSeconds: 0 };
    ...
    await screen.findByText('Song 1');
    expect(container.querySelector('.player-loading-hint')).toBeNull();
    expect(container.querySelector('.player-bar')?.getAttribute('data-network-loading')).toBeNull();
    expect(container.querySelector('.player-bar')?.getAttribute('data-playback-loading')).toBeNull();
    expect(container.querySelector('.player-bar')?.getAttribute('aria-busy')).not.toBe('true');
    expect(container.querySelector('.player-cover')?.getAttribute('data-loading')).toBeNull();
    expect(container.querySelector('.progress-track')?.getAttribute('data-loading')).toBeNull();
  });

  it('keeps same-path DLNA receiver status from taking over direct local playback', async () => {
    const track = makeTrack(1);
    const receiverStatus: ConnectReceiverStatus = {
      enabled: true,
      state: 'playing',
      advertisedName: 'ECHO DLNA',
      addresses: ['127.0.0.1'],
      currentClient: null,
      currentUri: track.path,
      metadata: {
        title: 'DLNA Ghost',
        artist: 'DLNA',
        album: 'DLNA',
    ...
    window.echo = {
      ...
      connect: {
        getStatus: vi.fn().mockResolvedValue(null),
        getReceiverStatus: vi.fn().mockResolvedValue(receiverStatus),
        onStatus: vi.fn(() => vi.fn()),
        onReceiverStatus: vi.fn(() => vi.fn()),
      },
      audio: {
        getStatus: vi.fn().mockResolvedValue(audioStatus(track)),
        onStatus: vi.fn(() => vi.fn()),
    ...
    await waitFor(() => expect(window.echo?.connect?.getReceiverStatus).toHaveBeenCalled());
    expect(screen.queryByText('DLNA Ghost')).toBeNull();
    expect(container.querySelector('.tag-dlna')).toBeNull();
  });
  ...
  it('uses the default progress track in the lyrics mini player even when waveform progress is enabled', async () => {
    const track = makeTrack(1, { duration: 180 });
    ...
    const { container } = render(
      <I18nProvider>
        <PlaybackQueueProvider>
          <QueueSeed lyricsMiniPlayer tracks={[track]} />
        </PlaybackQueueProvider>
      </I18nProvider>,
    );

    await screen.findByText('Song 1');
    await waitFor(() => expect(container.querySelector('.progress-track')).toBeTruthy());

    expect(container.querySelector('.progress-track')?.getAttribute('data-waveform')).toBeNull();
  ...
  it('advances queue when ended status arrives regardless of grace window', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    ...
    emitAudioStatus(statusHandlers, {
      ...audioStatus(firstTrack),
      state: 'ended',
      positionSeconds: firstTrack.duration - 2.848,
    });

    await new Promise((resolve) => window.setTimeout(resolve, 0));
    expect(playLocalFile).toHaveBeenCalled();
  });

  it('advances queue when native output still has a buffered tail', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
    ...
    emitAudioStatus(statusHandlers, {
      ...
      state: 'ended',
      positionSeconds: firstTrack.duration,
      nativeBufferedFrames: 136704,
      nativeBufferedMs: 2848,
    });
  ...
  it('advances queue when host signals ended state', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('does not auto-advance when playback stays playing at the track tail (watchdog removed)', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('advances queue when an ended status arrives before the track tail', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('advances queue when ended state arrives from the audio host', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('does NOT auto-advance even with ended state (host drives it now)', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('advances queue once per ended signal (no retry)', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
  ...
  it('tail watchdog does NOT fire (host drives it now)', async () => {
    const firstTrack = makeTrack(1);
    const secondTrack = makeTrack(2);
    const statusHandlers: Array<(status: AudioStatus) => void> = [];
    const playLocalFile = vi.fn().mockResolvedValue({
    ...
    window.echo = {
      ...
      audio: {
        getStatus: vi.fn().mockResolvedValue({
          ...audioStatus(firstTrack),
          positionSeconds: firstTrack.duration - 0.1,
        }),
        onStatus: vi.fn(subscribeAudioStatusHandlers(statusHandlers)),
        listDevices: vi.fn(),
        setOutput: vi.fn(),
  ...
  it('Spotify ended is synthesized when !isPlaying and progress within 350ms of duration', async () => {
    const spotifyTrack = makeTrack(1, {
      id: 'streaming:spotify:char-advance',
      path: 'streaming:spotify:char-advance',
      stableKey: 'streaming:spotify:char-advance',
      mediaType: 'streaming',
      provider: 'spotify',
      providerTrackId: 'char-advance',
      codec: 'spotify',
      sampleRate: null,
      bitDepth: null,
      bitrate: null,
    ...
    window.echo = {
      ...
      spotify: {
        ...
        getPlaybackState: vi.fn().mockResolvedValue({
          isPlaying: false,
          progressMs: spotifyTrack.duration * 1000 - 200,
          itemUri: 'spotify:track:char-advance',
          deviceId: 'spotify-device',
          deviceName: 'Spotify Desktop',
        }),
        ensureConnectDevice: vi.fn(),
