...
const spotifyEndedGraceSeconds = 0.35;
const trackSwitchVisualIntentPositionToleranceMs = 1500;
const playbackActionStatusGuardMs = 3500;
const committedPauseStatusMinGuardMs = 1000;
...
const readAudioExportFormatPatch = (patch: unknown): AudioExportFormat | null => {
  if (!patch || typeof patch !== 'object' || !Object.hasOwn(patch, 'audioExportFormat')) {
    return null;
  }
...
const readHiddenPlayerBarButtonIdsPatch = (patch: unknown): PlayerBarButtonId[] | null => {
  if (!patch || typeof patch !== 'object' || !Object.hasOwn(patch, 'hiddenPlayerBarButtonIds')) {
    return null;
  }
...
const buildDacArrivalCeremonyNotice = (
  ...
  return {
    ...
    detail: t('playerDacArrival.detail.exclusive', { mode: modeLabel }),
    modeLabel,
...
export const PlayerBar = ({
  ...
  lyricsCompactOnIdle = false,
  lyricsMiniPlayer = false,
  onOpenAudioSettings,
  onOpenQueue,
  ...
  const transportActionPromiseRef = useRef<Promise<PlaybackStatus | null> | null>(null);
  ...
  const shouldIgnoreAudioStatus = useCallback((nextAudioStatus: AudioStatus): boolean => {
    ...
    if (elapsedMs < playbackActionStatusGuardMs && !samePlayback && (nextAudioStatus.currentTrackId || nextAudioStatus.currentFilePath)) {
      return true;
    }

    if (elapsedMs < playbackActionStatusGuardMs && samePlayback && nextAudioStatus.state !== lastAction.state) {
      return true;
    }

    if (
      elapsedMs >= playbackActionStatusGuardMs ||
      (
        nextAudioStatus.state === lastAction.state &&
        (lastAction.state !== 'paused' || elapsedMs >= committedPauseStatusMinGuardMs)
      )
    ) {
  ...
  const currentTrackIsDirectLocal =
    currentTrack !== null &&
    !isReceiverTrackId(currentTrack.id) &&
    (currentTrack.mediaType ?? 'local') === 'local';
  const directLocalPlaybackOwnsCurrentTrack =
    currentTrackIsDirectLocal &&
    (baseState === 'loading' || baseState === 'playing' || baseState === 'paused') &&
    (Boolean(playbackAudioStatus?.currentTrackId && trackMatchesPlaybackIdentity(currentTrack, playbackAudioStatus.currentTrackId)) ||
      Boolean(playbackAudioStatus?.currentFilePath && trackMatchesPlaybackIdentity(currentTrack, playbackAudioStatus.currentFilePath)) ||
      Boolean(currentPlaybackStatus?.currentTrackId && trackMatchesPlaybackIdentity(currentTrack, currentPlaybackStatus.currentTrackId)) ||
      Boolean(currentPlaybackStatus?.filePath && trackMatchesPlaybackIdentity(currentTrack, currentPlaybackStatus.filePath)));
  ...
  const isReceiverPlaybackActive = Boolean(
    receiverHasCurrentMedia &&
      !directLocalPlaybackOwnsCurrentTrack &&
  ...
  const boundedCompactPositionSeconds =
    durationSeconds > 0 ? Math.min(durationSeconds, Math.max(0, positionSeconds)) : 0;
  const compactProgressPercent =
    durationSeconds > 0 ? Math.min(100, Math.max(0, (boundedCompactPositionSeconds / durationSeconds) * 100)) : 0;
  ...
  // The native audio host cannot observe Spotify playback state directly.
  useEffect(() => {
    ...
    const syncSpotifyProgress = async (): Promise<void> => {
      try {
        ...
        const endedAtTrackTail =
          ...
          durationMs > 0 &&
          progressMs >= Math.max(0, durationMs - spotifyEndedGraceSeconds * 1000);
  ...
  useEffect(() => {
    ...
    void isConnectDonatorUnlocked().then((unlocked) => {
      ...
      unsubscribe = connect?.onReceiverStatus?.((status) => {
        setReceiverStatus(status);
  ...
  const runSerializedTransportAction = useCallback(
    async (action: () => Promise<PlaybackStatus | null>): Promise<PlaybackStatus | null> => {
      const previousAction = transportActionPromiseRef.current;
      const actionPromise = (async (): Promise<PlaybackStatus | null> => {
        if (previousAction) {
          await previousAction.catch(() => null);
        }

        return runPlaybackAction(action);
      })();

      transportActionPromiseRef.current = actionPromise;
      try {
        return await actionPromise;
      } finally {
        if (transportActionPromiseRef.current === actionPromise) {
          transportActionPromiseRef.current = null;
        }
      }
    },
    [runPlaybackAction],
  ...
  const handlePlayPause = useCallback(async (): Promise<void> => {
    ...
    await runSerializedTransportAction(async () => {
      if (visualState === 'playing' || visualState === 'loading') {
  ...
  }, [activeReceiverStatus, applyConnectPlaybackStatus, currentTrack, isNativePausedVisualState, isSpotifyCurrentTrack, publishOptimisticPause, queue, runPlaybackAction, runSerializedTransportAction, visualState]);
  ...
  return (
    <>
      ...
      <div className="player-center">
        ...
        <PlayerProgress
          disabled={isLiveStreamPlayback || (!filePath && !isSpotifyCurrentTrack)}
          durationSeconds={durationSeconds}
          isLoading={isPlaybackPreparing}
          waveformEnabled={
            !lyricsMiniPlayer &&
            playerWaveformProgressEnabled &&
            !lowLoadPlaybackModeEnabled &&
            !isPlaybackPreparing
          }
