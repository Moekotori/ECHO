...
const staleStatusLogDedupeMs = 10_000;
...
const isExpiredVisualIntent = (intent: PlaybackVisualIntent | null, now = Date.now()): boolean =>
  Boolean(intent && now - intent.startedAtMs > trackSwitchPositionGuardMs);
...
const shouldClearVisualIntentForPatch = (
  ...
  intent: PlaybackVisualIntent | null = snapshot.playbackVisualIntent,
): boolean => {
  if (!intent) {
    return false;
  }
...
export const setPlaybackStatusSnapshot = (patch: Partial<Omit<PlaybackStatusSnapshot, 'version'>>): PlaybackStatusSnapshot => {
  ...
  const currentVisualIntent = snapshot.playbackVisualIntent;
  const visualIntentExpired = isExpiredVisualIntent(currentVisualIntent);
  const activeVisualIntent = visualIntentExpired ? null : currentVisualIntent;
  ...
  const shouldApplyPlaybackStatus =
    ...
    (playbackStatusStartsNewIntent || !patch.playbackStatus || !isStaleStatusForVisualIntent(patch.playbackStatus, activeVisualIntent));
  const shouldApplyAudioStatus =
    !nativeRefreshOverSpotify &&
    !nativeAudioOverSpotify &&
    (!patch.audioStatus || !isStaleStatusForVisualIntent(patch.audioStatus, activeVisualIntent));

  if (hasPlaybackStatusPatch) {
    refreshRequestId += 1;
  }

  const shouldClearVisualIntent = visualIntentExpired || shouldClearVisualIntentForPatch(patch, shouldApplyPlaybackStatus, shouldApplyAudioStatus, activeVisualIntent);
  if (hasAudioStatusPatch && !shouldApplyAudioStatus && patch.audioStatus) {
    logLyricsConsole(
      ...
      {
        level: 'warn',
        dedupeKey: 'reject-audio',
        dedupeMs: staleStatusLogDedupeMs,
      },
    );
  }
  ...
  if (shouldClearVisualIntent) {
    logLyricsConsole(
      visualIntentExpired ? 'clock.intent-expired' : 'clock.intent-cleared',
      {
        intent: summarizeIntentForLyricsLog(snapshot.playbackVisualIntent),
...
export const useThrottledSharedPlaybackStatus = (minIntervalMs: number): PlaybackStatusSnapshot => {
  const subscribeThrottled = useMemo(() => {
    const intervalMs = Math.max(0, Math.round(minIntervalMs));
    if (intervalMs <= 0) {
      return subscribe;
    }
    ...
    return (listener: () => void): (() => void) => {
      ...
      const unsubscribe = subscribe(() => {
        ...
        notifyTimer = window.setTimeout(notify, Math.max(0, intervalMs - elapsedMs));
      });
