...
const validThemeModes: AppThemeMode[] = ['light', 'dark', 'system', 'ambient'];
...
const isSettingsUsingProTheme = (settings: Partial<AppSettings>): boolean =>
  lockedThemePresets.has(settings.appearanceThemePreset as AppThemePreset) ||
  normalizeThemeCustomThemes(settings.appearanceCustomThemes ?? []).some((theme) => theme.id === settings.appearanceThemeCustomId && lockedThemePresets.has(theme.basePreset));

const resolveSettingsThemeUnlocked = (settings: Partial<AppSettings>, options: ThemePresetOptions = {}): boolean =>
  options.finalThemeUnlocked === true ||
  settings.finalThemeUnlockVersion === finalThemeUnlockVersion ||
  isSettingsUsingProTheme(settings);
...
export const resolveThemeMode = (mode: AppThemeMode): EffectiveTheme => {
  ...
  if (normalized === 'dark' || normalized === 'ambient') {
    return 'dark';
  }
...
export const applyThemeSettings = (settings: Partial<AppSettings>, options: ThemeApplyOptions = {}): EffectiveTheme => {
  const finalThemeUnlocked = resolveSettingsThemeUnlocked(settings, options);
...
const applyThemeModeNow = (
  ...
  const ambientMode = normalized === 'ambient';
  const activeCustomTheme = ambientMode ? undefined : normalizedCustomThemes.find((theme) => theme.id === normalizedCustomThemeId);
  const effectivePreset = ambientMode ? 'classic' : activeCustomTheme?.basePreset ?? normalizedPreset;
  const effectiveTheme = resolveThemeMode(normalized);
  const root = document.documentElement;
  const toneOverride = ambientMode ? undefined : activeCustomTheme?.[effectiveTheme] ?? normalizedOverrides[effectivePreset]?.[effectiveTheme];
...
export const loadPersistedThemeMode = async (): Promise<AppThemeMode> => {
  ...
  const finalThemeUnlocked = resolveSettingsThemeUnlocked(settings);
