...
describe('theme preferences', () => {
  it('applies and caches explicit theme modes', () => {
    ...
    updateThemeMode('ambient');

    expect(readThemeMode()).toBe('ambient');
  ...
  it('trusts a Pro theme returned by app settings without the legacy marker', () => {
    applyThemeSettings({
  ...
  it('keeps ambient standalone instead of layering preset or custom theme overrides', () => {
    const customThemes = [
      {
        id: 'theme-safe',
        name: 'Safe Theme',
    ...
    expect(document.documentElement.dataset.themePreset).toBe('classic');
    expect(document.documentElement.dataset.themeCustom).toBeUndefined();
    expect(document.documentElement.dataset.themeCustomId).toBeUndefined();
    expect(document.documentElement.style.getPropertyValue('--preset-app-bg')).toBe('');
    expect(document.documentElement.style.getPropertyValue('--theme-accent-solid-bg')).toBe('');
  });
  ...
  it('keeps ambient mode active when scheduled theme switching is enabled', () => {
    const settings = {
      appearanceTheme: 'ambient' as const,
      appearanceThemeScheduleEnabled: true,
      appearanceThemeScheduleDarkAt: '20:00',
      appearanceThemeScheduleLightAt: '07:00',
      appearanceThemePreset: 'classic' as const,
    ...
    expect(resolveThemeModeForSchedule(settings, new Date('2026-05-29T14:00:00'))).toBe('ambient');
