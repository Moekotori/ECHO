...
export const DesktopLyricsApp = (): JSX.Element => {
  ...
  const [desktopLyricsWindowVisible, setDesktopLyricsWindowVisible] = useState(true);
  ...
  useEffect(() => {
    ...
    const loadSettings = async (): Promise<void> => {
      try {
        ...
        setDesktopLyricsWindowVisible(desktopState?.visible !== false);
        setEnhancedLowLoadPlaybackActive(readEnhancedLowLoadPlaybackActive(appSettings));
      } catch {
        if (!disposed) {
          setSettings(fallbackSettings);
          setDesktopLyricsWindowVisible(true);
    ...
    const unsubscribe = window.echo?.desktopLyrics?.onStateChanged?.((state: DesktopLyricsState) => {
      ...
      setDesktopLyricsWindowVisible(state.visible);
    });
  ...
  useEffect(() => {
    ...
    const syncUpdateLoop = (): void => {
      ...
      if (document.visibilityState === 'visible' && desktopLyricsWindowVisible) {
        animationFrameRef.current = window.requestAnimationFrame(tick);
      } else {
        hiddenTimerId = window.setInterval(sync, enhancedLowLoadLyricSyncIntervalMs);
      }
    };
  ...
  }, [activeClock, desktopLyricsWindowVisible, enhancedLowLoadPlaybackActive, lyrics]);
