...
export function EqPlot({
  ...
  const draw = useCallback(() => {
    ...
    if (enabled && analyser) {
      ...
      rtaGrad.addColorStop(1, "rgba(247, 170, 181, 0.15)");
    ...
    grad.addColorStop(0, "rgba(247, 170, 181, 0.15)");
    grad.addColorStop(0.5, "rgba(247, 170, 181, 0.05)");
    grad.addColorStop(1, "rgba(247, 170, 181, 0)");
    ctx.fillStyle = grad;
    ctx.fill();

    ctx.beginPath();
    ctx.strokeStyle = "rgba(247, 170, 181, 1)";
    ctx.lineWidth = 2.5;
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    for (let i = 0; i < frequencies.length; i++) {
  ...
  }, [bands, enabled, frequencies, draggingIdx, hoverIdx, analyser]);
  ...
  return (
    <div className="hi-fi-eq-plot-main-wrapper no-drag" ref={containerRef}>
      <div className="preamp-vertical-container">
        ...
        <span className="preamp-label-title">{tr("PREAMP", "前级")}</span>
      </div>
      <div className="eq-plot-with-labels-container">
        ...
        <div className="eq-selected-info-bar">
          <div className="info-item">
            {tr("FREQ", "频率")}{" "}
            <b>{activeNode ? Math.round(activeNode.freq) : "--"} Hz</b>
          </div>
          <div className="info-item">
            {tr("GAIN", "增益")}{" "}
            <b>{activeNode ? activeNode.gain.toFixed(1) : "--"} dB</b>
          </div>
          <div className="info-item">
            Q{" "}
            <b>
