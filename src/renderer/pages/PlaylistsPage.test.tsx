...
describe('PlaylistsPage actions menu', () => {
  ...
  it('filters the playlist sidebar by local and streaming sources', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '歌单类型筛选' }));
    fireEvent.click(screen.getByRole('option', { name: '本地' }));
    await waitFor(() => expect(getPlaylistNames()).toEqual(['Road Mix']));

    fireEvent.click(screen.getByRole('button', { name: '歌单类型筛选' }));
    fireEvent.click(screen.getByRole('option', { name: '流媒体' }));
    await waitFor(() => expect(getPlaylistNames()).toEqual(['Cloud Mix']));
  });

  it('syncs NetEase account playlists from the explicit sync button', async () => {
    const localPlaylist = playlist({ id: 'playlist-local', name: 'Road Mix', sourceProvider: 'local', itemCount: 1 });
    const syncedPlaylist = playlist({
      id: 'playlist-netease',
      name: 'Cloud Mix',
      sourceProvider: 'netease',
      sourcePlaylistId: '1001',
      itemCount: 3,
    });
    ...
    window.echo = {
      ...
      library: {
        getPlaylists: vi.fn().mockResolvedValueOnce([localPlaylist]).mockResolvedValue([localPlaylist, syncedPlaylist]),
        getPlaylistItems: vi.fn().mockResolvedValue(page([item()])),
        getLikedTrackIds: vi.fn().mockResolvedValue({}),
      },
      playback: {
    ...
    await screen.findAllByText('Road Mix');
    fireEvent.click(screen.getByRole('button', { name: '歌单类型筛选' }));
    fireEvent.click(screen.getByRole('option', { name: '网易云' }));
    expect(listAccountPlaylists).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole('button', { name: '同步' }));

    await waitFor(() => expect(listAccountPlaylists).toHaveBeenCalledWith('netease'));
    await waitFor(() => expect(importPlaylistFromUrl).toHaveBeenCalledWith('https://music.163.com/#/playlist?id=1001'));
    expect(await screen.findByText('已同步网易云歌单：1 个歌单 / 3 首')).toBeTruthy();

    const sidebar = document.querySelector('.playlist-sidebar') as HTMLElement;
    await waitFor(() =>
      expect(Array.from(sidebar.querySelectorAll('.playlist-list-item strong span')).map((element) => element.textContent ?? '')).toEqual(['Cloud Mix']),
    );
  });

  it('separates local and streaming playlist sidebar context menus', async () => {
    const writeText = installClipboardWriteMock();
    const localPlaylist = playlist({ id: 'playlist-local', name: 'Road Mix', sourceProvider: 'local', itemCount: 1 });
    const streamingPlaylist = playlist({
      id: 'playlist-streaming',
    ...
    fireEvent.contextMenu(getPlaylistButton('Road Mix'), { clientX: 32, clientY: 48 });
    let menu = document.querySelector('.playlist-context-menu') as HTMLElement;
    expect(within(menu).getByRole('menuitem', { name: '重命名歌单' })).toBeTruthy();
    expect(within(menu).queryByRole('menuitem', { name: '分享歌单' })).toBeNull();

    fireEvent.contextMenu(getPlaylistButton('Cloud Mix'), { clientX: 32, clientY: 48 });
    menu = document.querySelector('.playlist-context-menu') as HTMLElement;
    expect(getPlaylistButton('Road Mix').dataset.active).toBe('true');
    expect(getPlaylistButton('Cloud Mix').dataset.active).toBeUndefined();
    expect(within(menu).getByRole('menuitem', { name: '分享歌单' })).toBeTruthy();
    expect(within(menu).queryByRole('menuitem', { name: '添加本地歌曲' })).toBeNull();

    fireEvent.click(within(menu).getByRole('menuitem', { name: '分享歌单' }));

    await waitFor(() => expect(writeText).toHaveBeenCalledWith('https://music.163.com/#/playlist?id=123456'));
    expect(await screen.findByText('歌单链接已复制')).toBeTruthy();
  });

  it('portals the playlist sidebar context menu to the document body for viewport positioning', async () => {
    const localPlaylist = playlist({ id: 'playlist-local', name: 'Road Mix', sourceProvider: 'local', itemCount: 1 });
    window.echo = {
      library: {
        getPlaylists: vi.fn().mockResolvedValue([localPlaylist]),
        getPlaylistItems: vi.fn().mockResolvedValue(page([])),
        getLikedTrackIds: vi.fn().mockResolvedValue({}),
      },
      playback: {
    ...
    const { container } = renderPlaylistsPage();

    await screen.findAllByText('Road Mix');
    const playlistButton = container.querySelector('.playlist-list-item') as HTMLElement;
    fireEvent.contextMenu(playlistButton, { clientX: 32, clientY: 48 });

    const layer = document.querySelector('.playlist-context-menu-layer') as HTMLElement;
    const menu = document.querySelector('.playlist-context-menu') as HTMLElement;
  ...
  it('deletes an imported streaming playlist from the sidebar context menu', async () => {
    const localPlaylist = playlist({ id: 'playlist-local', name: 'Road Mix', sourceProvider: 'local', itemCount: 1 });
    const streamingPlaylist = playlist({
      id: 'playlist-streaming',
      name: 'Cloud Mix',
    ...
    window.echo = {
      library: {
        getPlaylists: vi.fn().mockResolvedValueOnce([localPlaylist, streamingPlaylist]).mockResolvedValue([localPlaylist]),
        getPlaylistItems: vi.fn().mockResolvedValue(page([])),
        getLikedTrackIds: vi.fn().mockResolvedValue({}),
        deletePlaylist,
      },
    ...
    fireEvent.contextMenu(getPlaylistButton('Road Mix'), { clientX: 32, clientY: 48 });
    expect(within(document.querySelector('.playlist-context-menu') as HTMLElement).getByRole('menuitem', { name: '删除本地歌单' })).toBeTruthy();

    fireEvent.contextMenu(getPlaylistButton('Cloud Mix'), { clientX: 32, clientY: 48 });
    const menu = document.querySelector('.playlist-context-menu') as HTMLElement;
    expect(menu).toBeTruthy();

    fireEvent.click(within(menu).getByRole('menuitem', { name: '移除流媒体歌单' }));

    await waitFor(() => expect(deletePlaylist).toHaveBeenCalledWith('playlist-streaming'));
    expect(confirm).toHaveBeenCalledWith(expect.stringContaining('不会删除平台上的原歌单'));
    await waitFor(() => expect(screen.queryByText('Cloud Mix')).toBeNull());
    expect(await screen.findByText('流媒体歌单已删除：Cloud Mix')).toBeTruthy();
  });
  ...
  it('imports Spotify playlist links from the sidebar form', async () => {
    ...
    const dialog = await screen.findByRole('dialog', { name: '流媒体功能须知' });
    expect(importPlaylistFromUrl).not.toHaveBeenCalled();
    fireEvent.change(within(dialog).getByRole('textbox'), { target: { value: '我同意' } });
    fireEvent.click(within(dialog).getByRole('button', { name: '我同意并继续' }));

    await waitFor(() =>
      expect(setSettings).toHaveBeenCalledWith({ streamingPlaylistImportNoticeAccepted: true }),
    );
    ...
    const secondUrl = 'https://open.spotify.com/playlist/second';
    const secondInput = await screen.findByPlaceholderText('粘贴网易云 / QQ 音乐 / Spotify 歌单链接');
    fireEvent.change(secondInput, { target: { value: secondUrl } });
    fireEvent.submit(secondInput.closest('form') as HTMLFormElement);

    expect(screen.queryByRole('dialog', { name: '流媒体功能须知' })).toBeNull();
    await waitFor(() => expect(importPlaylistFromUrl).toHaveBeenCalledWith(secondUrl));
  });
