...
describe('ConnectPage HQPlayer controls', () => {
  ...
  it('resolves and plays a live room from a single Connect live link', async () => {
    const bridge = installEchoBridge(hqStatus('available'));
    renderConnectPage();
    ...
    await waitFor(() => expect(bridge.streaming.resolveLive).toHaveBeenCalledWith({
      url: 'https://live.bilibili.com/21465419?live_from=71002&visit_id=2iess9qeic80',
    }));
    await waitFor(() => expect(queueMock.playTrack).toHaveBeenCalledWith(
      expect.objectContaining({
        id: expect.stringMatching(/^live-stream:/u),
        mediaType: 'streaming',
        isLiveStream: true,
        provider: 'm3u8',
        title: '直播帕鲁',
        coverThumb: 'echo-image://remote/cover',
        liveVideoUrl: 'https://bilibili.example.test/live.m3u8',
      }),
      expect.objectContaining({
        routeToConnectOutput: false,
        forceRefresh: true,
    ...
    expect(JSON.parse(window.localStorage.getItem('echo.connect.radioStations.v2') ?? '[]')).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ url: 'https://live.bilibili.com/21465419?live_from=71002&visit_id=2iess9qeic80' })]),
    );
  });
