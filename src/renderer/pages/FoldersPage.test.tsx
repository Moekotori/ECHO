...
let libraryMock: {
  ...
  importOsuFolder: ReturnType<typeof vi.fn>;
  scanFolder: ReturnType<typeof vi.fn>;
  scanFolderChanges: ReturnType<typeof vi.fn>;
  rescanEmbeddedTags: ReturnType<typeof vi.fn>;
  refreshAlbumGrouping: ReturnType<typeof vi.fn>;
  refreshAlbumGroupingPlaybackSafe: ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  ...
  libraryMock = {
    ...
    addFolder: vi.fn(),
    importOsuFolder: vi.fn(),
    scanFolder: vi.fn(),
    scanFolderChanges: vi.fn(),
    rescanEmbeddedTags: vi.fn().mockResolvedValue([scanStatus({ folderId: 'folder-1' })]),
    refreshAlbumGrouping: vi.fn().mockResolvedValue(summary({ albumCount: 3, artistCount: 2 })),
    refreshAlbumGroupingPlaybackSafe: vi.fn().mockResolvedValue({ status: 'completed', summary: summary({ albumCount: 3, artistCount: 2 }) }),
    removeFolder: vi.fn(),
...
describe('FoldersPage', () => {
  ...
  it('does not expose the disabled osu folder import from the folders import panel', async () => {
    renderFoldersPage();

    await screen.findByRole('heading', { name: 'Folders' });

    expect(screen.queryByRole('button', { name: /osu!/u })).toBeNull();
    expect(libraryMock.importOsuFolder).not.toHaveBeenCalled();
  });

  it('shows first-import next steps when no local folders exist', async () => {
    libraryMock.getFolderOverviews.mockResolvedValue([]);

    renderFoldersPage();

    expect(await screen.findByText('No library folders yet.')).toBeTruthy();
    expect(screen.getByText(/Choose your music folder first/)).toBeTruthy();
    expect(screen.getByText(/Bad files will not block the whole import/)).toBeTruthy();
    expect(screen.getByText(/Adding a folder starts a background scan/)).toBeTruthy();
  });

  it('shows post-scan actions when the first import completes', async () => {
    libraryMock.getFolderOverviews.mockResolvedValue([
      overview({
        recentScan: scanStatus({
          status: 'completed',
          phase: 'finished',
          processedFiles: 2,
          totalFiles: 2,
    ...
    expect(await screen.findByText('After Scan Completes')).toBeTruthy();
    expect(screen.getByText(/play this folder or open subfolders/i)).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Play This Folder' })).toBeTruthy();
  });
  ...
  it('repairs the global album index from the local folder management panel', async () => {
    const changedHandler = vi.fn();
    window.addEventListener('library:changed', changedHandler);
    ...
    try {
      ...
      await screen.findByRole('heading', { name: 'Folders' });
      fireEvent.click(screen.getByRole('button', { name: 'Repair Album Index' }));

      expect(await screen.findByText('Repairing album index now. UI and playback may pause briefly.')).toBeTruthy();
      await waitFor(() => expect(libraryMock.refreshAlbumGrouping).toHaveBeenCalledTimes(1));
      expect(libraryMock.refreshAlbumGroupingPlaybackSafe).not.toHaveBeenCalled();
      await waitFor(() => expect(libraryMock.getFolderOverviews).toHaveBeenCalledTimes(2));
      expect(changedHandler).toHaveBeenCalledTimes(1);
      expect(await screen.findByText('Album index repaired: 3 albums / 2 artists.')).toBeTruthy();
    } finally {
      window.removeEventListener('library:changed', changedHandler);
    }
  });

  it('disables album index repair while a local scan is running', async () => {
    act(() => {
    ...
    const repairButton = await screen.findByRole('button', { name: 'Repair Album Index' });
    expect((repairButton as HTMLButtonElement).disabled).toBe(true);
    fireEvent.click(repairButton);
    expect(libraryMock.refreshAlbumGroupingPlaybackSafe).not.toHaveBeenCalled();
    expect(libraryMock.refreshAlbumGrouping).not.toHaveBeenCalled();
  });

  it('shows upper-left chrome notices before and after direct album index repair', async () => {
    ...
    try {
      ...
      await waitFor(() => expect(libraryMock.refreshAlbumGrouping).toHaveBeenCalledTimes(1));
      expect(noticeHandler).toHaveBeenCalledTimes(2);
      expect((noticeHandler.mock.calls[0]?.[0] as CustomEvent<string>).detail).toBe('Repairing album index now. UI and playback may pause briefly.');
      expect((noticeHandler.mock.calls[1]?.[0] as CustomEvent<string>).detail).toBe('Album index repaired: 3 albums / 2 artists.');
    } finally {
  ...
  it('shows real scan stages and an import summary after a local scan completes', async () => {
    act(() => {
      rememberLibraryScanStatus(scanStatus({
        status: 'completed',
        phase: 'finished',
    ...
    expect(screen.getByText('Reading metadata')).toBeTruthy();
    expect(screen.getAllByText(/Skipping cache|Cache skipped/).length).toBeGreaterThan(0);
