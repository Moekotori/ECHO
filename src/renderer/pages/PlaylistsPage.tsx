...
type AccountPlaylistSyncProvider = Extract<StreamingProviderName, 'netease' | 'qqmusic'>;
type PlaylistSourceFilter = 'all' | 'local' | 'streaming' | AccountPlaylistSyncProvider;
const playlistSourceFilterOptions: Array<{ value: PlaylistSourceFilter; labelKey: 'playlistsPage.filter.all' | 'playlistsPage.filter.local' | 'playlistsPage.filter.streaming' | 'playlistsPage.filter.netease' | 'playlistsPage.filter.qqmusic' }> = [
  { value: 'all', labelKey: 'playlistsPage.filter.all' },
  { value: 'local', labelKey: 'playlistsPage.filter.local' },
  { value: 'streaming', labelKey: 'playlistsPage.filter.streaming' },
  { value: 'netease', labelKey: 'playlistsPage.filter.netease' },
  { value: 'qqmusic', labelKey: 'playlistsPage.filter.qqmusic' },
];
type PlaylistContextMenuState = { playlist: LibraryPlaylist; position: { x: number; y: number } };
...
const isDeletableStreamingPlaylist = (playlist: LibraryPlaylist): boolean =>
  playlist.sourceProvider !== 'local' && playlist.kind !== 'system';
...
export const PlaylistsPage = (): JSX.Element => {
  ...
  const [pendingAccountPlaylistSyncProvider, setPendingAccountPlaylistSyncProvider] = useState<AccountPlaylistSyncProvider | null>(null);
  const [syncingAccountPlaylistProvider, setSyncingAccountPlaylistProvider] = useState<AccountPlaylistSyncProvider | null>(null);
  const [accountPlaylistSyncMessage, setAccountPlaylistSyncMessage] = useState<string | null>(null);
  ...
  const [sourceFilterMenuOpen, setSourceFilterMenuOpen] = useState(false);
  ...
  const [playlistContextMenu, setPlaylistContextMenu] = useState<PlaylistContextMenuState | null>(null);
  ...
  const sourceFilterMenuRef = useRef<HTMLDivElement | null>(null);
  ...
  const filteredOrderedPlaylists = useMemo(
    () =>
      orderedPlaylists.filter((playlist) => {
        ...
        if (playlistSourceFilter === 'netease' || playlistSourceFilter === 'qqmusic') {
          return playlist.sourceProvider === playlistSourceFilter;
        }
        return true;
      }),
  ...
  const isSelectedPlaylistNeteaseDailyRecommend = isNeteaseDailyRecommendPlaylist(selectedPlaylist);
  ...
  const canDownloadSelectedPlaylist = canDownloadPlaylist(selectedPlaylist, downloadsFeatureUnlocked);
  ...
  useEffect(() => {
    if (!sourceFilterMenuOpen) {
      return undefined;
    }

    const handlePointerDown = (event: MouseEvent): void => {
      if (!sourceFilterMenuRef.current?.contains(event.target as Node)) {
  ...
  const handleRefreshStreamingPlaylist = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const playlist = playlistOverride ?? selectedPlaylist;
    const playlistUrl = playlist ? streamingPlaylistUrl(playlist) : null;

    if (playlist) {
      setSelectedPlaylistId(playlist.id);
    }

    if (isNeteaseDailyRecommendPlaylist(playlist)) {
      await handleRefreshNeteaseDailyRecommend();
      return;
    }

    const streaming = window.echo?.streaming;
    if (!streaming?.importPlaylistFromUrl || !playlist || !playlistUrl) {
      if (playlist) {
        await loadPlaylists();
    ...
    try {
      const result = await streaming.importPlaylistFromUrl(playlistUrl);
      await loadPlaylists();
  ...
  const syncAccountPlaylistsForProvider = useCallback(async (provider: AccountPlaylistSyncProvider): Promise<void> => {
    const streaming = getStreamingBridge();
    if (!streaming?.listAccountPlaylists || !streaming.importPlaylistFromUrl || syncingAccountPlaylistProvider) {
      return;
    }

    const providerLabel = t(provider === 'netease' ? 'playlistsPage.filter.netease' : 'playlistsPage.filter.qqmusic');
    setSyncingAccountPlaylistProvider(provider);
    setError(null);
    setSpotifyPlaylistHelpUrl(null);
    setStatusMessage(null);
    setAccountPlaylistSyncMessage(t('playlistsPage.status.syncingAccountPlaylists', { provider: providerLabel }));

    try {
      const result = await streaming.listAccountPlaylists(provider);
      const playlistsToSync: StreamingAccountPlaylist[] = result.playlists.filter((playlist) => playlist.webUrl.trim().length > 0);
      setAccountPlaylistSyncMessage(t('playlistsPage.status.syncingAccountPlaylistsProgress', { provider: providerLabel, current: 0, total: playlistsToSync.length }));

      if (playlistsToSync.length === 0) {
        await loadPlaylists();
        const message = t('playlistsPage.status.noAccountPlaylists', { provider: providerLabel });
        setAccountPlaylistSyncMessage(message);
        return;
      }
      ...
      let syncedTracks = 0;
      let firstSyncedPlaylistId: string | null = null;
      let firstError: unknown = null;

      let completedPlaylists = 0;
      let nextPlaylistIndex = 0;
      const updateSyncProgress = (): void => {
        const message = t('playlistsPage.status.syncingAccountPlaylistsProgress', {
      ...
      const syncNextPlaylist = async (): Promise<void> => {
        for (;;) {
          const playlist = playlistsToSync[nextPlaylistIndex];
          nextPlaylistIndex += 1;
          if (!playlist) {
            return;
          }
          ...
          try {
            const imported = await streaming.importPlaylistFromUrl(playlist.webUrl);
            syncedPlaylists += 1;
            syncedTracks += imported.importedCount;
            firstSyncedPlaylistId = firstSyncedPlaylistId ?? imported.playlistId;
          } catch (playlistError) {
            failedPlaylists += 1;
            firstError = firstError ?? playlistError;
          } finally {
      ...
      const workerCount = Math.min(3, playlistsToSync.length);
      await Promise.all(Array.from({ length: workerCount }, () => syncNextPlaylist()));

      await loadPlaylists();
      if (firstSyncedPlaylistId) {
        setSelectedPlaylistId(firstSyncedPlaylistId);
        setPlaylistSearchInput('');
        setPlaylistSearch('');
        await loadItems(firstSyncedPlaylistId, 1, 'replace', '');
      }

      window.dispatchEvent(new Event('library:playlists-changed'));

      if (syncedPlaylists === 0 && firstError) {
        throw firstError;
      }

      const finalMessage =
        failedPlaylists > 0
          ? t('playlistsPage.status.syncedAccountPlaylistsPartial', { provider: providerLabel, playlists: syncedPlaylists, failed: failedPlaylists, tracks: syncedTracks })
          : t('playlistsPage.status.syncedAccountPlaylists', { provider: providerLabel, playlists: syncedPlaylists, tracks: syncedTracks });
      setAccountPlaylistSyncMessage(finalMessage);
    } catch (syncError) {
      setError(syncError instanceof Error ? syncError.message : String(syncError));
      setStatusMessage(null);
      setAccountPlaylistSyncMessage(null);
    } finally {
      setSyncingAccountPlaylistProvider((current) => (current === provider ? null : current));
    }
  }, [loadItems, loadPlaylists, syncingAccountPlaylistProvider, t]);

  const requestAccountPlaylistSync = useCallback((provider: AccountPlaylistSyncProvider): void => {
    if (syncingAccountPlaylistProvider) {
      return;
    }

    if (!streamingPlaylistImportNoticeAccepted) {
      setPendingAccountPlaylistSyncProvider(provider);
      setStreamingPlaylistNoticeConsent('');
      setStreamingPlaylistNoticeOpen(true);
      return;
    }

    void syncAccountPlaylistsForProvider(provider);
  }, [streamingPlaylistImportNoticeAccepted, syncAccountPlaylistsForProvider, syncingAccountPlaylistProvider]);

  const handlePlaylistSourceFilterChange = useCallback((nextFilter: PlaylistSourceFilter): void => {
    setPlaylistSourceFilter(nextFilter);
    setSourceFilterMenuOpen(false);
  }, []);
  ...
  const handlePlaylistContextMenu = (event: ReactMouseEvent<HTMLButtonElement>, playlist: LibraryPlaylist): void => {
    event.preventDefault();
    event.stopPropagation();
    setPlaylistContextMenu({
      playlist,
      position: {
        x: Math.min(event.clientX, Math.max(8, window.innerWidth - 264)),
        y: Math.min(event.clientY, Math.max(8, window.innerHeight - 360)),
      },
    });
  };

  const handleDeleteStreamingPlaylistFromContextMenu = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? playlistContextMenu?.playlist;
    setPlaylistContextMenu(null);
    if (!library || !playlist) {
      setError('Desktop bridge unavailable. Open ECHO Next in Electron to delete streaming playlists.');
      return;
    }
    if (!isDeletableStreamingPlaylist(playlist)) {
      return;
    }

    const sourceLabel = playlistSourceProviderLabel(playlist) ?? '流媒体';
    const confirmed = window.confirm(
      `删除流媒体歌单 "${playlist.name}" (${sourceLabel})?\n\n这只会从 ECHO 的本地歌单列表移除导入记录，不会删除平台上的原歌单。`,
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      setSelectedPlaylistId((current) => (current === playlist.id ? null : current));
      await loadPlaylists();
      setStatusMessage(`流媒体歌单已删除：${playlist.name}`);
      window.dispatchEvent(new Event('library:playlists-changed'));
    } catch (deleteError) {
  ...
  const handleDeletePlaylist = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? selectedPlaylist;
    setPlaylistContextMenu(null);
    if (!library || !playlist || playlist.kind === 'system' || !window.confirm(`删除歌单 "${playlist.name}"?`)) {
      return;
    }
  ...
  const handleRenamePlaylist = (playlistOverride?: LibraryPlaylist): void => {
    const playlist = playlistOverride ?? selectedPlaylist;
    if (!playlist || playlist.kind === 'system') {
      return;
    }
  ...
  const handleUpdatePlaylistSort = async (sortMode: PlaylistSortMode, playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? selectedPlaylist;
    if (!library || !playlist) {
      return;
    ...
    try {
      setPlaylistMenuOpen(false);
      setPlaylistContextMenu(null);
      const updated = await library.updatePlaylist({ playlistId: playlist.id, sortMode });
      await loadPlaylists();
      setSelectedPlaylistId(updated.id);
      setPlaylistSearchInput('');
      setPlaylistSearch('');
  ...
  const handleExportPlaylist = async (format: PlaylistExportFormat, playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? selectedPlaylist;
    if (!library?.exportPlaylist || !playlist) {
    ...
    try {
      setPlaylistMenuOpen(false);
      setPlaylistContextMenu(null);
      const exportedPath = await library.exportPlaylist({ playlistId: playlist.id, format });
      if (exportedPath) {
  ...
  const handleDownloadPlaylist = useCallback(async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    ...
    try {
      ...
      setPlaylistDownloadSession({
        ...
        total: Math.max(playlist.itemCount, 1),
        enqueued: 0,
        failedToQueue: 0,
        jobIds: [],
        active: true,
      ...
      const tracks = (await loadTracksForPlaylistDownload(playlist.id)).filter((track) => streamingProviderFromTrack(track) !== null);
      ...
      for (const track of tracks) {
        ...
        setDownloadingTrackId(track.id);
        try {
          const job = await createDownloadJobForTrack(track, { outputSubdirectory: playlist.name });
          enqueued += 1;
          setDownloadJobs((current) => (current.some((item) => item.id === job.id) ? current : [job, ...current]));
          setDownloadJobIdsByTrackId((current) => ({ ...current, [track.id]: job.id }));
  ...
  const handleImportStreamingPlaylist = async (): Promise<void> => {
    ...
    if (!window.echo?.streaming?.importPlaylistFromUrl || !url) {
      return;
    }
    ...
    await importStreamingPlaylistUrl(url);
  };

  const handleStreamingPlaylistNoticeCancel = (): void => {
    setStreamingPlaylistNoticeOpen(false);
    setStreamingPlaylistNoticeConsent('');
    setPendingAccountPlaylistSyncProvider(null);
  };

  const handleStreamingPlaylistNoticeConfirm = async (): Promise<void> => {
    const url = playlistUrl.trim();
    setStreamingPlaylistNoticeOpen(false);
    setStreamingPlaylistNoticeConsent('');
    setStreamingPlaylistImportNoticeAccepted(true);

    await window.echo?.app?.setSettings?.({ streamingPlaylistImportNoticeAccepted: true }).catch(() => undefined);

    if (pendingAccountPlaylistSyncProvider) {
      const provider = pendingAccountPlaylistSyncProvider;
      setPendingAccountPlaylistSyncProvider(null);
      await syncAccountPlaylistsForProvider(provider);
      return;
    }
  ...
  const handleOpenPlaylistExternalUrl = async (url: string | null): Promise<void> => {
    if (!url) {
      setError('这个流媒体歌单没有可打开的原平台链接。');
      setStatusMessage(null);
      return;
    }
  ...
  const contextMenuQueueSource = (playlist: LibraryPlaylist) => ({
    type: 'manual' as const,
    label: `Playlist: ${playlist.name}`,
  });

  const handlePlayPlaylistFromContextMenu = async (playlist: LibraryPlaylist): Promise<void> => {
    setPlaylistContextMenu(null);
    setSelectedPlaylistId(playlist.id);
    setError(null);

    try {
      const tracks = await loadPlaylistPlaybackTracks(playlist, '');
      if (tracks.length === 0) {
      ...
      await playPlaylistSequence(tracks, {
        label: playlist.name,
        playlistId: playlist.id,
        source: contextMenuQueueSource(playlist),
      });
      setStatusMessage(`正在按歌单顺序播放：${playlist.name}`);
    } catch (playError) {
      setError(playError instanceof Error ? playError.message : String(playError));
      setStatusMessage(null);
    }
  ...
  const handleAddPlaylistToQueueFromContextMenu = async (playlist: LibraryPlaylist): Promise<void> => {
    setPlaylistContextMenu(null);
    setSelectedPlaylistId(playlist.id);
    setError(null);
    ...
    try {
      const tracks = await loadPlaylistPlaybackTracks(playlist, '');
      if (tracks.length === 0) {
        setError('这个歌单没有可加入队列的歌曲。');
        setStatusMessage(null);
        return;
      }

      appendTracksToQueue(tracks, contextMenuQueueSource(playlist));
      setStatusMessage(`已添加 ${tracks.length} 首到队列：${playlist.name}`);
    } catch (queueError) {
  ...
  const handleAddLocalFilesToPlaylist = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playback = window.echo?.playback;
    const playlist = playlistOverride ?? selectedPlaylist;
    setPlaylistContextMenu(null);
    if (!library?.addLocalAudioFilesToPlaylist || (!library.chooseImportFiles && !playback) || !playlist) {
      setError('Desktop bridge unavailable. Open ECHO Next in Electron to add local songs.');
      setStatusMessage(null);
      return;
    }

    if (playlist.kind === 'system' || playlist.sourceProvider !== 'local') {
    ...
    try {
      ...
      const result = await library.addLocalAudioFilesToPlaylist(playlist.id, filePaths);
      await loadPlaylists();
      setSelectedPlaylistId(playlist.id);
      setPlaylistSearchInput('');
      setPlaylistSearch('');
  ...
  const handleChoosePlaylistCover = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? selectedPlaylist;
    setPlaylistContextMenu(null);
    if (!library || !playlist) {
    ...
    try {
      ...
      setSelectedPlaylistId(playlist.id);
      await library.updatePlaylist({ playlistId: playlist.id, coverPath: selection.path });
      await loadPlaylists();
      await loadItems(playlist.id);
  ...
  const handleClearPlaylistCover = async (playlistOverride?: LibraryPlaylist): Promise<void> => {
    const library = window.echo?.library;
    const playlist = playlistOverride ?? selectedPlaylist;
    setPlaylistContextMenu(null);
    if (!library || !playlist) {
    ...
    try {
      setSelectedPlaylistId(playlist.id);
      await library.updatePlaylist({ playlistId: playlist.id, coverId: null });
      await loadPlaylists();
      await loadItems(playlist.id);
  ...
  const handleSelectAdjacentPlaylist = useCallback(
    (direction: 'previous' | 'next'): boolean => {
      if (playlistPanelView !== 'local' || filteredOrderedPlaylists.length <= 1) {
        return false;
      }

      const currentIndex = selectedPlaylist
        ? filteredOrderedPlaylists.findIndex((playlist) => playlist.id === selectedPlaylist.id)
        : -1;
      if (currentIndex < 0) {
        const fallbackPlaylist = filteredOrderedPlaylists[0];
        if (!fallbackPlaylist) {
          return false;
        }

        setSelectedPlaylistId(fallbackPlaylist.id);
        return true;
      }

      const nextIndex = direction === 'previous' ? currentIndex - 1 : currentIndex + 1;
      const nextPlaylist = filteredOrderedPlaylists[nextIndex];
      if (!nextPlaylist) {
        return false;
      }
    ...
    [filteredOrderedPlaylists, playlistPanelView, selectedPlaylist],
  );
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (event: MouseEvent): void => {
      ...
      if (handleSelectAdjacentPlaylist(direction)) {
        event.preventDefault();
      }
    };
  ...
  const contextMenuPlaylist = playlistContextMenu?.playlist ?? null;
  const contextMenuIsRemote = Boolean(contextMenuPlaylist && contextMenuPlaylist.sourceProvider !== 'local');
  const contextMenuPlaylistUrl = contextMenuPlaylist ? streamingPlaylistUrl(contextMenuPlaylist) : null;
  const contextMenuCanDownload = canDownloadPlaylist(contextMenuPlaylist, downloadsFeatureUnlocked);
  const selectedSourceFilterOption =
    playlistSourceFilterOptions.find((option) => option.value === playlistSourceFilter) ?? playlistSourceFilterOptions[0];
  const selectedAccountSyncProvider =
    playlistSourceFilter === 'netease' || playlistSourceFilter === 'qqmusic' ? playlistSourceFilter : null;
  const canSyncSelectedAccountProvider = selectedAccountSyncProvider !== null && syncingAccountPlaylistProvider === null;
  ...
  return (
    <div className="playlists-page">
      <aside className="playlist-sidebar" aria-label={t('route.playlists.label')}>
        ...
        <AnimatePresence mode="wait" initial={false}>
          {playlistPanelView === 'local' ? (
            ...
            {accountPlaylistSyncMessage ? (
              <div className="playlist-sidebar-message" role="status">
                {syncingAccountPlaylistProvider ? <Loader2 className="spinning-icon" size={14} aria-hidden="true" /> : <Check size={14} aria-hidden="true" />}
                <span>{accountPlaylistSyncMessage}</span>
              </div>
            ) : null}

            <div className="playlist-source-controls">
              <div className="playlist-source-filter" ref={sourceFilterMenuRef}>
                <button
                  type="button"
                  className="playlist-source-filter-button"
                  aria-label={t('playlistsPage.filter.aria')}
                  aria-haspopup="listbox"
                  aria-expanded={sourceFilterMenuOpen}
                  disabled={syncingAccountPlaylistProvider !== null}
                  onClick={() => setSourceFilterMenuOpen((open) => !open)}
                >
                  <span>{t(selectedSourceFilterOption.labelKey)}</span>
                  {syncingAccountPlaylistProvider ? <Loader2 className="spinning-icon" size={15} aria-hidden="true" /> : <ChevronDown size={15} aria-hidden="true" />}
                </button>
                {sourceFilterMenuOpen ? (
                  <div className="playlist-source-filter-menu" role="listbox" aria-label={t('playlistsPage.filter.aria')}>
                    {playlistSourceFilterOptions.map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        role="option"
                        aria-selected={option.value === playlistSourceFilter}
                        onClick={() => handlePlaylistSourceFilterChange(option.value)}
                      >
                        <span>{t(option.labelKey)}</span>
                        {option.value === playlistSourceFilter ? <Check size={14} aria-hidden="true" /> : null}
                      </button>
                    ))}
                  </div>
                ) : null}
              ...
              <button
                className="playlist-source-sync-button"
                type="button"
                disabled={!canSyncSelectedAccountProvider}
                onClick={() => selectedAccountSyncProvider ? requestAccountPlaylistSync(selectedAccountSyncProvider) : undefined}
              >
                {syncingAccountPlaylistProvider ? <Loader2 className="spinning-icon" size={14} /> : <RefreshCw size={14} />}
                <span>{syncingAccountPlaylistProvider ? '同步中' : '同步'}</span>
              </button>
            </div>
      ...
      <section className="playlist-detail">
        ...
        {contextMenuPlaylist
          ? createPortal(
            <div className="track-menu-layer playlist-context-menu-layer" role="presentation" onMouseDown={() => setPlaylistContextMenu(null)}>
            <div
              className="track-context-menu playlist-context-menu"
              role="menu"
              aria-label={contextMenuIsRemote ? '流媒体歌单操作' : '本地歌单操作'}
              style={{ left: playlistContextMenu?.position.x ?? 0, top: playlistContextMenu?.position.y ?? 0 }}
              onMouseDown={(event) => event.stopPropagation()}
            >
              <div className="track-menu-heading">{contextMenuPlaylist.name}</div>
              <button className="track-menu-item" type="button" role="menuitem" disabled={contextMenuPlaylist.itemCount === 0} onClick={() => void handlePlayPlaylistFromContextMenu(contextMenuPlaylist)}>
                <Play size={16} />
                <span>播放歌单</span>
              </button>
              <button className="track-menu-item" type="button" role="menuitem" disabled={contextMenuPlaylist.itemCount === 0} onClick={() => void handleAddPlaylistToQueueFromContextMenu(contextMenuPlaylist)}>
                <ListPlus size={16} />
                <span>添加到队列</span>
              </button>
              ...
              {contextMenuIsRemote ? (
                <>
                  <div className="track-menu-heading">流媒体歌单</div>
                  <button className="track-menu-item" type="button" role="menuitem" disabled={!contextMenuPlaylistUrl} onClick={() => void handleCopyPlaylistShareLink(contextMenuPlaylistUrl).finally(() => setPlaylistContextMenu(null))}>
                    <Share2 size={16} />
                    <span>分享歌单</span>
                  </button>
                  <button className="track-menu-item" type="button" role="menuitem" disabled={!contextMenuPlaylistUrl} onClick={() => void handleOpenPlaylistExternalUrl(contextMenuPlaylistUrl).finally(() => setPlaylistContextMenu(null))}>
                  ...
                  <button
                    ...
                    type="button"
                    role="menuitem"
                    disabled={isRefreshingStreamingPlaylist || (!contextMenuPlaylistUrl && !isNeteaseDailyRecommendPlaylist(contextMenuPlaylist))}
                    onClick={() => {
                      setPlaylistContextMenu(null);
                      void handleRefreshStreamingPlaylist(contextMenuPlaylist);
                    }}
                  >
                    {isRefreshingStreamingPlaylist ? <Loader2 className="spinning-icon" size={16} /> : <RefreshCw size={16} />}
                    <span>{isNeteaseDailyRecommendPlaylist(contextMenuPlaylist) ? '刷新推荐' : '刷新歌单'}</span>
                  </button>
                  {contextMenuCanDownload ? (
                    <button
                      className="track-menu-item"
                      type="button"
                      role="menuitem"
                      disabled={contextMenuPlaylist.itemCount === 0 || playlistDownloadSummary?.isActive === true}
                      onClick={() => {
                        setPlaylistContextMenu(null);
                        void handleDownloadPlaylist(contextMenuPlaylist);
                      }}
                    >
                      {playlistDownloadSummary?.isActive ? <Loader2 className="spinning-icon" size={16} /> : <Download size={16} />}
                      <span>{playlistDownloadSummary?.isActive ? '下载中' : '下载歌单'}</span>
                ...
                <>
                  <div className="track-menu-heading">本地歌单</div>
                  {contextMenuPlaylist.kind !== 'system' ? (
                    <>
                      <button className="track-menu-item" type="button" role="menuitem" onClick={() => handleRenamePlaylist(contextMenuPlaylist)}>
                        <Pencil size={16} />
                        <span>重命名歌单</span>
                      </button>
                      <button className="track-menu-item" type="button" role="menuitem" disabled={isAddingLocalFiles} onClick={() => void handleAddLocalFilesToPlaylist(contextMenuPlaylist)}>
                        {isAddingLocalFiles ? <Loader2 className="spinning-icon" size={16} /> : <FilePlus2 size={16} />}
                        <span>{isAddingLocalFiles ? '添加中' : '添加本地歌曲'}</span>
                      </button>
                    </>
              ...
              {contextMenuIsRemote ? (
                isDeletableStreamingPlaylist(contextMenuPlaylist) ? (
                  <>
                    <button className="track-menu-item" data-danger="true" type="button" role="menuitem" onClick={() => void handleDeleteStreamingPlaylistFromContextMenu(contextMenuPlaylist)}>
                      <Trash2 size={16} />
                      <span>移除流媒体歌单</span>
                    </button>
                    <p className="playlist-context-menu-note">仅移除 ECHO 本地导入记录，不会删除平台上的原歌单。</p>
                  </>
                ) : null
              ) : contextMenuPlaylist.kind !== 'system' ? (
                <button className="track-menu-item" data-danger="true" type="button" role="menuitem" onClick={() => void handleDeletePlaylist(contextMenuPlaylist)}>
                  <Trash2 size={16} />
                  <span>删除本地歌单</span>
                </button>
              ) : null}
