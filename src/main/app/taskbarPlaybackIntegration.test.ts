...
describe('TaskbarPlaybackIntegration', () => {
  ...
  it('rebuilds taskbar thumbnail buttons on explicit refresh even when playback state is unchanged', async () => {
    const { TaskbarPlaybackIntegration } = await import('./taskbarPlaybackIntegration');
    const audioSession = createAudioSession();
    const integration = new TaskbarPlaybackIntegration({
      window,
    ...
    integration.initialize();
    expect(window.setThumbarButtons).not.toHaveBeenCalled();

    integration.refresh();
    expect(window.setThumbarButtons).toHaveBeenCalledTimes(1);

    integration.refresh();
    expect(window.setThumbarButtons).toHaveBeenCalledTimes(2);
    integration.dispose();
  });

  it('does not clear thumbnail buttons during initialization before playback starts', async () => {
    const { TaskbarPlaybackIntegration } = await import('./taskbarPlaybackIntegration');
    ...
    audioSession.emitStatus(makeStatus({ state: 'playing' }));
