...
import {
  ...
  type AudioEchoSrcComputeBackend,
...
const readJsonFileText = (filePath: string): string =>
...
const normalizeAudioEchoSrcFilterProfile = (value: unknown, fallback: AudioEchoSrcFilterProfile = 'poly-sinc-gauss-long'): AudioEchoSrcFilterProfile =>
  typeof value === 'string' && (audioEchoSrcFilterProfiles as readonly string[]).includes(value)
    ? value as AudioEchoSrcFilterProfile
    : fallback;

const normalizeAudioEchoSrcComputeBackend = (value: unknown): AudioEchoSrcComputeBackend =>
  value === 'cuda' ? 'cuda' : 'cpu';

const normalizeAudioPcmDitherMode = (value: unknown): AudioPcmDitherMode =>
  typeof value === 'string' && (audioPcmDitherModes as readonly string[]).includes(value)
    ? value as AudioPcmDitherMode
    : 'off';

const normalizeAudioSdmMode = (value: unknown): AudioSdmMode =>
  typeof value === 'string' && (audioSdmModes as readonly string[]).includes(value)
    ? value as AudioSdmMode
    : 'off';

const normalizeAudioSdmTargetRate = (value: unknown): AudioSdmTargetRate =>
  typeof value === 'string' && (audioSdmTargetRates as readonly string[]).includes(value)
    ? value as AudioSdmTargetRate
    : 'dsd128';

const normalizeAudioSdmQualityProfile = (value: unknown): AudioSdmQualityProfile =>
  typeof value === 'string' && (audioSdmQualityProfiles as readonly string[]).includes(value)
    ? value as AudioSdmQualityProfile
    : 'safe';

const normalizeAudioSdmComputeBackend = (value: unknown): AudioSdmComputeBackend =>
...
export const normalizeSettings = (value: unknown, options: NormalizeSettingsOptions = {}): AppSettings => {
  ...
  const lyricsRoseVinylBackgroundBlurPx = Number(settings.lyricsRoseVinylBackgroundBlurPx);
  ...
  const audioUseLibavDecode = settings.audioUseLibavDecode === true;
  ...
  return {
    ...
    audioEchoSrcAdvancedModeEnabled: settings.audioEchoSrcAdvancedModeEnabled === true,
    audioEchoSrcFilterProfile: normalizeAudioEchoSrcFilterProfile(settings.audioEchoSrcFilterProfile),
    audioEchoSrcFilterProfile1x: normalizeAudioEchoSrcFilterProfile(
      settings.audioEchoSrcFilterProfile1x,
      normalizeAudioEchoSrcFilterProfile(settings.audioEchoSrcFilterProfile),
    ),
    audioEchoSrcFilterProfileNx: normalizeAudioEchoSrcFilterProfile(settings.audioEchoSrcFilterProfileNx, 'poly-sinc-hb'),
    audioEchoSrcComputeBackend: normalizeAudioEchoSrcComputeBackend(settings.audioEchoSrcComputeBackend),
    audioPcmDitherMode: normalizeAudioPcmDitherMode(settings.audioPcmDitherMode),
    ...
    artistMergeStrategy,
    chineseCrossScriptSearchEnabled: true,
    albumWallVirtualizationEnabled: settings.albumWallVirtualizationEnabled === true,
    ...
    lyricsPageStyle: normalizeLyricsPageStyle(settings.lyricsPageStyle),
    ...
    lyricsRoseVinylBackgroundBlurPx: Number.isFinite(lyricsRoseVinylBackgroundBlurPx)
      ? Math.round(clamp(lyricsRoseVinylBackgroundBlurPx, 0, 48))
      : defaultSettings.lyricsRoseVinylBackgroundBlurPx,
    ...
    mvImmersiveBackgroundScalePercent: Number.isFinite(mvImmersiveBackgroundScalePercent)
      ? Math.round(clamp(mvImmersiveBackgroundScalePercent, 70, 220))
...
const containsProThemeState = (source: Partial<AppSettings>): boolean =>
  proOnlyThemePresetSet.has(source.appearanceThemePreset as AppThemePreset) ||
...
const shouldPersistFinalThemeRelock = (source: Partial<AppSettings>, normalized: AppSettings): boolean => {
  if (containsProThemeState(source)) {
    // A locked read can happen before the unlock plugin finishes reporting during startup.
    // Keep returned settings fail-closed, but do not overwrite a user's saved Pro theme
    // before the runtime entitlement path has had a chance to report.
    return false;
  }

  return (
    (typeof source.finalThemeUnlockVersion === 'string' && source.finalThemeUnlockVersion !== normalized.finalThemeUnlockVersion) ||
    source.finalThemeUnlockVersion === finalThemeUnlockVersion
  );
};
...
const hasRuntimeUnlockOverride = (options: NormalizeSettingsOptions): boolean =>
  options.finalThemeUnlocked === true ||
  typeof options.downloadsFeatureUnlocked === 'boolean';
...
export const getAppSettings = (options: NormalizeSettingsOptions = {}): AppSettings => {
  ...
  if (!cachedSettingsSource) {
    ...
    if (!existsSync(settingsPath)) {
      ...
      try {
        cachedSettingsSource = JSON.parse(readJsonFileText(settingsPath)) as Partial<AppSettings>;
      } catch {
        cachedSettingsSource = { ...defaultSettings };
      }
    }
