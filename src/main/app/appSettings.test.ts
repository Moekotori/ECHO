...
describe('app settings normalization', () => {
  ...
  it('keeps old settings files compatible when coverCacheDir is missing', async () => {
    ...
    expect(settings.lyricsPlayerBarDrawerCompactOnIdleEnabled).toBe(false);
    ...
    expect(settings.lyricsPageStyle).toBe('default');
    ...
    expect(settings.lyricsRoseVinylBackgroundBlurPx).toBe(18);
  ...
  it('normalizes launch at login as an explicit opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).launchAtLoginEnabled).toBe(false);
    expect(normalizeSettings({ launchAtLoginEnabled: true }).launchAtLoginEnabled).toBe(true);
    expect(normalizeSettings({ launchAtLoginEnabled: 'true' as never }).launchAtLoginEnabled).toBe(false);
  });

  it('normalizes lyrics/MV graphics pressure guard as an explicit opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).lyricsMvGraphicsPressureGuardEnabled).toBe(false);
    expect(normalizeSettings({ lyricsMvGraphicsPressureGuardEnabled: true }).lyricsMvGraphicsPressureGuardEnabled).toBe(true);
    expect(normalizeSettings({ lyricsMvGraphicsPressureGuardEnabled: 'true' as never }).lyricsMvGraphicsPressureGuardEnabled).toBe(false);
  });

  it('normalizes album wall virtualization as an explicit opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).albumWallVirtualizationEnabled).toBe(false);
    expect(normalizeSettings({ albumWallVirtualizationEnabled: true }).albumWallVirtualizationEnabled).toBe(true);
    expect(normalizeSettings({ albumWallVirtualizationEnabled: 'true' as never }).albumWallVirtualizationEnabled).toBe(false);
  });
  ...
  it('loads settings files that were saved with a UTF-8 BOM', async () => {
    ...
    mkdirSync(userDataPath, { recursive: true });
    const settingsPath = join(userDataPath, 'echo-settings.json');
    writeFileSync(settingsPath, `\uFEFF${JSON.stringify({ playerVolume: 0.42 }, null, 2)}\n`, 'utf8');
  ...
  it('normalizes appearance theme modes', async () => {
    ...
    expect(normalizeSettings({ appearanceTheme: 'ambient' }).appearanceTheme).toBe('ambient');
  ...
  it('keeps Pro themes locked unless the donator unlock plugin is valid for the current marker', async () => {
    ...
    expect(normalizeSettings({ appearanceThemePreset: 'FINAL', finalThemeUnlockVersion: null }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('FINAL');
    expect(normalizeSettings({ appearanceThemePreset: 'nyanCat', finalThemeUnlockVersion: null }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('nyanCat');
    expect(normalizeSettings({ appearanceThemePreset: 'darkSideMoon', finalThemeUnlockVersion: null }, { finalThemeUnlocked: true }).appearanceThemePreset).toBe('darkSideMoon');
  });
  ...
  it('does not rewrite a plugin-based Pro theme when startup reads settings before runtime unlock', async () => {
    vi.resetModules();
    const { finalThemeUnlockVersion } = await import('../../shared/constants/featureUnlocks');
    userDataPath = join(tmpdir(), `echo-next-settings-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    tempRoots.push(userDataPath);
    ...
    const savedSettings = {
      appearanceTheme: 'dark',
      appearanceThemePreset: 'darkSideMoon',
      finalThemeUnlockVersion: null,
    };
    writeFileSync(settingsPath, `${JSON.stringify(savedSettings, null, 2)}\n`, 'utf8');

    const { getAppSettings } = await import('./appSettings');
    const lockedStartupView = getAppSettings();
    const persistedAfterStartupRead = JSON.parse(readFileSync(settingsPath, 'utf8')) as Record<string, unknown>;
    const unlockedRuntimeView = getAppSettings({ finalThemeUnlocked: true });
    const persistedAfterRuntimeRead = JSON.parse(readFileSync(settingsPath, 'utf8')) as Record<string, unknown>;

    expect(lockedStartupView.appearanceThemePreset).toBe('classic');
    expect(lockedStartupView.finalThemeUnlockVersion).toBeNull();
    expect(persistedAfterStartupRead).toMatchObject(savedSettings);
    expect(unlockedRuntimeView.appearanceThemePreset).toBe('darkSideMoon');
    expect(unlockedRuntimeView.finalThemeUnlockVersion).toBe(finalThemeUnlockVersion);
    expect(persistedAfterRuntimeRead).toMatchObject(savedSettings);
  });
  ...
  it('keeps hardware acceleration enabled unless explicitly disabled', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).hardwareAccelerationDisabled).toBe(false);
    expect(normalizeSettings({ hardwareAccelerationDisabled: true }).hardwareAccelerationDisabled).toBe(true);
    expect(normalizeSettings({ hardwareAccelerationDisabled: 'yes' as never }).hardwareAccelerationDisabled).toBe(false);
  });

  it('normalizes shuffle recent-repeat avoidance', async () => {
    const { normalizeSettings } = await import('./appSettings');
    ...
    expect(normalizeSettings({ playbackShuffleAvoidRecentCount: 'soon' as never }).playbackShuffleAvoidRecentCount).toBe(25);
  });

  it('keeps Chinese cross-script search enabled without a disable setting', async () => {
    const { normalizeSettings } = await import('./appSettings');
    ...
    expect(normalizeSettings({ chineseCrossScriptSearchEnabled: false }).chineseCrossScriptSearchEnabled).toBe(true);
  ...
  it('keeps automatic update checks enabled unless explicitly disabled', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).autoUpdateEnabled).toBe(true);
    expect(normalizeSettings({ autoUpdateEnabled: true }).autoUpdateEnabled).toBe(true);
    expect(normalizeSettings({ autoUpdateEnabled: false }).autoUpdateEnabled).toBe(false);
    expect(normalizeSettings({ autoUpdateEnabled: 'yes' as never }).autoUpdateEnabled).toBe(true);
  });

  it('enables the streaming entry by default while keeping the notice opt-in', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).streamingFeatureEnabled).toBe(true);
    expect(normalizeSettings({ streamingFeatureEnabled: true }).streamingFeatureEnabled).toBe(true);
    expect(normalizeSettings({ streamingFeatureEnabled: false }).streamingFeatureEnabled).toBe(false);
    expect(normalizeSettings({ streamingFeatureEnabled: 'yes' as never }).streamingFeatureEnabled).toBe(true);
    expect(normalizeSettings({}).osuDownloaderFeatureEnabled).toBe(false);
    expect(normalizeSettings({ osuDownloaderFeatureEnabled: true }).osuDownloaderFeatureEnabled).toBe(true);
    expect(normalizeSettings({ osuDownloaderFeatureEnabled: false }).osuDownloaderFeatureEnabled).toBe(false);
    expect(normalizeSettings({ osuDownloaderFeatureEnabled: 'yes' as never }).osuDownloaderFeatureEnabled).toBe(false);
    expect(normalizeSettings({}).streamingPlaylistImportNoticeAccepted).toBe(false);
    expect(normalizeSettings({ streamingPlaylistImportNoticeAccepted: true }).streamingPlaylistImportNoticeAccepted).toBe(true);
    expect(normalizeSettings({ streamingPlaylistImportNoticeAccepted: false }).streamingPlaylistImportNoticeAccepted).toBe(false);
    expect(normalizeSettings({ streamingPlaylistImportNoticeAccepted: 'yes' as never }).streamingPlaylistImportNoticeAccepted).toBe(false);
  });
  ...
  it('keeps ECHO SRC disabled by default and normalizes safe PCM targets', async () => {
    ...
    expect(normalizeSettings({}).audioEchoSrcAdvancedModeEnabled).toBe(false);
    expect(normalizeSettings({ audioEchoSrcAdvancedModeEnabled: true }).audioEchoSrcAdvancedModeEnabled).toBe(true);
    expect(normalizeSettings({}).audioEchoSrcFilterProfile).toBe('poly-sinc-gauss-long');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'poly-sinc-ext2-xla' }).audioEchoSrcFilterProfile).toBe('poly-sinc-ext2-xla');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'minringFIR-mp' }).audioEchoSrcFilterProfile).toBe('minringFIR-mp');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'apod-minring' }).audioEchoSrcFilterProfile).toBe('apod-minring');
    expect(normalizeSettings({ audioEchoSrcFilterProfile1x: 'apod-long' }).audioEchoSrcFilterProfile1x).toBe('apod-long');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'poly-sinc-gauss-hires-mp' }).audioEchoSrcFilterProfile).toBe('poly-sinc-gauss-hires-mp');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'poly-sinc-xtr-xla' }).audioEchoSrcFilterProfile).toBe('poly-sinc-xtr-xla');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'poly-sinc-ext2-long' }).audioEchoSrcFilterProfile1x).toBe('poly-sinc-ext2-long');
    expect(normalizeSettings({}).audioEchoSrcFilterProfileNx).toBe('poly-sinc-hb');
    expect(normalizeSettings({ audioEchoSrcFilterProfileNx: 'sinc-xla' }).audioEchoSrcFilterProfileNx).toBe('sinc-xla');
    expect(normalizeSettings({ audioEchoSrcFilterProfile: 'nonsense' as never }).audioEchoSrcFilterProfile).toBe('poly-sinc-gauss-long');
    expect(normalizeSettings({}).audioEchoSrcComputeBackend).toBe('cpu');
    expect(normalizeSettings({ audioEchoSrcComputeBackend: 'cuda' }).audioEchoSrcComputeBackend).toBe('cuda');
    expect(normalizeSettings({ audioEchoSrcComputeBackend: 'amd' as never }).audioEchoSrcComputeBackend).toBe('cpu');
    expect(normalizeSettings({}).audioPcmDitherMode).toBe('off');
    expect(normalizeSettings({ audioPcmDitherMode: 'ns-9' }).audioPcmDitherMode).toBe('ns-9');
    expect(normalizeSettings({ audioPcmDitherMode: 'shouty' as never }).audioPcmDitherMode).toBe('off');
  });

  it('keeps SDM independent from PCM settings and normalizes planned targets', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).audioSdmMode).toBe('off');
    expect(normalizeSettings({}).audioSdmTargetRate).toBe('dsd128');
    expect(normalizeSettings({}).audioSdmQualityProfile).toBe('safe');
    expect(normalizeSettings({}).audioSdmComputeBackend).toBe('cpu');
    expect(normalizeSettings({ audioSdmMode: 'dsdPassthrough' }).audioSdmMode).toBe('dsdPassthrough');
    expect(normalizeSettings({ audioSdmMode: 'pcmToDsd' }).audioSdmMode).toBe('pcmToDsd');
    expect(normalizeSettings({ audioSdmMode: 'family8x' as never }).audioSdmMode).toBe('off');
    expect(normalizeSettings({ audioSdmTargetRate: 'dsd512' }).audioSdmTargetRate).toBe('dsd512');
    expect(normalizeSettings({ audioSdmTargetRate: 'pcm384' as never }).audioSdmTargetRate).toBe('dsd128');
    expect(normalizeSettings({ audioSdmQualityProfile: 'reference' }).audioSdmQualityProfile).toBe('reference');
  ...
  it('keeps miniaudio output experiment disabled until explicitly enabled', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).audioMiniaudioOutputExperimentalEnabled).toBe(false);
    expect(normalizeSettings({ audioMiniaudioOutputExperimentalEnabled: true }).audioMiniaudioOutputExperimentalEnabled).toBe(true);
    expect(normalizeSettings({ audioMiniaudioOutputExperimentalEnabled: true }).audioUseNativeOutput).toBe(true);
    expect(normalizeSettings({ audioMiniaudioOutputExperimentalEnabled: true }).audioUseMiniaudioOutput).toBe(true);
    expect(normalizeSettings({ audioUseMiniaudioOutput: true }).audioMiniaudioOutputExperimentalEnabled).toBe(true);
    expect(normalizeSettings({ audioUseMiniaudioOutput: true }).audioUseNativeOutput).toBe(true);
    expect(normalizeSettings({ audioMiniaudioOutputExperimentalEnabled: false }).audioMiniaudioOutputExperimentalEnabled).toBe(false);
    expect(normalizeSettings({ audioMiniaudioOutputExperimentalEnabled: 'yes' as never }).audioMiniaudioOutputExperimentalEnabled).toBe(false);
  });

  it('normalizes libav decode setting', async () => {
    const { normalizeSettings } = await import('./appSettings');

    expect(normalizeSettings({}).audioUseLibavDecode).toBe(false);
    expect(normalizeSettings({ audioUseLibavDecode: true }).audioUseLibavDecode).toBe(true);
    expect(normalizeSettings({ audioUseLibavDecode: 'yes' as never }).audioUseLibavDecode).toBe(false);
  });
  ...
  it('normalizes lyrics settings', async () => {
    ...
    expect(
      normalizeSettings({
        ...
        lyricsPlayerBarDrawerCompactOnIdleEnabled: 'yes' as never,
        ...
        lyricsRoseVinylBackgroundBlurPx: 999,
      ...
      lyricsRoseVinylBackgroundBlurPx: 48,
    ...
    expect(
      normalizeSettings({
        ...
        lyricsRoseVinylBackgroundBlurPx: 20.5,
        lyricsSmartAlignmentEnabled: true,
      ...
      lyricsRoseVinylBackgroundBlurPx: 21,
      lyricsSmartAlignmentEnabled: true,
      lyricsHighResolutionNetworkCoverEnabled: true,
      lyricsMusicReactiveVisualsEnabled: false,
      lyricsBackgroundMode: 'cover',
  ...
  it('keeps the bottom signal path control enabled unless explicitly disabled', async () => {
    const { normalizeSettings } = await import('./appSettings');
    ...
    expect(normalizeSettings({ signalPathControlEnabled: true }, { finalThemeUnlocked: true }).signalPathControlEnabled).toBe(true);
  ...
  it('does not clear a saved signal path control when persisting unrelated startup settings', async () => {
    vi.resetModules();
    userDataPath = join(tmpdir(), `echo-next-settings-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    tempRoots.push(userDataPath);
    mkdirSync(userDataPath, { recursive: true });
    const settingsPath = join(userDataPath, 'echo-settings.json');
    writeFileSync(settingsPath, `${JSON.stringify({ signalPathControlEnabled: true }, null, 2)}\n`, 'utf8');

    const { getAppSettings, setAppSettings } = await import('./appSettings');
    const lockedStartupView = getAppSettings();
    const nextSettings = setAppSettings({
      rememberedWindowSize: { width: 1280, height: 820 },
    });
    const persisted = JSON.parse(readFileSync(settingsPath, 'utf8')) as Record<string, unknown>;

    expect(lockedStartupView.signalPathControlEnabled).toBe(true);
    expect(nextSettings.signalPathControlEnabled).toBe(true);
    expect(persisted.signalPathControlEnabled).toBe(true);
    expect(persisted.rememberedWindowSize).toEqual({ width: 1280, height: 820 });
  });
  ...
  it('normalizes MV network settings', async () => {
    ...
    expect(
      normalizeSettings({
        ...
        mvTitleOnlySearch: 'yes' as never,
    ...
    expect(normalizeSettings({ mvImmersiveBackgroundScalePercent: 55 })).toMatchObject({
      mvImmersiveBackgroundScalePercent: 70,
    });
