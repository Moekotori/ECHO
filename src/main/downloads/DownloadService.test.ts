...
describe('DownloadService', () => {
  ...
  it('keeps osu keyword search results relevant for artist and title terms', async () => {
    ...
    expect(fetchRunner).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ headers: expect.objectContaining({ referer: 'https://catboy.best/', 'user-agent': expect.stringContaining('Mozilla/5.0') }) }),
    );
    expect(fetchRunner).toHaveBeenCalledWith(
  ...
  it('uses Sayobot osu search results before probing Catboy', async () => {
    const fetchRunner = vi.fn(async (url: string | URL | Request) => {
      const rawUrl = String(url);
      if (rawUrl.includes('api.sayobot.cn/beatmaplist')) {
        return {
          ...
          json: async () => ({
            status: 0,
            data: [
              { sid: 2250850, artist: 'Moe Shop', title: 'Lovesick (w/ maisou)', creator: 'Disguise', play_count: 18399 },
              { sid: 2177214, artist: 'Moe Shop', title: 'Notice (w/ TORIENA)', creator: 'Paraxia', play_count: 5401 },
            ],
          }),
        };
      }
    ...
    const response = await service.search({ query: 'moe shop', limitPerProvider: 2, provider: 'osu' });

    expect(response.errors).toEqual([]);
    expect(response.results.map((result) => result.id)).toEqual(['2250850', '2177214']);
    expect(fetchRunner).not.toHaveBeenCalledWith(
      expect.stringContaining('https://catboy.best/api/search'),
      expect.anything(),
    );
  });
