...
export class DownloadService extends EventEmitter {
  ...
  private async searchOsuBeatmapsets(query: string, limitPerProvider: number): Promise<DownloadSearchResult[]> {
    ...
    for (const searcher of [
      ...
      try {
        ...
        const selected = this.selectOsuSearchResults(query, results, limitPerProvider);
        if (selected.length >= limitPerProvider) {
          return selected;
        }
      } catch (error) {
    ...
    const deduped = this.selectOsuSearchResults(query, results, limitPerProvider);
    if (deduped.length > 0 || successfulSearches > 0 || errors.length === 0) {
      return deduped;
    }
  ...
  private selectOsuSearchResults(query: string, results: DownloadSearchResult[], limitPerProvider: number): DownloadSearchResult[] {
  ...
  private osuDownloadSources(beatmapsetId: string): OsuDownloadSource[] {
    ...
    const mirrorHeaders = (referer: string, userAgent = osuDownloadUserAgent): Record<string, string> => ({
      Accept: osuArchiveAccept,
      Referer: referer,
    ...
    const sources: OsuDownloadSource[] = [
      ...
      },
      {
        ...
        headers: mirrorHeaders('https://catboy.best/', browserUserAgent),
      },
      {
        mirror: 'nerinyan',
        label: 'NeriNyan',
  ...
  private async importAndBind(jobId: string, optionsOverride: { emitProgress?: boolean } = {}): Promise<void> {
    ...
    const coverUrl = options.suggestedCoverUrl ?? job.thumbnailUrl;
    const isOsuImport =
      options.suggestedComment?.startsWith('beatmap id: ') === true || options.suggestedComment?.startsWith('beatmapset id: ') === true;
