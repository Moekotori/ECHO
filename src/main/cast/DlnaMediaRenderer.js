...
function scoreLanCandidate(addr, name) {
  ...
  if (
    /wi-?fi|wlan|无线|802\.11|ethernet|eth\d|en\d|局域网|本地连接|乙太網路/i.test(
      name,
    )
  )
    s += 18;
...
function extractDidlDurationSec(xml) {
  if (!xml) return 0;
  const m =
    xml.match(/<res[^>]*\bduration\s*=\s*"([^"]+)"/i) ||
    xml.match(/\bduration\s*=\s*"([^"]+)"/i);
  if (!m?.[1]) return 0;
...
export class DlnaMediaRenderer {
  constructor({ audioEngine, getMainWindow, beforePlayHook, onCastActivity }) {
    ...
    this.running = false;
    this.friendlyName = "Echoes Studio";
  ...
  _onSsdpMessage(msg, rinfo) {
    ...
    if (st === "upnp:rootdevice") {
      ...
      this.deviceUuid &&
      new RegExp(`^uuid:${this.deviceUuid}$`, "i").test(st)
    ) {
...
<root xmlns="urn:schemas-upnp-org:device-1-0">
  ...
  <device>
    ...
    <manufacturer>Echoes Studio</manufacturer>
    <manufacturerURL>https://github.com</manufacturerURL>
    <modelDescription>Echoes DLNA MediaRenderer</modelDescription>
    <modelName>EchoesRenderer</modelName>
...
<CurrentPresetNameList>FactoryDefaults</CurrentPresetNameList>
        ...
        if (action === "GetVolume") {
          const pct = Math.round(
            (typeof this.audioEngine.getVolume === "function"
              ? this.audioEngine.getVolume()
              : 1) * 100,
          );
...
<CurrentMute>0</CurrentMute>
  ...
  async _handleAvTransport(action, body, res) {
    ...
    if (action === "SetAVTransportURI") {
      ...
      if (metaXml && metaXml.length > DIDL_MAX)
        metaXml = metaXml.slice(0, DIDL_MAX);
      this.currentDidlXml = metaXml || "";
      const rawMeta = parseDidlLiteMetadata(this.currentDidlXml);
      const refined = refineDlnaMeta(
        rawMeta,
        this.currentUri,
        this.currentDidlXml,
      );
