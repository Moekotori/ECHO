...
export type LibraryWatcherRescanCoordinator = {
  ...
  removeDeletedPaths?: (folderId: string, paths: string[]) => unknown;
...
export class LibraryWatcherService {
  ...
  private pendingDeletedPaths = new Map<string, Set<string>>();
  ...
  private confirmPendingEvent(key: string): void {
    ...
    if (eventType === 'unlink') {
      ...
      this.enqueueDeletedPath(pending.folderId, pending.path);
  ...
  private enqueueDeletedPath(folderId: string, filePath: string): void {
    if (!this.diagnostics.autoRescanEnabled || !this.rescanCoordinator?.removeDeletedPaths) {
      return;
    }

    const normalizedPath = resolve(filePath);
    const folderPaths = this.pendingDeletedPaths.get(folderId) ?? new Set<string>();
    const alreadyQueued = folderPaths.has(normalizedPath);
    ...
    folderPaths.add(normalizedPath);
    this.pendingDeletedPaths.set(folderId, folderPaths);
    this.updatePendingPathCount();
    this.scheduleDeletedPathFlush(folderId);
  }

  private scheduleDeletedPathFlush(folderId: string): void {
    const existing = this.deleteTimers.get(folderId);
    if (existing) {
      clearTimeout(existing);
    }

    const timer = setTimeout(() => {
      this.flushDeletedPaths(folderId);
    }, Math.min(this.rescanDebounceMs, 250));
    timer.unref?.();
    this.deleteTimers.set(folderId, timer);
  }

  private flushDeletedPaths(folderId: string): void {
    this.deleteTimers.delete(folderId);
    const paths = this.pendingDeletedPaths.get(folderId);
    ...
    this.pendingDeletedPaths.delete(folderId);
    this.updatePendingPathCount();

    try {
      const result = this.rescanCoordinator?.removeDeletedPaths?.(folderId, batch);
  ...
  private getPendingPathCount(): number {
    ...
    for (const paths of this.pendingDeletedPaths.values()) {
      count += paths.size;
    }
    return count;
  }
