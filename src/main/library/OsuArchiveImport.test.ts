...
describe('OsuArchiveImport', () => {
  ...
  it('writes BPM tags from osu timing points when importing an osz archive', async () => {
    const root = makeTempRoot();
    const archivePath = writeArchive(root, {
      'song.osu':
  ...
  it('reads bpm from the first usable uninherited timing point', () => {
    const metadata = parseOsuFileMetadata(
      '[General]\nAudioFilename: audio.mp3\n\n[TimingPoints]\n0,500,4,1,0,100,0,0\n250,-100,4,1,0,100,0,0\n500,461.538461538,4,1,0,100,1,0\n',
    );
