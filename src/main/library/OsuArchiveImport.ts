...
const parseOsuTimingPointBpm = (line: string): number | null => {
  const parts = parseOsuCsvLine(line);
  const beatLength = Number(parts[1]?.trim());
  if (!Number.isFinite(beatLength) || beatLength <= 0) {
    return null;
  }

  const uninherited = parts[6]?.trim();
  if (uninherited && uninherited !== '1') {
    return null;
  }

  const bpm = 60000 / beatLength;
  return Number.isFinite(bpm) && bpm > 0 && bpm <= 400 ? Math.round(bpm * 100) / 100 : null;
};
...
export const parseOsuFileMetadata = (content: string): OsuArchiveMetadata => {
  ...
  for (const rawLine of content.split(/\r?\n/u)) {
    ...
    if (section === 'timingpoints' && metadata.bpm === null) {
      metadata.bpm = parseOsuTimingPointBpm(line);
      continue;
    }
...
export const mimeTypeForOsuCoverPath = (filePath: string): string => mimeTypeForOsuCoverEntry(filePath);
...
export const buildOsuImportTags = (
  metadata: OsuArchiveMetadata,
  archivePath: string,
  beatmapsetId: string | null,
): EditableTrackTags => {
  ...
  const beatmapId = cleanBeatmapSetId(metadata.beatmapId);
  const fallbackBeatmapsetId = beatmapId ? null : beatmapsetId;
  ...
  return {
    ...
    year: null,
    genre: null,
    ...(metadata.bpm !== null ? { bpm: metadata.bpm } : {}),
    comment: beatmapId ? `beatmap id: ${beatmapId}` : fallbackBeatmapsetId ? `beatmapset id: ${fallbackBeatmapsetId}` : null,
  };
};
...
export const importOsuArchiveAsMp3 = async (request: OsuArchiveImportRequest): Promise<OsuArchiveImportResult> => {
  ...
  const coverData = coverEntry
    ...
    : null;
  const tags = buildOsuImportTags(metadata, request.archivePath, beatmapsetId);
