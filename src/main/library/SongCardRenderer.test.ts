...
describe('SongCardRenderer', () => {
  ...
  it('uses the same cover image for foreground and nonblank background treatment', async () => {
    ...
    const foregroundSample = (await image.clone().extract({ left: 1120, top: 300, width: 1, height: 1 }).raw().toBuffer()).subarray(0, 3);
    const backgroundSample = (await image.clone().extract({ left: 220, top: 240, width: 1, height: 1 }).raw().toBuffer()).subarray(0, 3);
  ...
  it('renders visible song metadata over the background', async () => {
    ...
    const textRegion = await sharp(result.pngBuffer)
      .extract({ left: 140, top: 140, width: 760, height: 520 })
      .removeAlpha()
      .raw()
      .toBuffer();
    let brightPixels = 0;
