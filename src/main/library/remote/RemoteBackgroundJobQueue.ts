...
export class RemoteBackgroundJobQueue {
  ...
  private effectiveConcurrency(sourceId: string | null): Record<RemoteBackgroundJobKind, number> {
    ...
    const sourceConfig = sourceId ? this.getSourceConfig(sourceId) : null;
  ...
  private getSourceConfig(sourceId: string): Record<string, unknown> | null {
    const store = this.store as RemoteLibraryStore & {
      getSourceConfig?: (sourceId: string) => Record<string, unknown> | null;
    };

    if (typeof store.getSourceConfig === 'function') {
      return store.getSourceConfig(sourceId);
    }

    return this.store.getSource(sourceId)?.config ?? null;
  }
