...
export class RemoteLibraryStore {
  ...
  hasSource(id: string): boolean {
    const row = this.database.prepare<[string], { found: number }>('SELECT 1 AS found FROM remote_sources WHERE id = ?').get(id);
    return row?.found === 1;
  }

  getSourceConfig(id: string): Record<string, unknown> | null {
    const row = this.database.prepare<[string], { config_json: string | null }>('SELECT config_json FROM remote_sources WHERE id = ?').get(id);
