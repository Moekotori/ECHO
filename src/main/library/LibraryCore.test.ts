...
type EmbeddedTrackTagWriter = NonNullable<NonNullable<Parameters<typeof createLibraryService>[1]>['writeEmbeddedTrackTags']>;
...
class FakeFileScanner implements FileScanner {
  readonly calls: string[] = [];
  readonly optionCalls: ScanOptions[] = [];
...
const createHarness = (
  overrides: { coverExtractor?: CoverExtractor; metadataReader?: MetadataReader; fileScanner?: FileScanner; writeEmbeddedTrackTags?: EmbeddedTrackTagWriter } = {},
) => {
...
describe('Library Core', () => {
  ...
  it('filters songs by PCM sample rate and DSD format', async () => {
    const files: ScannedFile[] = [];
    const metadataByPath = new Map<string, MetadataResult>();
    const metadataReader: MetadataReader = {
      read: vi.fn(async (filePath: string) => metadataByPath.get(filePath) ?? metadataResult()),
    };
    const harness = createHarness({
      fileScanner: new FakeFileScanner(files),
      metadataReader,
      coverExtractor: new FakeCoverExtractor(),
    });
    const cdPath = writeAudioFile(harness.folder, 'CD Track.flac');
    const hiResPath = writeAudioFile(harness.folder, 'Hi Res Track.flac');
    const dsdPath = writeAudioFile(harness.folder, 'Native DSD.dsf');
    const otherPath = writeAudioFile(harness.folder, 'Podcast Rate.wav');
    [cdPath, hiResPath, dsdPath, otherPath].forEach((path, index) => {
      files.push({
        path,
        sizeBytes: 100 + index,
    ...
    metadataByPath.set(cdPath, metadataResult({
      title: 'CD Track',
      codec: 'FLAC',
      sampleRate: 44_100,
      bitDepth: 16,
    }));
    metadataByPath.set(hiResPath, metadataResult({
      title: 'Hi Res Track',
      codec: 'FLAC',
      sampleRate: 192_000,
      bitDepth: 24,
    }));
    metadataByPath.set(dsdPath, metadataResult({
      title: 'Native DSD',
      codec: 'DSF',
      sampleRate: 2_822_400,
      bitDepth: 1,
    }));
    metadataByPath.set(otherPath, metadataResult({
      title: 'Podcast Rate',
      codec: 'WAV',
      sampleRate: 32_000,
      bitDepth: 16,
    ...
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'sampleRate44100' }).items.map((track) => track.title)).toEqual(['CD Track']);
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'sampleRate192000' }).items.map((track) => track.title)).toEqual(['Hi Res Track']);
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'dsd' }).items.map((track) => track.title)).toEqual(['Native DSD']);
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'otherSampleRate' }).items.map((track) => track.title)).toEqual(['Podcast Rate']);
    expect(harness.service.getTracks({ pageSize: 10, audioFormatFilter: 'all' }).total).toBe(4);

    harness.cleanup();
  });

  it('filters songs to osu imports from embedded beatmap tags', async () => {
    const files: ScannedFile[] = [];
    const metadataByPath = new Map<string, MetadataResult>();
    const metadataReader: MetadataReader = {
      read: vi.fn(async (filePath: string) => metadataByPath.get(filePath) ?? metadataResult()),
    ...
    const osuPath = writeAudioFile(harness.folder, 'osu import.mp3');
    const normalPath = writeAudioFile(harness.folder, 'normal song.mp3');
    const fanPath = writeAudioFile(harness.folder, 'osu fan song.mp3');
    [osuPath, normalPath, fanPath].forEach((path, index) => {
      files.push({
        path,
        sizeBytes: 100 + index,
        mtimeMs: 1704067200000 + index,
    ...
    metadataByPath.set(osuPath, metadataResult({
      title: 'Downloaded osu Song',
      album: 'osu! beatmapset 2141496',
    }));
    metadataByPath.set(normalPath, metadataResult({
      title: 'Normal Song',
      album: 'Regular Album',
    }));
    metadataByPath.set(fanPath, metadataResult({
      title: 'osu Fan Song',
      album: 'osu! fan collection',
    }));
  ...
  it('imports osu folders as mp3-only library tracks that pass the osu filter', async () => {
    const files: ScannedFile[] = [];
    const metadataByPath = new Map<string, MetadataResult>();
    const metadataReader: MetadataReader = {
      read: vi.fn(async (filePath: string) => metadataByPath.get(filePath) ?? metadataResult()),
    ...
    const writeEmbeddedTrackTags = vi.fn(async ({ filePath, tags }: Parameters<EmbeddedTrackTagWriter>[0]) => {
      metadataByPath.set(filePath, metadataResult({
        title: tags.title,
        artist: tags.artist,
        album: tags.album,
    ...
    const setDir = join(harness.folder, '2492872 Artist - Title');
    mkdirSync(setDir, { recursive: true });
    const audioPath = writeAudioFile(setDir, 'song.mp3');
    const wavPath = writeAudioFile(setDir, 'hitnormal.wav');
    const coverPath = join(setDir, 'bg.jpg');
    writeFileSync(coverPath, Buffer.from([1, 2, 3]));
    writeFileSync(
      join(setDir, 'hard.osu'),
      '[General]\nAudioFilename: song.mp3\n\n[Metadata]\nTitle: ASCII Title\nTitleUnicode: Imported Title\nArtist: ASCII Artist\nArtistUnicode: Imported Artist\nBeatmapID: 5318008\nBeatmapSetID: 2492872\n\n[Events]\n0,0,"bg.jpg",0,0\n',
      'utf8',
    );
    files.push({
      path: audioPath,
    ...
    const result = await harness.service.importOsuFolder(harness.folder);
    await harness.service.waitForScan(result.scan.id);
    ...
    expect(writeEmbeddedTrackTags).toHaveBeenCalledWith(expect.objectContaining({
      filePath: audioPath,
      coverData: expect.objectContaining({ mimeType: 'image/jpeg' }),
    ...
    expect(writeEmbeddedTrackTags).not.toHaveBeenCalledWith(expect.objectContaining({ filePath: wavPath }));
    expect(fileScanner.optionCalls).toHaveLength(0);
    expect(harness.service.getTracks({ pageSize: 10 }).items.map((track) => track.path)).toEqual([audioPath]);
    expect(harness.service.getTracks({ pageSize: 10, showOsuOnly: true }).items.map((track) => track.title)).toEqual(['Imported Title']);
    expect(harness.service.getAlbums({ pageSize: 10 }).total).toBe(0);
  ...
  it('writes edited track tags to the source file before confirming the library update', async () => {
    const writeEmbeddedTrackTags = vi.fn<EmbeddedTrackTagWriter>(async () => undefined);
    const harness = createHarness({ writeEmbeddedTrackTags });
    const filePath = writeAudioFile(harness.folder, 'Society.mp3');
    const imported = await harness.service.importAudioFile(filePath);
    ...
    const tags = {
      ...
      album: 'Society',
      albumArtist: 'pathetic240px',
      trackNo: null,
      discNo: null,
      year: 2026,
      genre: null,
    };
    const updated = await harness.service.updateTrackTags({
      trackId: imported.id,
      tags,
      coverPath: null,
      coverUrl: null,
    ...
    expect(updated.title).toBe('Society');
    expect(updated.artist).toBe('pathetic240px');
    expect(updated.embeddedMetadataStatus).toBe('present');
    harness.cleanup();
  });

  it('does not persist edited track tags in the library when the source write fails', async () => {
    const writeEmbeddedTrackTags = vi.fn<EmbeddedTrackTagWriter>(async () => {
      throw new Error('tag writer boom');
    });
    const harness = createHarness({ writeEmbeddedTrackTags });
    const filePath = writeAudioFile(harness.folder, 'Write Failure.mp3');
    const imported = await harness.service.importAudioFile(filePath);

    await expect(
      harness.service.updateTrackTags({
        trackId: imported.id,
        tags: {
          title: 'Manual Title',
    ...
    ).rejects.toThrow('tag writer boom');

    expect(writeEmbeddedTrackTags).toHaveBeenCalledTimes(1);
    expect(harness.service.getTrack(imported.id)?.title).toBe(imported.title);
    expect(harness.service.getTrack(imported.id)?.artist).toBe(imported.artist);
    harness.cleanup();
  });
  ...
  it('getAlbums artist sort orders Chinese album artists by pinyin', async () => {
    const harness = createHarness();
    const artists = [
      ['ado', 'Ado'],
      ['caodong', '\u8349\u4e1c\u6ca1\u6709\u6d3e\u5bf9'],
    ...
    for (const [id, artist] of artists) {
      const filePath = writeAudioFile(harness.folder, `${id}.flac`);
      harness.metadataService.overrides.set(
        filePath,
        baseMetadata({
          title: `${id} song`,
    ...
    const sorted = harness.service.getAlbums({ pageSize: 10, sort: 'artist' });

    expect(sorted.items.map((album) => album.albumArtist)).toEqual(artists.map(([, artist]) => artist));
    harness.cleanup();
  });

  it('getAlbums default sort orders Chinese album titles by pinyin', async () => {
    const harness = createHarness();
    const albums = [
      ['magic', '\u9b54\u6cd5\u7535\u53f0'],
      ['fantasy', '\u8303\u7279\u897f'],
    ...
    for (const [id, title] of albums) {
      const filePath = writeAudioFile(harness.folder, `${id}.flac`);
      harness.metadataService.overrides.set(
        filePath,
        baseMetadata({
          title: `${id} song`,
          artist: 'Album Sort Artist',
          album: title,
          albumArtist: 'Album Sort Artist',
        }),
      );
    }
    harness.addFolder();
    ...
    const sorted = harness.service.getAlbums({ pageSize: 10, sort: 'default' });

    expect(sorted.items.map((album) => album.title)).toEqual([
      '1989',
      '\u516b\u5ea6\u7a7a\u95f4',
      '\u8303\u7279\u897f',
      '\u9b54\u6cd5\u7535\u53f0',
  ...
  it('getArtistAlbums keeps a main-artist album when one track has guest fallback album artist', async () => {
    const files: ScannedFile[] = [];
    const metadataByPath = new Map<string, MetadataResult>();
    const metadataReader: MetadataReader = {
      read: vi.fn(async (filePath: string) => metadataByPath.get(filePath) ?? metadataResult()),
    ...
    const addFile = (name: string): string => {
      const filePath = writeAudioFile(harness.folder, name);
      files.push({
        path: filePath,
        sizeBytes: 100 + files.length,
        mtimeMs: 1704067200000 + files.length,
      });
      return filePath;
    };
    const first = addFile('Welcome To New York.flac');
    const second = addFile('Blank Space.flac');
    const third = addFile('Style.flac');
    const guest = addFile('Bad Blood Remix.flac');
    metadataByPath.set(
      first,
      metadataResult(metadataWithSources(
        { title: 'Welcome To New York', artist: 'Taylor Swift', album: '1989', albumArtist: 'Taylor Swift', trackNo: 1 },
        { albumArtist: 'artist_fallback' },
      )),
    );
    metadataByPath.set(
      second,
      metadataResult(metadataWithSources(
        { title: 'Blank Space', artist: 'Taylor Swift', album: '1989', albumArtist: 'Taylor Swift', trackNo: 2 },
        { albumArtist: 'artist_fallback' },
      )),
    );
    metadataByPath.set(
      third,
      metadataResult(metadataWithSources(
        { title: 'Style', artist: 'Taylor Swift', album: '1989', albumArtist: 'Taylor Swift', trackNo: 3 },
        { albumArtist: 'artist_fallback' },
      )),
    );
    metadataByPath.set(
      guest,
      metadataResult(metadataWithSources(
        { title: 'Bad Blood Remix', artist: 'Kendrick Lamar', album: '1989', albumArtist: 'Kendrick Lamar', trackNo: 4 },
        { albumArtist: 'artist_fallback' },
      )),
    );
    harness.addFolder();
    ...
    harness.service.refreshAlbumGrouping();
    expect(harness.service.getTracks({ pageSize: 10 }).total).toBe(4);
    expect(harness.service.getAlbums({ pageSize: 10 }).total).toBe(1);
    const artist = harness.service.getArtists({ pageSize: 10 }).items.find((item) => item.name === 'Taylor Swift');
    if (!artist) {
      throw new Error('Taylor Swift artist missing');
    }
    const albums = harness.service.getArtistAlbums(artist.id, { page: 1, pageSize: 10 });

    expect(artist).toMatchObject({ name: 'Taylor Swift', albumCount: 1 });
    expect(albums.total).toBe(1);
    expect(albums.items[0]).toMatchObject({ title: '1989', albumArtist: 'Taylor Swift', trackCount: 4 });
    harness.cleanup();
  });
