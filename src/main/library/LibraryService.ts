...
export class LibraryService {
  ...
  private removeDeletedPathsFromWatcher(_folderId: string, paths: string[]): number {
    const trackIds = Array.from(
      new Set(
        paths
          .map((filePath) => this.store.getTrackByPath(resolve(filePath))?.id ?? null)
    ...
    const changed = this.deleteTracks(trackIds);
    if (changed > 0) {
      this.notifyLibraryChanged();
    }
  ...
  private getWatcherService(): LibraryWatcherService {
    if (!this.watcherService) {
      this.watcherService = new LibraryWatcherService({
        ...
        rescanCoordinator: {
          removeDeletedPaths: (folderId, paths) => this.removeDeletedPathsFromWatcher(folderId, paths),
