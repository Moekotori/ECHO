...
import { getPlaybackPerformanceSnapshot } from '../diagnostics/PlaybackPerformanceDiagnostics';
import { getRecentRuntimePerformanceStalls } from '../diagnostics/RuntimePerformanceDiagnostics';
...
const albumIndexAppliedFieldKeys = new Set<keyof NetworkApplyResult['appliedFields']>([
  'title',
  'artist',
  'album',
  'albumArtist',
...
const hasAlbumIndexAppliedField = (result: NetworkApplyResult): boolean =>
...
type TrimmedMap = Pick<Map<string, unknown>, 'delete' | 'keys' | 'size'>;

const trimMapToLimit = (
  cache: TrimmedMap,
  limit: number,
): { beforeEntries: number; afterEntries: number; removedEntries: number } => {
  const beforeEntries = cache.size;
...
const emptyLibraryPage = <T>(query?: Pick<LibraryPageQuery, 'page' | 'pageSize'>): LibraryPage<T> => ({
  items: [],
  page: Math.max(1, Math.trunc(query?.page ?? 1)),
  pageSize: Math.max(1, Math.trunc(query?.pageSize ?? 0)),
  total: 0,
  hasMore: false,
});

const emptyLibrarySummary = (): LibrarySummary => ({
  songCount: 0,
  albumCount: 0,
  artistCount: 0,
  folderCount: 0,
...
const emptyPlaybackMemoryGraph = (): PlaybackMemoryGraph => ({
  generatedAt: new Date(0).toISOString(),
  totals: {
    playCount: 0,
    completedCount: 0,
...
const emptyAlbumOnlineInfo = (albumId: string): AlbumOnlineInfo => ({
  albumId,
  status: 'empty',
  sources: [],
  match: null,
...
const getLibraryPerformanceDiagnostics = (): LibraryDiagnostics['performance'] => {
  ...
  const lastSlowIpcChannel = playbackPerformance.lastSlowIpcChannel;
  const lastSlowIpcDurationMs = playbackPerformance.lastSlowIpcDurationMs;
  const lastSlowIpcAgeMs = playbackPerformance.lastSlowIpcAgeMs;
  const hasSlowIpc = lastSlowIpcChannel !== null && lastSlowIpcDurationMs !== null && lastSlowIpcAgeMs !== null;

  return {
    lastSlowIpc: hasSlowIpc
      ? {
          channel: lastSlowIpcChannel,
          durationMs: lastSlowIpcDurationMs,
          ageMs: lastSlowIpcAgeMs,
          failed: playbackPerformance.lastSlowIpcFailed,
        }
      : null,
    recentStalls: getRecentRuntimePerformanceStalls(6),
  };
};

const diagnosticsDirectorySizeEntryBudget = 400;
const diagnosticsSizeSnapshotTtlMs = 60_000;

type DiagnosticSizeSnapshot = {
  path: string;
...
export class LibraryService {
  ...
  private readonly playbackSafeTracksCache = new Map<string, LibraryPage<LibraryTrack>>();
  private readonly playbackSafeAlbumsCache = new Map<string, LibraryPage<LibraryAlbum>>();
  private readonly playbackSafeAlbumDetailsCache = new Map<string, LibraryAlbumDetail | null>();
  private readonly playbackSafeAlbumTracksCache = new Map<string, LibraryPage<LibraryTrack>>();
  private readonly playbackSafeAlbumOnlineInfoCache = new Map<string, AlbumOnlineInfo>();
  ...
  constructor(
    ...
    this.unregisterSoftMemoryCleanup = registerSoftMemoryCleanupTask('library-playback-safe-caches', () => this.releaseSoftMemoryPressure());
  }

  private rememberPlaybackSafeRead<T>(cache: Map<string, T>, key: string, value: T): T {
    cache.set(key, value);
    if (cache.size > playbackSafeReadCacheLimit) {
      const oldestKey = cache.keys().next().value;
      if (oldestKey) {
        cache.delete(oldestKey);
      }
  ...
  releaseSoftMemoryPressure(): SoftMemoryCleanupTaskResult {
    const caches = [
      this.playbackSafeTracksCache,
      this.playbackSafeAlbumsCache,
      this.playbackSafeAlbumDetailsCache,
      this.playbackSafeAlbumTracksCache,
      this.playbackSafeAlbumOnlineInfoCache,
    ...
    return {
      ...
      beforeEntries: stats.reduce((total, item) => total + item.beforeEntries, 0),
      afterEntries: stats.reduce((total, item) => total + item.afterEntries, 0),
  ...
  scanFolder(folderId: string, options: LibraryScanOptions = {}): LibraryScanStatus {
    ...
    const scanOptions = folder.importProfile === 'osu'
      ? {
        ...options,
        audioExtensions: ['.mp3'],
        osuImport: true,
  ...
  async importOsuFolder(folderPath: string): Promise<ImportOsuFolderResult> {
    let folder = this.addFolder(folderPath);
    this.store.setFolderImportProfile(folder.id, 'osu');
    folder = this.store.getFolder(folder.id) ?? { ...folder, importProfile: 'osu' };
    this.scanJobQueue.cancelScansForFolder(folder.id);
    const embeddedTagResult = await writeOsuFolderEmbeddedTags(folder.path, {
      writeEmbeddedTrackTags: this.embeddedTrackTagWriter,
    });
    const { matchedAudioPaths, ...publicEmbeddedTagResult } = embeddedTagResult;
    if (embeddedTagResult.metadataFiles > 0) {
      const removedTracks = this.store.markTracksMissingFromFolder(folder.id, matchedAudioPaths);
      if (removedTracks > 0) {
        this.scheduleGroupingRefresh();
        this.notifyLibraryChanged();
      }
    }
    const scan = this.scanJobQueue.scanPaths(folder, matchedAudioPaths, {
      audioExtensions: ['.mp3'],
      osuImport: true,
      reduceScanPressure: true,
    });
  ...
  getTracksPlaybackSafe(query?: LibraryPageQuery): Promise<LibraryPage<LibraryTrack>> {
    ...
    return runNonCriticalMainWork({
      name: 'library:get-tracks',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeTracksCache, key, this.getTracks(query)),
      fallback: () => this.playbackSafeTracksCache.get(key) ?? emptyLibraryPage(query),
    });
  }
  ...
  getAlbumsPlaybackSafe(query?: LibraryPageQuery): Promise<LibraryPage<LibraryAlbum>> {
    const key = playbackSafeCacheKey(query);
    return runNonCriticalMainWork({
      name: 'library:get-albums',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeAlbumsCache, key, this.getAlbums(query)),
      fallback: () => this.playbackSafeAlbumsCache.get(key) ?? emptyLibraryPage(query),
    });
  }
  ...
  getAlbumPlaybackSafe(albumId: string): Promise<LibraryAlbumDetail | null> {
    return runNonCriticalMainWork({
      name: 'library:get-album',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeAlbumDetailsCache, albumId, this.getAlbum(albumId)),
      fallback: () => this.playbackSafeAlbumDetailsCache.get(albumId) ?? null,
    });
  }
  ...
  getAlbumOnlineInfoPlaybackSafe(albumId: string, options: AlbumOnlineInfoRequestOptions = {}): Promise<AlbumOnlineInfo> {
    const key = playbackSafeCacheKey({ albumId, options });
    return runNonCriticalMainWork({
      name: 'library:get-album-online-info',
      work: async () => this.rememberPlaybackSafeRead(this.playbackSafeAlbumOnlineInfoCache, key, await this.getAlbumOnlineInfo(albumId, options)),
      fallback: () => this.playbackSafeAlbumOnlineInfoCache.get(key) ?? emptyAlbumOnlineInfo(albumId),
    });
  }
  ...
  getAlbumForTrackPlaybackSafe(trackId: string): Promise<LibraryAlbum | null> {
    return runNonCriticalMainWork({
      name: 'library:get-album-for-track',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeAlbumForTrackCache, trackId, this.getAlbumForTrack(trackId)),
      fallback: () => this.playbackSafeAlbumForTrackCache.get(trackId) ?? null,
    });
  }
  ...
  getArtistsPlaybackSafe(query?: LibraryPageQuery): Promise<LibraryPage<LibraryArtist>> {
    const key = playbackSafeCacheKey(query);
    return runNonCriticalMainWork({
      name: 'library:get-artists',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeArtistsCache, key, this.getArtists(query)),
      fallback: () => this.playbackSafeArtistsCache.get(key) ?? emptyLibraryPage(query),
    });
  }
  ...
  getArtistAlbumsPlaybackSafe(
    artistId: string,
    query?: Pick<LibraryPageQuery, 'page' | 'pageSize' | 'sort'>,
  ): Promise<LibraryPage<LibraryAlbum>> {
    const key = playbackSafeCacheKey({ artistId, query });
    return runNonCriticalMainWork({
      name: 'library:get-artist-albums',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeArtistAlbumsCache, key, this.getArtistAlbums(artistId, query)),
      fallback: () => this.playbackSafeArtistAlbumsCache.get(key) ?? emptyLibraryPage(query),
    });
  }
  ...
  getAlbumTracksPlaybackSafe(albumId: string, query?: Pick<LibraryPageQuery, 'page' | 'pageSize'>): Promise<LibraryPage<LibraryTrack>> {
    const key = playbackSafeCacheKey({ albumId, query });
    return runNonCriticalMainWork({
      name: 'library:get-album-tracks',
      work: () => this.rememberPlaybackSafeRead(this.playbackSafeAlbumTracksCache, key, this.getAlbumTracks(albumId, query)),
      fallback: () => this.playbackSafeAlbumTracksCache.get(key) ?? emptyLibraryPage(query),
    });
  }
  ...
  getSummaryPlaybackSafe(): Promise<LibrarySummary> {
    return runNonCriticalMainWork({
      name: 'library:get-summary',
      work: () => {
        this.playbackSafeSummaryCache = this.getSummary();
        return this.playbackSafeSummaryCache;
      },
      fallback: () => this.playbackSafeSummaryCache ?? emptyLibrarySummary(),
    });
  }
  ...
  getDiagnostics(): LibraryDiagnostics {
    ...
    return {
      ...
      moveCandidates: this.moveCandidateEnabled ? this.getMoveCandidates() : [],
      performance: getLibraryPerformanceDiagnostics(),
    };
  }

  private getCoverCacheSizeForDiagnostics(): number | null {
    const now = Date.now();
    if (
      this.coverCacheSizeSnapshot?.path === this.coverCacheDir &&
      now - this.coverCacheSizeSnapshot.measuredAtMs <= diagnosticsSizeSnapshotTtlMs
    ) {
      return this.coverCacheSizeSnapshot.sizeBytes;
    }

    const sizeBytes = directorySize(this.coverCacheDir, { maxEntries: diagnosticsDirectorySizeEntryBudget });
    this.coverCacheSizeSnapshot = {
      path: this.coverCacheDir,
      sizeBytes,
      measuredAtMs: now,
    };
    return sizeBytes;
  ...
  async importAudioFile(
    ...
    return runMainBackgroundTask('library-import-files', async () => {
      ...
      const metadata = await repairAlacTechnicalMetadataBeforeWrite(
        normalizedPath,
        await this.metadataReader.read(normalizedPath),
      );
  ...
  async getPlaybackMemoryGraphPlaybackSafe(query?: PlaybackHistoryQuery): Promise<PlaybackMemoryGraph> {
    const key = playbackSafeCacheKey(query);
    return runNonCriticalMainWork({
      name: 'library:get-playback-memory-graph',
      work: () => this.getPlaybackMemoryGraph(query),
      fallback: () => this.playbackMemoryGraphCache.get(key) ?? emptyPlaybackMemoryGraph(),
    });
  }
  ...
  refreshAlbumGroupingPlaybackSafe(): Promise<LibraryAlbumGroupingRefreshResult> {
    if (this.hasRunningJobs()) {
      throw new Error('Cannot refresh album grouping while a library scan is running.');
    }

    return runNonCriticalMainWork<LibraryAlbumGroupingRefreshResult>({
      name: 'library:refresh-album-grouping',
      work: () => ({
        status: 'completed' as const,
        summary: this.refreshAlbumGrouping(),
      }),
      fallback: () => {
  ...
  async updateTrackTags(request: LibraryTrackTagUpdateRequest): Promise<LibraryTrack> {
    ...
    const embeddedWriteStatus = await this.writeEmbeddedTagsNowOrQueue({
    ...
    const tagUpdate = {
      ...
      embeddedMetadataStatus: embeddedWriteStatus === 'written' ? 'present' as const : 'pending' as const,
      embeddedCoverStatus: coverData ? (embeddedWriteStatus === 'written' ? 'present' as const : 'pending' as const) : currentTrack.embeddedCoverStatus,
    };
  ...
  private async applyNetworkCandidateCover(
    ...
    this.reindexAlbumForAppliedMetadata(row.id, {
      status: 'applied_missing_only',
      appliedFields: { coverId: appliedCoverId },
    });
  ...
  private reindexAlbumForAppliedMetadata(trackId: string, result: NetworkApplyResult): void {
    if (!hasAlbumIndexAppliedField(result)) {
      return;
    }

    this.store.seedAlbumsForTracks([trackId], this.albumService, new Date().toISOString(), this.albumRefreshOptions());
    this.scheduleGroupingRefresh();
  }
  ...
  setCoverCacheDir(coverCacheDir: string): void {
    ...
    this.coverCacheSizeSnapshot = null;
  ...
  private async writeEmbeddedTagsNowOrQueue(request: EmbeddedTagWriteRequest): Promise<'written' | 'queued'> {
    if (await shouldDelayEmbeddedTagWriteForAudio(request.filePath)) {
      this.scheduleEmbeddedTagWrite(request);
      return 'queued';
    }

    await this.runEmbeddedTagWrite(request);
    return 'written';
  }

  private async runEmbeddedTagWrite(request: EmbeddedTagWriteRequest): Promise<void> {
    await this.embeddedTrackTagWriter({
  ...
  private scheduleEmbeddedTagWrite(request: EmbeddedTagWriteRequest, attempt = 1): void {
    const attemptWrite = async (): Promise<void> => {
      if (await shouldDelayEmbeddedTagWriteForAudio(request.filePath)) {
        const retryTimer = setTimeout(() => {
          this.scheduleEmbeddedTagWrite(request, attempt);
        }, embeddedTagWriteRetryDelayMs);
        retryTimer.unref?.();
        return;
      }
      ...
      try {
        ...
        await this.syncTrackFileStat(request.trackId, {
          ...
          embeddedCoverStatus: request.coverData ? 'present' : undefined,
        });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (attempt < embeddedTagWriteMaxAttempts) {
          console.warn(`${request.errorPrefix} for ${request.filePath}; retrying (${attempt}/${embeddedTagWriteMaxAttempts}): ${message}`);
          const retryTimer = setTimeout(() => {
            this.scheduleEmbeddedTagWrite(request, attempt + 1);
          }, embeddedTagWriteRetryDelayMs);
          retryTimer.unref?.();
          return;
        }

        console.warn(`${request.errorPrefix} for ${request.filePath}: ${message}`);
        await this.markEmbeddedTagWriteFailed(request.trackId, Boolean(request.coverData));
      }
    };
  ...
  private async syncTrackFileStat(
    trackId: string,
    statusUpdate: Partial<Pick<LibraryTrack, 'embeddedMetadataStatus' | 'embeddedCoverStatus'>> = {},
  ): Promise<void> {
    ...
    const tagUpdate = {
      ...
      embeddedMetadataStatus: statusUpdate.embeddedMetadataStatus ?? track.embeddedMetadataStatus,
      embeddedCoverStatus: statusUpdate.embeddedCoverStatus ?? track.embeddedCoverStatus,
    };
    this.store.updateTrackTags(track.id, tagUpdate, undefined, await this.store.prepareTrackTagSearchTerms(track.id, tagUpdate));
  }

  private async markEmbeddedTagWriteFailed(trackId: string, includesCover: boolean): Promise<void> {
    const track = this.store.getTrack(trackId);
    if (!track || !existsSync(track.path)) {
      return;
    }
    ...
    const tagUpdate = {
      ...
      fieldSources: track.fieldSources,
      metadataStatus: track.metadataStatus ?? 'ok',
      embeddedMetadataStatus: 'error' as const,
      embeddedCoverStatus: includesCover ? 'error' as const : track.embeddedCoverStatus,
    };
    this.store.updateTrackTags(track.id, tagUpdate, undefined, await this.store.prepareTrackTagSearchTerms(track.id, tagUpdate));
  }
}
...
const directorySize = (targetPath: string, options: { maxEntries?: number } = {}): number | null => {
