...
const onsetEnvelope = (samples: Float32Array): Float32Array => {
  ...
  const transientEnergy = new Float32Array(frameCount);
  const energyFlux = new Float32Array(frameCount);
  const transientFlux = new Float32Array(frameCount);
  ...
  for (let frame = 0; frame < frameCount; frame += 1) {
    ...
    let sum = 0;
    let transientSum = 0;
    let previousSample = samples[start - 1] ?? 0;
    for (let offset = 0; offset < frameSize; offset += 1) {
      const sample = samples[start + offset] ?? 0;
      sum += sample * sample;
      const delta = sample - previousSample;
      transientSum += delta * delta;
      previousSample = sample;
    }
    energy[frame] = Math.sqrt(sum / frameSize);
    transientEnergy[frame] = Math.sqrt(transientSum / frameSize);
  }

  for (let frame = 1; frame < frameCount; frame += 1) {
    const historyStart = Math.max(0, frame - 10);
    const energyBaseline = median(Array.from(energy.slice(historyStart, frame)));
    const transientBaseline = median(Array.from(transientEnergy.slice(historyStart, frame)));
    energyFlux[frame] = Math.max(0, energy[frame] - energyBaseline * 1.1);
    transientFlux[frame] = Math.max(0, transientEnergy[frame] - transientBaseline * 1.08);
  }

  const energyScale = percentile(Array.from(energyFlux), 0.95) || Math.max(...energyFlux);
  const transientScale = percentile(Array.from(transientFlux), 0.95) || Math.max(...transientFlux);
  for (let frame = 0; frame < frameCount; frame += 1) {
    const normalizedEnergy = energyScale > 0 ? clamp(energyFlux[frame] / energyScale, 0, 1.5) : 0;
    const normalizedTransient = transientScale > 0 ? clamp(transientFlux[frame] / transientScale, 0, 1.5) : 0;
    envelope[frame] = normalizedEnergy * 0.58 + normalizedTransient * 0.42;
  }
