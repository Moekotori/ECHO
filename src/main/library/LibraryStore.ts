...
const knownPcmSampleRates = [44_100, 48_000, 88_200, 96_000, 176_400, 192_000, 352_800, 384_000];
const knownPcmSampleRateSql = knownPcmSampleRates.join(', ');
const dsdAudioFormatSql = "(UPPER(COALESCE(codec, '')) IN ('DSF', 'DFF', 'DSD') OR COALESCE(sample_rate, 0) >= 1000000)";
const osuFieldSourceSql = "lower(trim(COALESCE(json_extract(field_sources_json, '$.osu'), ''))) = 'osu'";
const osuLegacyAlbumSql = "lower(trim(COALESCE(album, ''))) LIKE 'osu! beatmapset %'";
const osuImportedTrackSql = `(media_type = 'local' AND (${osuFieldSourceSql} OR ${osuLegacyAlbumSql}))`;

const isOsuImportedFieldSources = (fieldSources: Record<string, unknown>): boolean =>
  String(fieldSources.osu ?? '').trim().toLowerCase() === 'osu';

const isLegacyOsuAlbumTitle = (album: unknown): boolean =>
  String(album ?? '').trim().toLowerCase().startsWith('osu! beatmapset ');

const unifiedTrackAudioFormatFilterSql = (filter?: LibraryAudioFormatFilter): string => {
  switch (filter) {
    ...
    case 'sampleRate88200':
      return 'sample_rate = 88200';
    case 'sampleRate96000':
      return 'sample_rate = 96000';
    case 'sampleRate176400':
      return 'sample_rate = 176400';
    case 'sampleRate192000':
      return 'sample_rate = 192000';
    case 'sampleRate352800':
      return 'sample_rate = 352800';
    case 'sampleRate384000':
      return 'sample_rate = 384000';
    case 'dsd':
      return dsdAudioFormatSql;
    case 'otherSampleRate':
      return `COALESCE(sample_rate, 0) > 0 AND sample_rate NOT IN (${knownPcmSampleRateSql}) AND NOT ${dsdAudioFormatSql}`;
    case 'all':
    default:
      return '';
  }
...
export class LibraryStore {
  ...
  addFolder(folderPath: string): LibraryFolder {
    ...
    this.run(
      `INSERT INTO folders (id, path, name, status, enabled, import_profile, last_scan_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      id,
      normalizedPath,
      name,
    ...
    return {
      ...
      name,
      status: 'active',
      importProfile: 'normal',
      createdAt: timestamp,
      updatedAt: timestamp,
    };
  }

  setFolderImportProfile(folderId: string, importProfile: 'normal' | 'osu', timestamp = nowIso()): void {
    this.run('UPDATE folders SET import_profile = ?, updated_at = ? WHERE id = ?', importProfile, timestamp, folderId);
  }
  ...
  getActiveScanJobsForFolder(folderId: string): LibraryScanStatus[] {
    return this.allRows(
      "SELECT * FROM scan_jobs WHERE folder_id = ? AND status IN ('queued', 'running') ORDER BY created_at ASC",
      folderId,
    ).map((row) => this.mapScanJob(row));
  }
  ...
  private buildStandardAlbumGroupsForRows(tracks: DbRow[], albumService: AlbumService): Map<string, StandardAlbumGroup> {
    ...
    tracks.forEach((track, index) => {
      ...
      if (isOsuImportedFieldSources(fieldSources) || isLegacyOsuAlbumTitle(title)) {
        return;
      }
  ...
  getTracks(query?: LibraryPageQuery): LibraryPage<LibraryTrack> {
    ...
    const audioFormatFilterSql = unifiedTrackAudioFormatFilterSql(query?.audioFormatFilter);
    const osuOnlyFilterSql = query?.showOsuOnly === true ? osuImportedTrackSql : '';
    const libraryWhereParts = [
      mediaTypeFilter ? 'media_type = ?' : '',
      sourceId ? 'source_id = ?' : '',
      osuOnlyFilterSql,
      audioFormatFilterSql,
      excludeTrackIdsSql,
    ].filter(Boolean);
  ...
  private unifiedAlbumOrderSql(sort: string): string {
    switch (sort) {
      ...
      case 'titleDesc':
        return 'ORDER BY echo_library_sort_key(title) COLLATE NOCASE DESC, title COLLATE NOCASE DESC, echo_library_sort_key(album_artist) COLLATE NOCASE, album_artist COLLATE NOCASE';
      case 'durationAsc':
  ...
  private mapFolder(row: DbRow): LibraryFolder {
    return {
      ...
      importProfile: row.import_profile === 'osu' ? 'osu' : 'normal',
      createdAt: String(row.created_at),
      updatedAt: String(row.updated_at),
    };
  }
