...
export class NetworkMetadataService {
  ...
  private appliedHandler: ((trackId: string, result: NetworkApplyResult) => void) | null = null;
  ...
  setAppliedHandler(handler: ((trackId: string, result: NetworkApplyResult) => void) | null): void {
        ...
        try {
          ...
          for (const candidate of candidates) {
            ...
            if (result.status === 'applied_missing_only') {
              applied.push(result);
              this.notifyApplied(trackId, result);
  ...
  applyMissingOnly(candidateId: string, options?: NetworkApplyOptions): NetworkApplyResult {
    ...
    const result = this.merge.applyMissingOnly(candidateId, false, options?.fields);
    if (candidate) {
      this.notifyApplied(candidate.trackId, result);
    }
    return result;
  }
  ...
  applySelected(candidateId: string, options?: NetworkApplyOptions): NetworkApplyResult {
    const candidate = this.store.getMetadataCandidate(candidateId);
    const result = this.merge.applyMissingOnly(candidateId, true, options?.fields);
    if (candidate) {
      this.notifyApplied(candidate.trackId, result);
    }
    return result;
  ...
  private notifyApplied(trackId: string, result: NetworkApplyResult): void {
    if (result.status !== 'applied_missing_only' || Object.keys(result.appliedFields).length === 0) {
      return;
    }

    this.appliedHandler?.(trackId, result);
  }
