...
const getVendorOrFallback = (database: OpraDatabase, vendorId: string): OpraVendor =>
  database.vendors.get(vendorId) ?? { id: vendorId, name: vendorId, logoPath: null, blurb: null };
...
export class OpraService {
  ...
  async search(request: OpraHeadphoneCorrectionSearchRequest): Promise<OpraHeadphoneCorrectionSearchResult> {
    ...
    const scoredProducts = Array.from(database.products.values())
      .map((product) => {
        const vendor = getVendorOrFallback(database, product.vendorId);
  ...
  async browse(request: OpraHeadphoneCorrectionBrowseRequest = {}): Promise<OpraHeadphoneCorrectionBrowseResult> {
    ...
    const selectedVendor = selectedProduct ? getVendorOrFallback(database, selectedProduct.vendorId) : null;
  ...
  async apply(request: OpraHeadphoneCorrectionApplyRequest): Promise<OpraHeadphoneCorrectionApplyResult> {
    ...
    const preset = EqStateStore.savePreset(preview.preset);
    let state = EqStateStore.loadEqState();
    const shouldEnable = request.enableEq !== false && !state.enabled;
    state = { ...state, ...preset, presetId: preset.id, presetName: preset.name, enabled: shouldEnable || state.enabled };
