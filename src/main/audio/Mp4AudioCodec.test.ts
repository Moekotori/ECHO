...
describe('MP4 audio codec sample-entry reader', () => {
  ...
  it('reads ALAC sample-entry codecs from .alac files', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'echo-mp4-codec-'));
    const filePath = join(tempDir, 'lossless.alac');

    try {
      await writeFile(filePath, mp4WithAudioCodec('alac'));

      await expect(readMp4AudioSampleEntryCodec(filePath)).resolves.toBe('ALAC');
      await expect(resolveMp4ContainerAudioCodec(filePath, 'MPEG-4')).resolves.toBe('ALAC');
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
  });
