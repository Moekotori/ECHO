...
export class EqBridge extends EventEmitter {
  ...
  private nativeControlConnected = false;
  ...
  connect(port: number): void {
    ...
    this.nativeControlConnected = false;
    ...
    socket.on('connect', () => {
      ...
      connected = true;
      this.nativeControlConnected = true;
      socket.setTimeout(0);
  ...
  async syncStateToNative(): Promise<void> {
    ...
    const roomCorrectionIrPath = this.roomCorrectionIrPath;
    ...
    if (roomCorrectionIrPath && existsSync(roomCorrectionIrPath) && roomCorrectionState.irId && roomCorrectionState.irName) {
    ...
    } else if (roomCorrectionState.irId || roomCorrectionState.enabled || roomCorrectionIrPath) {
      this.roomCorrectionState = {
        ...defaultRoomCorrectionState(),
  ...
  private async sendNativeNow(message: Record<string, unknown>): Promise<EqState> {
    ...
    if ((!this.nativeControlConnected || !socket.writable) && !(await this.waitForNativeControlConnection(socket))) {
      return this.getState();
    }
  ...
  private waitForNativeControlConnection(socket: net.Socket): Promise<boolean> {
    if (this.socket === socket && this.nativeControlConnected && !socket.destroyed && socket.writable) {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      const finish = (connected: boolean): void => {
        ...
        socket.off('timeout', handleClose);
        resolve(connected);
      };
      const handleConnect = (): void => finish(this.socket === socket && !socket.destroyed && socket.writable);
      const handleClose = (): void => finish(false);

      socket.once('connect', handleConnect);
      socket.once('close', handleClose);
      socket.once('error', handleClose);
      socket.once('timeout', handleClose);
...
export const getEqBridge = (): EqBridge => {
  if (!defaultEqBridge) {
    ...
    defaultEqBridge.setMaxListeners(512);
  }
