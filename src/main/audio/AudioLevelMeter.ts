...
const hardClipThreshold = 1.02;
const hardClipReleaseThreshold = 1;
const hardClipEventCooldownSeconds = 2;
...
export const createAudioLevelTelemetry = (
  ...
  const estimatedOutputTruePeakDb = addDb(snapshot.inputTruePeakDb ?? null, estimatedGainDb);
  const outputClipPeakDb = estimatedOutputTruePeakDb ?? estimatedOutputPeakDb;
  const outputHardClipped = snapshot.clipCount > 0 && outputClipPeakDb !== null && outputClipPeakDb >= hardClipThresholdDb;
  const intersamplePeakDb =
    snapshot.inputTruePeakDb === null || snapshot.inputTruePeakDb === undefined || snapshot.inputPeakDb === null
      ? null
      : Math.round((snapshot.inputTruePeakDb - snapshot.inputPeakDb) * 10) / 10;
  ...
  return {
    ...
    inputTruePeakDb: snapshot.inputTruePeakDb ?? null,
    estimatedOutputTruePeakDb,
    truePeakHeadroomDb: estimatedOutputTruePeakDb === null ? null : Math.round(-estimatedOutputTruePeakDb * 10) / 10,
    intersamplePeakDb,
...
export class PcmLevelMeterTransform extends Transform {
  ...
  private truePeakHistory: number[][] = [];
  ...
  constructor(
    ...
    this.truePeakHistory = Array.from({ length: this.channels }, () => []);
  ...
  getSnapshot(): PcmLevelSnapshot {
    ...
    return {
      ...
      inputTruePeakDb: dbFromLinear(this.truePeakAbs),
  ...
  private observeSample(input: Buffer, offset: number, absoluteSampleIndex: number): void {
    ...
    this.observeTruePeakSample(absoluteSampleIndex % this.channels, sample);
  ...
  private observeTruePeakSample(channelIndex: number, sample: number): void {
    this.truePeakAbs = Math.max(this.truePeakAbs, Math.abs(sample));
    const history = this.truePeakHistory[channelIndex];
    if (!history) {
      return;
    }
    ...
    const [previous, left, right, next] = history as [number, number, number, number];
    for (const fraction of [0.25, 0.5, 0.75]) {
      const interpolated =
        previous * (-fraction * (fraction - 1) * (fraction - 2) / 6) +
        left * ((fraction + 1) * (fraction - 1) * (fraction - 2) / 2) +
        right * (-(fraction + 1) * fraction * (fraction - 2) / 2) +
        next * ((fraction + 1) * fraction * (fraction - 1) / 6);
      if (Number.isFinite(interpolated)) {
        this.truePeakAbs = Math.max(this.truePeakAbs, Math.abs(interpolated));
      }
    }
  }
