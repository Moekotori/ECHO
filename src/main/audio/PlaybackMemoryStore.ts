import { existsSync, mkdirSync, readFileSync, rmSync, writeFile, writeFileSync } from 'node:fs';
...
const normalizeProbe = (value: unknown): PlaybackProbeHint | undefined => {
  ...
  if (fileSampleRate !== null) {
    probe.fileSampleRate = Math.round(fileSampleRate);
  }
  ...
  if (bitDepth !== null) {
    probe.bitDepth = Math.round(bitDepth);
  }
  if (bitrate !== null) {
    probe.bitrate = Math.round(bitrate);
  }
...
export class PlaybackMemoryStore {
  ...
  save(status: AudioStatus): void {
    ...
    if (status.state === 'playing') {
      // Use async write to avoid blocking the audio thread during playback
      writeFile(memoryPath, data, 'utf8', () => {});
    } else {
      writeFileSync(memoryPath, data, 'utf8');
    }
  }
