...
describe('AudioLevelMeter', () => {
  ...
  it('counts clearly separated hard clipping events without counting every waveform cycle', async () => {
    const separatedClips = [1.08, ...Array.from({ length: 90000 }, () => 0), -1.09];
    const result = await runMeter(separatedClips, { channels: 1 });

    expect(result.snapshot.clipCount).toBe(2);
  });

  it('uses the configured sample rate when debouncing hard clip events', async () => {
    const srcSeparatedClips = [1.08, ...Array.from({ length: 220000 }, () => 0), -1.09];
    const result = await runMeter(srcSeparatedClips, { channels: 2, sampleRateHz: 352800 });
  ...
  it('estimates intersample true peak without changing playback bytes', async () => {
    const samples = [0, 1, 1, 0];
    const input = pcmBuffer(samples);
    const result = await runMeter(samples, { channels: 1 });

    expect(result.snapshot.inputPeakDb).toBe(0);
    expect(result.snapshot.inputTruePeakDb).toBeGreaterThan(0);
    expect(result.output.equals(input)).toBe(true);
  });

  it('treats tiny decoder overshoots as headroom pressure without counting hard clips', async () => {
    const result = await runMeter([0.4, 1.015, -1.014, 0.6]);

    expect(result.snapshot.inputPeakDb).toBe(0.1);
    expect(result.snapshot.clipCount).toBe(0);
    expect(result.snapshot.lastClipAt).toBeNull();
  });
  ...
  it('carries true peak estimates into output headroom telemetry', () => {
    const telemetry = createAudioLevelTelemetry(
      {
        inputPeakDb: 0,
        inputRmsDb: -12,
        inputTruePeakDb: 1.2,
      ...
      eqState({ enabled: true, dspHeadroomDb: -3 }),
      channelBalanceState(),
    );

    expect(telemetry.estimatedOutputTruePeakDb).toBe(-1.8);
    expect(telemetry.truePeakHeadroomDb).toBe(1.8);
    expect(telemetry.intersamplePeakDb).toBe(1.2);
  });
