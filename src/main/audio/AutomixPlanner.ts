...
export const normalizeAutomixAnalysis = (
  ...
  const durationSeconds = normalizeDuration(analysis.durationSeconds || probe.durationSeconds);
  if (durationSeconds <= 0) {
    return {
      ...estimated,
      status: 'unavailable',
...
const headBucketSlice = (curve: number[]): number =>
  Math.max(4, Math.min(curve.length, Math.ceil(curve.length * 0.42)));

const analysisWindowSeconds = (start: number, end: number, durationSeconds: number): number =>
  clamp(
    end > start ? end - start : durationSeconds * 0.12,
    4,
    Math.min(36, durationSeconds * 0.25),
  );
...
const selectNextEntryEnergyBuckets = (analysis: TrackTransitionAnalysis, nextStartSeconds: number): number[] => {
  ...
  const headBucketCount = headBucketSlice(analysis.energyCurve);
  const introWindowSeconds = analysisWindowSeconds(
    analysis.introStartSeconds,
    analysis.introEndSeconds,
...
const estimateLowEnergyOutroTrimSeconds = (analysis: TrackTransitionAnalysis): number => {
  ...
  const fallingTailThreshold =
    tailPeak - tailEnd >= 0.18
      ...
      : 0.34;
  const outroWindowSeconds = analysisWindowSeconds(
    analysis.outroStartSeconds,
    analysis.outroEndSeconds,
    analysis.durationSeconds,
  );
...
export const planAutomixTransition = (input: AutomixPlanInput): AutomixTransitionPlan | null => {
  ...
  if (shortTrackFallback || unavailableAnalysis) {
    ...
    fallbackReason = unavailableAnalysis ? 'analysis_unavailable' : 'short_track';
  } else if (beatAligned) {
    mode = 'beatAligned';
  } else if (energyReady) {
    mode = 'energyFade';
  ...
  const minOverlap = shortTrackFallback || unavailableAnalysis ? 0.8 : 2;
  const maxOverlap = Math.min(maxTransitionSeconds, availableCurrentSeconds - 0.4, availableNextSeconds - 0.4);
  overlapSeconds = clamp(overlapSeconds, minOverlap, maxOverlap);
  if (overlapSeconds <= 0) {
    return null;
  }
  ...
  return {
    ...
    tempoRatio,
    advanceAtSeconds: roundToMillis(currentFadeStartSeconds + overlapSeconds * 0.5),
    skipIntroSilence,
