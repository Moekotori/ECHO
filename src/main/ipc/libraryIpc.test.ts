...
const installLibraryService = () => {
  ...
  const service = {
    ...
    getAlbum: vi.fn(),
    getAlbumPlaybackSafe: vi.fn(),
    getAlbumOnlineInfo: vi.fn((albumId: string) => Promise.resolve({
      albumId,
      status: 'empty',
      sources: [],
      match: null,
    ...
    getAlbumOnlineInfoPlaybackSafe: vi.fn((albumId: string) => Promise.resolve({
      albumId,
      status: 'empty',
      sources: [],
      match: null,
    ...
    getAlbumForTrackPlaybackSafe: vi.fn(),
    getArtists: vi.fn(),
    getArtistsPlaybackSafe: vi.fn(),
    getArtist: vi.fn(),
    ...
    getArtistAlbumsPlaybackSafe: vi.fn(),
    ...
    getAlbumTracksPlaybackSafe: vi.fn(),
    getSummary: vi.fn(() => ({ songCount: 2, albumCount: 1, artistCount: 2, folderCount: 1, totalDuration: 2, lastScanAt: null })),
    getSummaryPlaybackSafe: vi.fn(() => Promise.resolve({ songCount: 2, albumCount: 1, artistCount: 2, folderCount: 1, totalDuration: 2, lastScanAt: null })),
    ...
    getPlaybackHistoryPlaybackSafe: vi.fn(() => Promise.resolve({
      items: [],
      page: 1,
      pageSize: 50,
      total: 0,
    ...
    getPlaybackHistorySummaryPlaybackSafe: vi.fn(() => Promise.resolve({
      todayCount: 0,
    ...
    getPlaybackHistorySummary: vi.fn(() => ({
      todayCount: 1,
      todayPlayedSeconds: 180,
      totalCount: 8,
      latestPlayedAt: '2026-05-20T00:00:00.000Z',
      rangeCount: 3,
      rangePlayedSeconds: 420,
      rangeLatestPlayedAt: '2026-05-20T00:00:00.000Z',
    })),
    getPlaybackStatsDashboard: vi.fn(() => ({
      generatedAt: '2026-05-20T00:00:00.000Z',
      totals: {
        playCount: 3,
        completedCount: 2,
      ...
      topTracks: [],
      topArtists: [{ artist: 'Aimer', playCount: 3, completedCount: 2, playedSeconds: 420 }],
      topAlbums: [],
      formatBreakdown: [],
      qualityBreakdown: [],
      dailyActivity: [{ date: '2026-05-20', playCount: 3, playedSeconds: 420 }],
    })),
    ...
    getPlaybackMemoryGraphPlaybackSafe: vi.fn(() => Promise.resolve({
      generatedAt: '2026-05-20T00:00:00.000Z',
      totals: {
        playCount: 0,
        completedCount: 0,
...
describe('library IPC', () => {
  ...
  it('refreshes album grouping through the playback-safe IPC path', async () => {
    const service = installLibraryService();

    const result = await handlers[IpcChannels.LibraryRefreshAlbumGroupingPlaybackSafe]!();

    expect(service.refreshAlbumGroupingPlaybackSafe).toHaveBeenCalledTimes(1);
    expect(service.refreshAlbumGrouping).not.toHaveBeenCalled();
    expect(result).toMatchObject({ status: 'completed', summary: { albumCount: 1 } });
  });

  it('reads playback history, summary, and stats through playback-safe paths', async () => {
    const service = installLibraryService();
    ...
    const result = await handlers[IpcChannels.LibraryGetPlaybackStatsDashboard]!(null, {
      from: '2026-05-18T00:00:00.000Z',
      to: '2026-05-25T00:00:00.000Z',
      statsMode: 'activity',
    });

    expect(service.getPlaybackHistory).not.toHaveBeenCalled();
    ...
    expect(service.getPlaybackHistorySummaryPlaybackSafe).toHaveBeenCalledWith({
      from: '2026-05-18T00:00:00.000Z',
      to: '2026-05-25T00:00:00.000Z',
    });
    expect(service.getPlaybackStatsDashboard).not.toHaveBeenCalled();
    expect(service.getPlaybackStatsDashboardPlaybackSafe).toHaveBeenCalledWith({
      from: '2026-05-18T00:00:00.000Z',
      to: '2026-05-25T00:00:00.000Z',
      statsMode: 'activity',
    });
    expect(historyResult).toMatchObject({ total: 0 });
    expect(summaryResult).toMatchObject({ rangeCount: 0, rangePlayedSeconds: 0 });
    expect(result).toMatchObject({ totals: { playCount: 3 }, topArtists: [{ artist: 'Aimer' }] });
  });
  ...
  it('loads album online info through the direct async IPC path', async () => {
    const service = installLibraryService();

    const result = await handlers[IpcChannels.LibraryGetAlbumOnlineInfo]!(null, 'album-1', {
      force: true,
      provider: 'wikipedia',
    });

    expect(service.getAlbumOnlineInfo).toHaveBeenCalledWith('album-1', { force: true, provider: 'wikipedia' });
    expect(service.getAlbumOnlineInfoPlaybackSafe).not.toHaveBeenCalled();
    expect(result).toMatchObject({ albumId: 'album-1' });
  });
