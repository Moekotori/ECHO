...
const audioFormatFilterValues = new Set<LibraryAudioFormatFilter>([
  'all',
  'sampleRate44100',
  'sampleRate48000',
  'sampleRate88200',
...
const normalizeQuery = (value: unknown): LibraryPageQuery => {
  ...
  if (typeof input.audioFormatFilter === 'string' && audioFormatFilterValues.has(input.audioFormatFilter as LibraryAudioFormatFilter)) {
    query.audioFormatFilter = input.audioFormatFilter as LibraryAudioFormatFilter;
  }
...
export const registerLibraryIpc = (): void => {
  ...
  ipcMain.handle(IpcChannels.LibraryImportOsuFolder, () => {
    throw new Error('导入 osu! 文件夹功能已禁用。');
  });
  ...
  ipcMain.handle(IpcChannels.LibraryRefreshAlbumGroupingPlaybackSafe, () => getLibraryService().refreshAlbumGroupingPlaybackSafe());
  ...
  ipcMain.handle(IpcChannels.LibraryGetPlaybackHistory, (_event, query: unknown) =>
    getLibraryService().getPlaybackHistoryPlaybackSafe(normalizePlaybackHistoryQuery(query)),
  );
  ipcMain.handle(IpcChannels.LibraryGetPlaybackHistorySummary, (_event, query: unknown) =>
    getLibraryService().getPlaybackHistorySummaryPlaybackSafe(normalizePlaybackHistoryQuery(query)),
  );
