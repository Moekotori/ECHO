...
import { resolveLiveStream } from '../streaming/LiveStreamResolver';
...
const normalizeLiveResolveRequest = (value: unknown): StreamingLiveResolveRequest => {
  const input = requireObject(value, 'streaming live resolve request');
  return {
    url: requireText(input.url, 'live URL'),
  };
};
...
export const registerStreamingIpc = (): void => {
  ...
  ipcMain.handle(IpcChannels.StreamingListAccountPlaylists, async (_event, provider: unknown) => {
    try {
      return await getStreamingService().listAccountPlaylists(requireLikedProvider(provider));
    } catch (error) {
      throw friendlyError(error, 'Streaming account playlists could not be listed.');
    }
  });
  ...
  ipcMain.handle(IpcChannels.StreamingResolveLive, async (_event, request: unknown) => {
    try {
      const input = normalizeLiveResolveRequest(request);
      return await resolveLiveStream(input.url);
    } catch (error) {
      throw friendlyError(error, 'Live stream could not be resolved.');
    }
  });
