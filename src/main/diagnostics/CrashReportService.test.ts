...
describe('CrashReportService', () => {
  ...
  it('truncates long renderer routes and keeps largest process metrics aligned', () => {
    const service = new CrashReportService(tempDir);
    service.initialize();
    const browserProcess = {
      pid: 100,
    ...
    const rendererProcess = {
      ...
      type: 'Tab',
      name: 'renderer',
      workingSetBytes: 320_000_000,
      peakWorkingSetBytes: 360_000_000,
      privateBytes: 250_000_000,
      cpuPercent: 0.5,
    };
    const longDataRoute = `data:text/html;charset=utf-8,${'x'.repeat(320)}SECRET_AFTER_LIMIT`;

    service.reportMemoryPressure({
      timestamp: '2026-06-21T02:11:00.000Z',
      thresholdBytes: 3 * 1024 * 1024 * 1024,
      totalWorkingSetBytes: 3_600_000_000,
      totalPrivateBytes: 2_700_000_000,
      processCount: 2,
      ...
      metrics: [browserProcess, rendererProcess],
      topProcesses: [browserProcess, rendererProcess],
      rendererProcesses: [
        {
          timestamp: '2026-06-21T02:11:00.000Z',
          pid: 200,
          ...
          route: longDataRoute,
          routeDetail: {
            locationRoute: longDataRoute,
            reactRouteId: null,
            pageMode: null,
            activeRouteId: null,
            activeRouteHidden: null,
            visibleRouteIds: [],
            lyricsViewMode: null,
    ...
    const sessionDir = service.getSessionDir()!;
    const report = readFileSync(join(sessionDir, 'memory-pressure-report.md'), 'utf8');
    const latest = readFileSync(join(sessionDir, 'memory-pressure.latest.json'), 'utf8');
    expect(report).toContain('Largest process: Browser (2.14 GiB working, 1.77 GiB private).');
    expect(report).toContain('Dominant renderer sample:');
    expect(report).toContain('[truncated, hash ');
    expect(report).not.toContain('SECRET_AFTER_LIMIT');
    expect(latest).not.toContain('SECRET_AFTER_LIMIT');
  });

  it('flags duplicate MV video decode pressure for lyrics MV renderer reports', () => {
    const service = new CrashReportService(tempDir);
    service.initialize();
    const tabProcess = {
      pid: 220,
      ...
      privateBytes: 3_100_000_000,
      cpuPercent: 6.5,
    };

    const event = service.reportMemoryPressure({
      timestamp: '2026-06-21T02:12:00.000Z',
      thresholdBytes: 3 * 1024 * 1024 * 1024,
      totalWorkingSetBytes: 3_900_000_000,
      totalPrivateBytes: 3_500_000_000,
      processCount: 2,
      source: 'electron-app-metrics',
      currentProcess: {
        pid: 100,
        rssBytes: 420_000_000,
        heapTotalBytes: 140_000_000,
        heapUsedBytes: 120_000_000,
        externalBytes: 30_000_000,
        arrayBuffersBytes: 10_000_000,
      },
      metrics: [tabProcess],
      topProcesses: [tabProcess],
      rendererProcesses: [
        {
          timestamp: '2026-06-21T02:12:00.000Z',
          pid: 220,
          ...
          route: 'index.html',
          routeDetail: {
            locationRoute: 'index.html',
            reactRouteId: 'lyrics',
            pageMode: 'lyrics:mv',
            activeRouteId: 'lyrics',
            activeRouteHidden: false,
            visibleRouteIds: ['lyrics'],
            lyricsViewMode: 'mv',
            settingsSection: null,
          },
          process: tabProcess,
          heap: {
            usedJSHeapSize: 70_000_000,
            totalJSHeapSize: 120_000_000,
          ...
          images: {
            ...
            loadedImageCount: 4,
            brokenImageCount: 0,
            echoCoverImageCount: 2,
            echoImageProtocolImageCount: 0,
            dataImageCount: 0,
            remoteImageCount: 0,
            estimatedDecodedBytes: 40_000_000,
            largestImages: [],
            lifecycle: {
              ...
              unmounted: 10,
              srcChanged: 4,
              bySrcKind: { 'echo-cover': 26 },
              byVariant: { original: 8, large: 6, thumb: 12 },
              recentEvents: [
                {
                  ageMs: 800,
                  type: 'src-change',
          ...
          visibleState: {
            currentReactRoute: 'lyrics',
            currentPageMode: 'lyrics:mv',
            lyricsPageVisible: true,
            mvPanelVisible: true,
            mvForegroundVideoCount: 1,
            mvBackgroundVideoCount: 1,
            mvForegroundYoutubeFrameCount: 0,
            mvBackgroundYoutubeFrameCount: 0,
          },
          userActions: {
            recentWindowMs: 30_000,
            counts: {
              'app:navigate:lyrics': 1,
              click: 2,
            },
            recent: [
              {
                ageMs: 1200,
                type: 'app:navigate:lyrics',
                routeId: 'songs',
                detail: 'mode:mv',
              },
            ],
    ...
    expect(event.graphicsPressure).toMatchObject({
      kind: 'lyrics-mv-render-pressure',
      duplicateMvVideoDecode: true,
      rendererPid: 220,
    });

    const report = readFileSync(join(service.getSessionDir()!, 'memory-pressure-report.md'), 'utf8');
    expect(report).toContain('Duplicate MV video decode pressure is visible');
    expect(report).toContain('## Most Likely Next Inspections');
    expect(report).toContain('Lyrics/MV graphics or GPU resources');
    expect(report).toContain('## Graphics Pressure Recommendation');
    expect(report).toContain('Recommendation: lyrics-mv-render-pressure');
    expect(report).toContain('Current reduction state: lyrics reduced no, MV reduced no');
    expect(report).toContain('### Renderer Pressure Ranking');
    expect(report).toContain('Recent activity correlation');
    expect(report).toContain('React route/page mode: lyrics / lyrics:mv');
    expect(report).toContain('Recent user actions (30s)');
    expect(report).toContain('Image lifecycle (30s)');
    expect(report).toContain('Video details');
    expect(report).toContain('lyrics-mv-background-video');
  });

  it('breaks down per-process growth in retained memory trend samples', () => {
    const service = new CrashReportService(tempDir);
    service.initialize();
    ...
    service.reportMemoryPressure({
      ...
      metrics: [
        {
          ...
          type: 'Tab',
          name: 'renderer',
          workingSetBytes: 1_400_000_000,
          peakWorkingSetBytes: 1_500_000_000,
          privateBytes: 1_200_000_000,
          cpuPercent: 3.5,
        },
        {
          pid: 330,
          ...
          peakWorkingSetBytes: 520_000_000,
          privateBytes: 1_700_000_000,
          cpuPercent: 0.8,
        },
      ],
      topProcesses: [
      ...
      rendererProcesses: [
        {
          ...
          images: {
            ...
            loadedImageCount: 8,
            brokenImageCount: 0,
            echoCoverImageCount: 8,
            echoImageProtocolImageCount: 0,
            dataImageCount: 0,
            remoteImageCount: 0,
            estimatedDecodedBytes: 60_000_000,
            largestImages: [],
            lifecycle: {
              recentWindowMs: 30_000,
              mounted: 8,
        ...
        recentRequests: [
          {
            ...
            method: 'GET',
            urlHash: 'url-large',
            resourceHash: 'cover-large',
            source: 'local-cover-cache',
            outcome: 'ok',
            statusCode: 200,
            startedAt: '2026-06-21T02:11:58.000Z',
            completedAt: '2026-06-21T02:11:59.000Z',
            durationMs: 10,
            knownBytes: 4_000_000,
          },
        ],
        maxRecentRequests: 120,
        byScheme: { 'echo-cover': 1 },
        byOutcome: { ok: 1 },
        bySource: { 'local-cover-cache': 1 },
        byStatusCode: { '200': 1 },
        byRouteKind: { large: 1 },
        byVariant: { large: 1 },
        byTargetHost: {},
        totalKnownBytesServed: 4_000_000,
        recentKnownBytesServed: 4_000_000,
        trackedUniqueResourceCount: 1,
      ...
      recentSamples: [
        {
          timestamp: '2026-06-21T02:10:00.000Z',
          totalWorkingSetBytes: 1_800_000_000,
          totalPrivateBytes: 1_900_000_000,
          topProcesses: [
            {
              pid: 220,
              type: 'Tab',
            ...
            },
            {
              ...
              type: 'GPU',
              serviceName: 'GPU',
              workingSetBytes: 260_000_000,
              privateBytes: 600_000_000,
              cpuPercent: 0.2,
            },
          ],
        },
    ...
    const report = readFileSync(join(service.getSessionDir()!, 'memory-pressure-report.md'), 'utf8');
    expect(report).toContain('## Sudden Memory Spike Events');
    expect(report).toContain('retained sample window(s) crossed sudden-spike thresholds');
    expect(report).toContain('lyrics:click');
    expect(report).toContain('lyrics:mount:echo-cover');
    expect(report).toContain('echo-cover:large');
    expect(report).toContain('### Memory Inflection Windows');
    expect(report).toContain('Largest Process Delta');
    expect(report).toContain('### Process Growth Breakdown');
    expect(report).toContain('GPU#330');
    expect(report).toContain('GPU process memory is high');
  });

  it('summarizes echo-cover variant pressure in memory pressure reports', () => {
    const service = new CrashReportService(tempDir);
    service.initialize();

    service.reportMemoryPressure({
      timestamp: '2026-06-21T02:13:00.000Z',
      thresholdBytes: 3 * 1024 * 1024 * 1024,
      totalWorkingSetBytes: 3_600_000_000,
      totalPrivateBytes: 2_900_000_000,
      processCount: 1,
      source: 'electron-app-metrics',
      currentProcess: {
        pid: 100,
        rssBytes: 500_000_000,
        heapTotalBytes: 200_000_000,
        heapUsedBytes: 150_000_000,
      ...
      metrics: [
        {
          ...
          type: 'Tab',
          name: 'renderer',
          workingSetBytes: 3_200_000_000,
          peakWorkingSetBytes: 3_300_000_000,
          privateBytes: 2_700_000_000,
        },
      ],
      topProcesses: [
        {
      ...
      coverProtocol: {
        ...
        recentRequests: [
          {
            ...
            method: 'GET',
            urlHash: 'url-1',
            resourceHash: 'cover-a',
            source: 'local-cover-cache',
            outcome: 'ok',
            statusCode: 200,
            startedAt: '2026-06-21T02:12:59.000Z',
            completedAt: '2026-06-21T02:13:00.000Z',
            durationMs: 8,
            knownBytes: 12_000_000,
          },
          {
            id: 2,
            scheme: 'echo-cover',
        ...
        maxRecentRequests: 120,
        byScheme: { 'echo-cover': 6 },
        byOutcome: { ok: 6 },
        bySource: { 'local-cover-cache': 6 },
        byStatusCode: { '200': 6 },
        byRouteKind: { original: 4, large: 2 },
        byVariant: { original: 4, large: 2 },
        byTargetHost: {},
        totalKnownBytesServed: 30_000_000,
        recentKnownBytesServed: 16_000_000,
        trackedUniqueResourceCount: 2,
        uniqueResourceTrackingTruncated: false,
        topResources: [
          {
            scheme: 'echo-cover',
            ...
            resourceHash: 'cover-a',
            requestCount: 4,
            knownBytes: 24_000_000,
            lastOutcome: 'ok',
            lastStatusCode: 200,
            lastCompletedAt: '2026-06-21T02:13:00.000Z',
          },
        ],
      },
      appVersion: '1.0.1-test',
    ...
    const report = readFileSync(join(service.getSessionDir()!, 'memory-pressure-report.md'), 'utf8');
    expect(report).toContain('### Echo-cover Variant Pressure');
    expect(report).toContain('### Cover/Image Hotspot Analysis');
    expect(report).toContain('Recent pressure by variant');
    expect(report).toContain('Repeated/heavy resources');
    expect(report).toContain('| original | 4 | 1 | 1 | 11.4 MiB | 22.9 MiB |');
    expect(report).toContain('| large | 2 | 1 | 1 | 3.8 MiB | 0.0 MiB |');
  });
