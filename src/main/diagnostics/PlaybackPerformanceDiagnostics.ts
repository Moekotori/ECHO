...
const slowPlaybackStepWarnCooldownMs = 10_000;
...
const logStep = (context: PlaybackPerformanceContext, durationMs: number): void => {
  ...
  if (roundedDurationMs >= slowPlaybackStepWarnThresholdMs) {
    const warnKey = `${context.operation}:${context.phase}`;
    const now = Date.now();
    const lastWarnAt = lastSlowPlaybackStepWarnAtByKey.get(warnKey);
    if (lastWarnAt !== undefined && now - lastWarnAt < slowPlaybackStepWarnCooldownMs) {
      return;
    }

    lastSlowPlaybackStepWarnAtByKey.set(warnKey, now);
    console.warn(
...
export const recordIpcMainHandlerDuration = (
  ...
  const warnKey = `${channel}:${failed ? 'failed' : 'slow'}`;
  const lastWarnAt = lastSlowIpcWarnAtByKey.get(warnKey);
  const now = Date.now();
  if (lastWarnAt !== undefined && now - lastWarnAt < slowIpcWarnCooldownMs) {
    return;
  }

  lastSlowIpcWarnAtByKey.set(warnKey, now);
  console.warn(
