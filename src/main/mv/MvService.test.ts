...
describe('MvService', () => {
  ...
  it('allows immersive MV background scale below 100 percent', () => {
    const database = {
      close: vi.fn(),
      prepare: vi.fn(() => ({
        run: vi.fn(() => ({ changes: 0 })),
    ...
    const service = new MvService(database, { getTrack: () => null }, undefined, undefined, []);

    expect(service.setSettings({ immersiveBackgroundScalePercent: 70 }).immersiveBackgroundScalePercent).toBe(70);
    expect(appSettingsMock.current.mvImmersiveBackgroundScalePercent).toBe(70);
  });
  ...
  it('uses the configured auto-apply threshold for network MV candidates', async () => {
    ...
    service.setSettings({ autoApplyThreshold: 0.85, preferHighestViewCount: false });
    await service.searchNetworkCandidates(track.id);
    expect(service.getSelectedVideo(track.id)).toBeNull();
  ...
  it('uses title and artist for MV search when title-only search is disabled', async () => {
    const provider: MainMvOnlineProvider = {
      id: 'bilibili',
      search: vi.fn(async () => []),
      resolve: vi.fn(async () => [makeResolvedVariant()]),
    ...
    service.setSettings({ titleOnlySearch: false });
    await service.searchNetworkCandidates(track.id);
