...
function writeCrashReport(type, error, extraContext = {}) {
  try {
    ...
    const report = [
      ...
      "",
      "--- SYSTEM INFO ---",
      ...Object.entries(getSystemInfo()).map(
        ([k, v]) => `  ${k.padEnd(16)}: ${v}`,
      ),
      "",
      "--- ERROR ---",
...
export function initCrashReporter(getAudioEngineStatus = null) {
  ...
  try {
    crashReporter.start({
      productName: "Echoes Studio",
      companyName: "EchoesStudio",
      submitURL: "", // 不上传，仅本地保存
    ...
    console.warn(
      "[CrashReporter] Native crashReporter init failed:",
      e.message,
    );
  }
  ...
  console.log(
    `[CrashReporter] All handlers registered. Reports → ${getCrashDir()}`,
  );
}
