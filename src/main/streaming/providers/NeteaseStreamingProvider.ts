...
const mapNeteaseAccountPlaylist = (playlistValue: Record<string, unknown>, userId: string): StreamingAccountPlaylist | null => {
  const providerPlaylistId = neteaseIdText(playlistValue.id ?? playlistValue.playlistId);
  const title = text(playlistValue.name);
  if (!providerPlaylistId || !title) {
    return null;
  }

  const creator = asRecord(playlistValue.creator);
  const creatorUserId = neteaseIdText(creator.userId ?? creator.userid);
  const rawCover = playlistValue.coverImgUrl ?? playlistValue.picUrl ?? playlistValue.coverUrl;
  const ownership =
    creatorUserId && creatorUserId === userId
      ? 'created'
      : playlistValue.subscribed === true || playlistValue.ordered === true
        ? 'favorited'
        : 'unknown';
  ...
  return {
    ...
    providerPlaylistId,
    title,
    description: text(playlistValue.description),
    creator: text(creator.nickname ?? creator.name ?? playlistValue.creatorName),
    coverUrl: neteaseImageUrl(rawCover, 600),
    coverThumb: neteaseImageUrl(rawCover, 160),
    trackCount: integer(playlistValue.trackCount ?? playlistValue.size ?? playlistValue.songCount),
    ownership,
...
export class NeteaseStreamingProvider implements StreamingProvider {
  ...
  async listAccountPlaylists(): Promise<StreamingAccountPlaylist[]> {
    const cookie = accountCookie();
    if (!cookie) {
      throw new Error('请先在设置里登录网易云音乐账号。');
    }
    ...
    if (ncm?.user_playlist) {
      try {
        bodies.push((await ncm.user_playlist({ uid: userId, limit: 1000, offset: 0, cookie })).body);
      } catch {
    ...
    for (const body of bodies) {
      for (const record of neteasePlaylistRecords(body)) {
        const playlist = mapNeteaseAccountPlaylist(record, userId);
        if (playlist && !playlists.has(playlist.providerPlaylistId)) {
          playlists.set(playlist.providerPlaylistId, playlist);
        }
      }
    }
