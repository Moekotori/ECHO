...
const qqPlaylistTitleKeys = ['dissname', 'diss_name', 'dirName', 'dirname', 'dir_name', 'name', 'title', 'titleName'] as const;
const qqPlaylistIdKeys = ['dissid', 'disstid', 'dissId', 'dirid', 'dirId', 'dirID', 'dir_id', 'playlistId', 'id', 'tid'] as const;
...
const mapPlaylist = (playlistValue: unknown): StreamingPlaylist => {
  ...
  const providerPlaylistId = qqPlaylistId(playlist) ?? text(playlist.name) ?? 'playlist';
  const title = qqPlaylistTitle(playlist) ?? 'QQ Music Playlist';
  const rawCover = text(playlist.imgurl) ?? text(playlist.logo) ?? text(playlist.diss_cover) ?? text(playlist.picurl) ?? text(playlist.cover_url) ?? text(playlist.coverUrl);
  ...
  return {
    ...
    trackCount: integer(playlist.song_count ?? playlist.songCount ?? playlist.songnum ?? playlist.song_cnt ?? playlist.total_song_num),
  };
};

const mapQqAccountPlaylist = (playlistValue: unknown, ownership: StreamingAccountPlaylist['ownership']): StreamingAccountPlaylist | null => {
  const playlist = mapPlaylist(playlistValue);
  if (!playlist.providerPlaylistId || playlist.providerPlaylistId === 'playlist' || playlist.providerPlaylistId === '0') {
    return null;
  }
  ...
  return {
    ...playlist,
    ownership,
    webUrl: `https://y.qq.com/n/ryqq/playlist/${encodeURIComponent(playlist.providerPlaylistId)}`,
  };
};

const uniqueQqAccountPlaylists = (items: StreamingAccountPlaylist[]): StreamingAccountPlaylist[] => {
  const playlists = new Map<string, StreamingAccountPlaylist>();
  for (const item of items) {
    if (!playlists.has(item.providerPlaylistId)) {
      playlists.set(item.providerPlaylistId, item);
    }
  }
...
export class QQMusicStreamingProvider implements StreamingProvider {
  ...
  async listAccountPlaylists(): Promise<StreamingAccountPlaylist[]> {
    ...
    if (!cookie || uin === '0') {
      throw new Error('请先在设置里登录 QQ 音乐账号。');
    }

    const results: StreamingAccountPlaylist[] = [];
    const errors: unknown[] = [];

    for (const [ownership, endpoint] of [
      ['created', 'https://c.y.qq.com/rsc/fcgi-bin/fcg_user_created_diss'] as const,
    ]) {
      try {
        results.push(...await this.fetchAccountPlaylistsFromLegacyEndpoint(cookie, uin, endpoint, ownership));
      } catch (error) {
        errors.push(error);
      }
    }
    ...
    const playlists = uniqueQqAccountPlaylists(results);
    if (playlists.length === 0 && errors.length > 0) {
      const firstError = errors.find((error): error is Error => error instanceof Error);
      throw firstError ?? new Error('QQ 音乐歌单列表读取失败。');
    }
  ...
  private async fetchAccountPlaylistsFromProfileAsset(
    cookie: string,
    uin: string,
    reqtype: string,
    ownership: StreamingAccountPlaylist['ownership'],
  ): Promise<StreamingAccountPlaylist[]> {
    const gtk = String(qqGtkFromCookie(cookie));
    const params = new URLSearchParams({
      loginUin: uin,
      hostUin: uin,
    ...
    return findPlaylistRecords(data)
      .map((playlist) => mapQqAccountPlaylist(playlist, ownership))
      .filter((playlist): playlist is StreamingAccountPlaylist => Boolean(playlist));
  }

  private async fetchAccountPlaylistsFromLegacyEndpoint(
    cookie: string,
    uin: string,
    endpoint: string,
    ownership: StreamingAccountPlaylist['ownership'],
    ...
    const data = await jsonFetch(`${endpoint}?${params.toString()}`, {
      headers: qqHeaders(cookie),
      timeoutMs: 12_000,
    });
