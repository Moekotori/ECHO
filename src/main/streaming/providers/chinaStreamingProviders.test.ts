...
describe('China streaming providers', () => {
  ...
  it('lists QQ Music account playlists from created and favorite endpoints', async () => {
    const fetchRunner = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({
          data: {
            disslist: [{ diss_name: '本地创建歌单', diss_cover: 'http://y.gtimg.cn/cover.jpg', dirid: 7788, song_cnt: 3 }],
          },
        }),
      )
      .mockResolvedValueOnce(
        jsonResponse({
          data: {
            cdlist: [{ dissid: '8899', dissname: '收藏歌单', songnum: 9, nickname: 'Other' }],
          },
        }),
      );
    vi.stubGlobal('fetch', fetchRunner);

    const playlists = await new QQMusicStreamingProvider().listAccountPlaylists();
    ...
    expect(playlists).toEqual([
      expect.objectContaining({
        ...
        title: '本地创建歌单',
        ownership: 'created',
        webUrl: 'https://y.qq.com/n/ryqq/playlist/7788',
        trackCount: 3,
      }),
      expect.objectContaining({
    ...
    expect(String(fetchRunner.mock.calls[0][0])).toContain('fcg_user_created_diss');
    expect(String(fetchRunner.mock.calls[1][0])).toContain('fcg_get_profile_order_asset');
    expect(String(fetchRunner.mock.calls[1][0])).toContain('reqtype=3');
  });
  ...
  it('lists NetEase account playlists as created or favorited', async () => {
        ...
        playlist: [
          {
            id: 1001,
            name: '我创建的歌单',
            creator: { userId: 42, nickname: 'Tester' },
            trackCount: 12,
            coverImgUrl: 'https://p.music.126.net/created.jpg',
          },
          {
            id: 1002,
            name: '我收藏的歌单',
            creator: { userId: 99, nickname: 'Other' },
            subscribed: true,
            trackCount: 8,
          },
        ],
    ...
    setNeteaseApiForTests({ login_status: loginStatus, user_playlist: userPlaylist });

    const playlists = await new NeteaseStreamingProvider().listAccountPlaylists();
    ...
    expect(playlists[0]?.coverThumb).toContain(encodeURIComponent('https://p.music.126.net/created.jpg?param=160y160'));
    expect(playlists[1]).toMatchObject({
      providerPlaylistId: '1002',
      ownership: 'favorited',
    });
  });
