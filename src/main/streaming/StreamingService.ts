...
export class StreamingService {
  ...
  constructor(
    ...
    this.unregisterSoftMemoryCleanup = registerSoftMemoryCleanupTask('streaming-memory-cache', () => this.releaseSoftMemoryPressure());
  }
  ...
  dispose(): void {
    ...
    for (const timer of this.pendingAlbumCacheWriteTimers.values()) {
      clearTimeout(timer);
    }
    this.pendingAlbumCacheWriteTimers.clear();
  }
  ...
  async listAccountPlaylists(providerName: LikedSongsSyncProviderName): Promise<StreamingAccountPlaylistsResult> {
    const provider = this.registry.get(providerName);
    if (!provider.listAccountPlaylists) {
      throw new Error('This streaming provider does not support account playlist listing.');
    }

    return {
      provider: providerName,
      playlists: await this.callProvider(provider, () => provider.listAccountPlaylists!(), 'Streaming account playlists'),
...
export const createStreamingProviderRegistry = (): StreamingProviderRegistry => {
  ...
  registry.register(new QobuzStreamingProvider());
...
export const closeDefaultStreamingService = (): void => {
  defaultStreamingService?.dispose();
