...
export type M3u8PlaybackTarget = {
  url: string;
...
const m3u8PlaybackTargetPayloadPrefix = 'echo-m3u8-target:';
...
const isHeaderRecord = (value: unknown): value is Record<string, string> =>
  ...
  Object.entries(value as Record<string, unknown>).every(([key, entry]) => typeof key === 'string' && typeof entry === 'string');

const decodeM3u8PlaybackTargetPayload = (raw: string): M3u8PlaybackTarget | null => {
  if (!raw.startsWith(m3u8PlaybackTargetPayloadPrefix)) {
    return null;
  }

  try {
    const parsed = JSON.parse(raw.slice(m3u8PlaybackTargetPayloadPrefix.length)) as Record<string, unknown>;
    ...
    return {
      url,
      headers: isHeaderRecord(parsed.headers) ? parsed.headers : {},
      mimeType: typeof parsed.mimeType === 'string' && parsed.mimeType.trim() ? parsed.mimeType.trim() : null,
      supportsRange: parsed.supportsRange !== false,
    };
  } catch {
    return null;
  }
...
export const decodeM3u8PlaybackTarget = (providerTrackId: string): M3u8PlaybackTarget => {
  const raw = decodeM3u8ProviderTrackId(providerTrackId);
  return decodeM3u8PlaybackTargetPayload(raw) ?? {
    url: raw,
    headers: {},
    mimeType: null,
    supportsRange: true,
