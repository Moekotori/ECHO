...
export function collectAudioFilesRecursive(entryPath, out) {
  try {
...
function scanFolders(folders) {
  ...
  for (const folder of folders) {
    collectAudioFilesRecursive(folder, files)
  }
  return uniqueByPath(files)
}

export function rescanImportedFolders(folders, existingPaths = []) {
  ...
  return scanFolders(normalizedFolders).filter((entry) => !existingPathSet.has(entry.path))
}
...
export function createLibraryWatchManager({ onChange }) {
  ...
  const runRescan = () => {
    ...
    scanning = true
    try {
      const nextSnapshot = buildSnapshot(scanFolders(watchedFolders))
  ...
  return {
    start(folders) {
      watchedFolders = Array.isArray(folders)
        ...
        : []
      closeAllWatchers()
      snapshot = buildSnapshot(scanFolders(watchedFolders))
      rebuildWatchers()
      return {
        ok: true,
