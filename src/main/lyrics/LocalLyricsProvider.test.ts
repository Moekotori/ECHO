...
import { maxLyricsTextBytes } from './lyricsParser';
...
describe('LocalLyricsProvider', () => {
  ...
  it('skips oversized sidecar lyrics before reading them', async () => {
    const root = makeTempRoot();
    const filePath = join(root, 'Echo Song.flac');
    writeFileSync(filePath, 'audio');
    writeFileSync(join(root, 'Echo Song.lrc'), 'x'.repeat(maxLyricsTextBytes + 1));
    ...
    await expect(provider.search(request(query(filePath)))).resolves.toEqual([]);
  });
