...
const readTextFile = (filePath: string): string | null => {
  try {
    if (statSync(filePath).size > maxLyricsTextBytes) {
      return null;
    }
...
const syncTextToLrc = (syncText: LyricsSyncText): string | null => {
  ...
  for (const line of syncText) {
    if (lines.length >= maxParsedLyricLines) {
      break;
    }

    if (typeof line.text === 'string' && line.text.trim() && typeof line.timestamp === 'number' && Number.isFinite(line.timestamp)) {
      lines.push(`${lrcTime(line.timestamp ?? 0)}${line.text.trim()}`);
    }
  }
...
const lyricsTextToProviderText = (value: string | null): { syncedLyrics: string | null; plainLyrics: string | null } => {
  ...
  if (value.length > maxLyricsTextChars) {
    return { syncedLyrics: null, plainLyrics: null };
  }
