...
export const maxLyricsTextChars = 1_000_000;
export const maxLyricsTextBytes = 2_000_000;
export const maxParsedLyricLines = 2_000;
export const maxLyricLineTextChars = 2_000;
export const maxLyricWordTimingsPerLine = 160;
export const maxSerializedLyricLinesChars = 3_000_000;

const isLyricsTextWithinLimit = (value: string): boolean => value.length <= maxLyricsTextChars;

const isLyricLineTextWithinLimit = (line: LyricLine): boolean =>
  line.text.length > 0 &&
  line.text.length <= maxLyricLineTextChars &&
  (line.words === undefined || line.words.length <= maxLyricWordTimingsPerLine) &&
  (line.translation == null || line.translation.length <= maxLyricLineTextChars) &&
  (line.romanization == null || line.romanization.length <= maxLyricLineTextChars) &&
  (line.kana == null || line.kana.length <= maxLyricLineTextChars);

const limitLyricLines = (lines: LyricLine[]): LyricLine[] =>
  lines.filter(isLyricLineTextWithinLimit).slice(0, maxParsedLyricLines);

const appendLyricLine = (lines: LyricLine[], line: LyricLine): void => {
  if (lines.length < maxParsedLyricLines) {
    lines.push(line);
  }
};
...
const attachWordTimings = (
  ...
  if (!words?.length || words.length > maxLyricWordTimingsPerLine) {
    return line;
  }
...
export const parseTtmlLyrics = (ttmlText: string): LyricLine[] => {
  if (!isLyricsTextWithinLimit(ttmlText) || !ttmlRootPattern.test(ttmlText)) {
    return [];
  }
  ...
  for (const match of ttmlText.matchAll(ttmlParagraphPattern)) {
    ...
    if (line) {
      appendLyricLine(lines, line);
    }
  }

  return limitLyricLines(lines.sort((left, right) => left.timeMs - right.timeMs));
};

export const parseSyncedLyrics = (lrcText: string): LyricLine[] => {
  if (!isLyricsTextWithinLimit(lrcText)) {
    return [];
  }
  ...
  for (const rawLine of lrcText.split(/\r?\n/)) {
    ...
    if (neteaseYrcLine) {
      appendLyricLine(lines, neteaseYrcLine);
      continue;
    }
    ...
    if (looksLikeBracketEnhancedLine(line, timestamps, leadingMatches)) {
      ...
      for (const match of leadingMatches) {
        ...
        appendLyricLine(lines, { timeMs, ...attachWordTimings(splitInlineTranslation(text), words) });
      }
...
export const parsePlainLyrics = (plainText: string): LyricLine[] => {
  if (!isLyricsTextWithinLimit(plainText) || ttmlRootPattern.test(plainText)) {
    return [];
  }

  return limitLyricLines(plainText
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export const serializeLyricLines = (lines: LyricLine[]): string => {
  const safeLines = limitLyricLines(lines);
  let retainedLineCount = safeLines.length;
  let serialized = JSON.stringify(safeLines);

  while (serialized.length > maxSerializedLyricLinesChars && retainedLineCount > 0) {
    retainedLineCount = Math.max(0, Math.floor(retainedLineCount * 0.75));
    serialized = JSON.stringify(safeLines.slice(0, retainedLineCount));
  }
...
const deserializeWordTimings = (value: unknown, lineText: string): LyricWordTiming[] | undefined => {
  if (!Array.isArray(value) || value.length > maxLyricWordTimingsPerLine) {
    return undefined;
  }
...
export const deserializeLyricLines = (linesJson: string): LyricLine[] => {
  if (linesJson.length > maxSerializedLyricLinesChars) {
    return [];
  }
