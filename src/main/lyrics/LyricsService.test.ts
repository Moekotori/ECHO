...
describe('LyricsService', () => {
  ...
  it('reuses an in-flight lyrics lookup for duplicate track requests', async () => {
    let resolveLyrics: (lyrics: TrackLyrics) => void = () => undefined;
    const providerLyrics = new Promise<TrackLyrics>((resolve) => {
      resolveLyrics = resolve;
    });
    const onlineProvider = {
      getLyrics: vi.fn(async () => providerLyrics),
      searchCandidates: vi.fn(async () => []),
    };
    const { service, online } = createHarness({
      appSettings: settings({ lyricsRomanizationEnabled: false, lyricsTranslationEnabled: false }),
      onlineProvider,
    });
    ...
    expect(first?.providerLyricsId).toBe('lrclib-1');
  ...
  it('returns recent stored candidates without repeating the provider search', async () => {
    const lrclibResult: LyricsProviderResult = {
      ...
      durationSeconds: 120,
      instrumental: false,
      plainLyrics: 'Candidate line',
      syncedLyrics: null,
      sourceLabel: 'LRCLIB',
      raw: { id: 'lrclib-candidate' },
    };
    const lrclibProvider: LyricsProvider = {
      ...
      search: vi.fn(async () => [lrclibResult]),
    };
    const { service } = createHarness({
      appSettings: settings({
      ...
      onlineProvider: lrclibProvider as never,
    });
