...
describe('lyricsParser', () => {
  ...
  it('caps abnormal lyric line counts without changing normal parsing', () => {
    const synced = Array.from({ length: maxParsedLyricLines + 12 }, (_, index) => `[00:${String(index % 60).padStart(2, '0')}.00]Line ${index}`).join('\n');
    const plain = Array.from({ length: maxParsedLyricLines + 12 }, (_, index) => `Line ${index}`).join('\n');

    expect(parseSyncedLyrics(synced)).toHaveLength(maxParsedLyricLines);
    expect(parsePlainLyrics(plain)).toHaveLength(maxParsedLyricLines);
  });

  it('rejects oversized lyric text before parsing', () => {
    const oversizedText = `${'x'.repeat(maxLyricsTextChars + 1)}`;

    expect(parsePlainLyrics(oversizedText)).toEqual([]);
    expect(parseSyncedLyrics(`[00:01.00]${oversizedText}`)).toEqual([]);
  });

  it('rejects oversized serialized lyric caches', () => {
    expect(deserializeLyricLines('x'.repeat(maxSerializedLyricLinesChars + 1))).toEqual([]);
  });
