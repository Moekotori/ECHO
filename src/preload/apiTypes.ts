...
export type EchoApi = {
  ...
  taskbarMiniPlayer: {
    ...
    getState: () => Promise<TaskbarMiniPlayerState>;
    setEnabled: (enabled: boolean) => Promise<TaskbarMiniPlayerState>;
    onStateChanged: (handler: (state: TaskbarMiniPlayerState) => void) => () => void;
  };
  library: {
  ...
  streaming: {
    ...
    resolveLive?: (request: StreamingLiveResolveRequest) => Promise<StreamingLiveResolveResult>;
    ...
    listAccountPlaylists?: (provider: Extract<StreamingProviderName, 'netease' | 'qqmusic'>) => Promise<StreamingAccountPlaylistsResult>;
  ...
  accounts: {
    ...
    startNeteaseQrLogin?: () => Promise<NeteaseQrLoginStartResult>;
    pollNeteaseQrLogin?: (key: string) => Promise<NeteaseQrLoginPollResult>;
