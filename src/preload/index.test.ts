...
describe('preload SMTC API', () => {
  ...
  it('exposes Audio CD helpers through IPC', async () => {
    const request = { driveId: 'cd:D:', trackIndex: 1 };

    await exposedApi!.audioCd.getStatus('cd:D:');
    await exposedApi!.audioCd.playTrack(request);

    expect(ipcRenderer.invoke).toHaveBeenCalledWith(IpcChannels.AudioCdGetStatus, 'cd:D:');
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(IpcChannels.AudioCdPlayTrack, request);
  });
  ...
  it('exposes ECHO Pro plugin activation through IPC', async () => {
    const request = {
      mode: 'key' as const,
      qq: '12345',
  ...
  it('exposes plugin management APIs through IPC', async () => {
    ...
    await exposedApi!.plugins.listMarket();
    await exposedApi!.plugins.installMarket('echo.market-tool');
    ...
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(IpcChannels.PluginsListMarket);
    expect(ipcRenderer.invoke).toHaveBeenCalledWith(IpcChannels.PluginsInstallMarket, 'echo.market-tool');
