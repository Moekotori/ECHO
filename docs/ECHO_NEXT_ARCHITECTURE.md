...
> **⚠️ 音频架构重构进行中（refactor/audio-architecture 分支）**：`NativeOutputBridge` 单体已拆分为 facade re-export，真正实现在 `NativePcmHostProcess`。`JuceDecodePipeline` / `EqBridge` 等旧模块仍存在但正在迁移。音频相关改动前先加载 `.skill/echo-audio-architecture-refactor` 对应的 SKILL.md。
...
User action
  ...
  -> AudioSession
  -> DecoderPipeline
  -> NativePcmHostProcess facade / system output
...
- AudioSession / DecoderPipeline / NativePcmHostProcess (facade) errors。
