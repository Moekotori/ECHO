...
- RPC-002 本轮继续补 Professional Status reference inspectability：reference assignment 行按 `orderedProfileSections` 显示 active/inactive、merge group、split reason 与 latency owner，并新增 merge groups / latency owners detail row；jsdom visual-state 测试覆盖这些 row 的 warning 语义，补齐“Signal Path 能解释每个 UI section engine assignment”的只读状态面证据。
