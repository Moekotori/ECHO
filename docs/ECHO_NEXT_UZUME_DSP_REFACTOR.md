...
- RPC-002 本轮继续补 direct-like output device mismatch visual-state guard：Professional Status 对 exclusive/direct-like requested rate 与 actual device rate 不一致保持 warning，显示 requested/actual rate、`direct-like-rate-mismatch`、`inspect-device-rate-mismatch` recommendation，并继续显示 `scheduler not-enabled`。此项仍是 reference-only policy telemetry，不表示 output policy 或 production scheduler 已改变。
