...
- 本轮 batch visual-state guard 审计保持 90 个测试总数不变，并在既有 Professional Status / PlayerBar visual-state tests 内追加 drift cases：formatPathPlan 缺 key、realtime summary 缺 reference-only reason、SRC null residual 漂移、validation check fail despite overall pass、PCM ingress sanitizer count 漂移、block committed-frame 漂移、serial-null missing merged reason 均回 warning。
