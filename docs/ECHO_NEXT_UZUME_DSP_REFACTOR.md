...
- RPC-002 本轮继续补 expected Output Device Policy visual-state guard：只有 direct-like rate match、requested/actual rate 一致、shared rate 不适用、bit-perfect candidate true、resampling/mismatch false、recommendation none 与 reference-only reasons 全部对齐时才标 good/process；若 telemetry 漂成 direct-like-ready 但仍有 resampling/mismatch 或 inspect recommendation，Signal Path / Professional Status 回 warning。此项仍是 reference-only policy guard，不改变 output device / mixer policy。
...
- RPC-002 本轮继续补 expected Latency Budget visual-state guard：只有 `reference-budget-summary-no-runtime-scheduler`、`realtimeBackend: not-enabled`、`schedulerState: reference-only`、callback read-committed-only 与 latency compensation deferred reasons 全部对齐时才标 good/process；若 telemetry 漂成 production scheduler / realtime backend，Signal Path / Professional Status 回 warning。此项仍是 reference-only 语义 guard，不启用 production scheduler 或 latency compensation。
...
- RPC-002 本轮继续补 expected Readiness Contract visual-state guard：只有 readiness contract 保持 main playback owns timeline、`productionScheduler: not-enabled`、current-generation-only、stale commit blocked、GPU prewarm deferred 与 reference-only reasons 全部对齐时才标 good/process；若 telemetry 漂成 production scheduler 或丢失 timeline boundary reason，Signal Path / Professional Status 回 warning。此项仍是 reference-only contract guard，不启用 production scheduler，也不让 Renderer 控制 timeline。
...
- RPC-002 本轮继续补 Latency Budget / Generation Cache Key ready visual-state baseline：Professional Status / Signal Path 能显示 ready latency budget 与 generation cache key，并在文案里继续暴露 `reference-only` 与 `renderer inspect-only`；后续 expected guards 已继续收紧，不再只按 `state: ready` 判定 good/process。
...
- RPC-002 本轮继续补 expected Generation Cache Key visual-state guard：只有 generation cache key 保持 generation/timeline/profile/device keying、gapless album segment keying 一致、stale commit rejected、late current generation only retained for future、renderer inspect-only 与 reference-only reasons 全部对齐时才标 good/process；若 telemetry 漂成 production cache writer 或 allow-stale commit，Signal Path / Professional Status 回 warning。此项仍是 reference-only keying guard，不启用 production cache writer。
...
- RPC-002 本轮补 expected visual-state guard batch：一次性收紧 Signal Path reference formatPath 六条完整性、Realtime Budget Summary reference-only reason、SRC budget same-rate null residual、SRC formal validation per-check、PCM ingress clean path、block boundary exact coverage/padding、Shared Convolution serial-null residual/reasons；drift cases 统一回 warning，split/inactive serial-null 仍保持 non-danger muted。此项仍是 reference-only UI guard，不表示 production realtime SRC、callback scheduler、production convolver 或 serial-null gate 已启用。
...
- RPC-002 本轮继续收紧 PCM output quantization visual-state fallback：fixed-point quantized report 只有 expected deterministic dither/quantization contract 对齐时才是 good/process；若 dither/enabled contract 漂移，即使没有 rejected state 也回 warning。此项仍是 reference-only UI guard，不表示 production output writer 或 dither gate 已启用。
...
- RPC-002 本轮继续补 Signal Path visual-state 审计覆盖：`PlayerBar.test.tsx` 现在锁定 `Reference merge groups`、`Reference latency owners` 为 process/non-warning，并让 `UZUME headroom reference` 在 recommended headroom 缺口存在时保持 warning、`UZUME limiter reference` 在 standby 时保持 process；Safety Meter near-limit 仍是 warning。此项只补 UI 状态语义证据，不表示 runtime 控制或 production limiter 已启用。
- RPC-002 本轮按一次性缺口审计确认剩余缺口集中在 Headroom / Safety / Limiter inspect surface：compiled reference plan 新增 `headroomSafetyLimiter` report，把 recommended headroom、stage peak/true-peak attribution、sample-domain limiter trigger/current+max gain reduction/limited frames 从 reference helper 汇入 Signal Path / Professional Status；visual-state drift guard 锁定 true-peak/lookahead 误标回 warning。此项仍是 RPC-002 reference telemetry，不启用 production limiter、runtime telemetry ring 或 callback DSP。
...
- RPC-001/RPC-002 相关 renderer/main vitest 6 个文件、508 个测试通过，覆盖 DSP 页 `未实现` UI、Signal Path skeleton/reference 文案、Professional Status、AudioSettingsDrawer、EQ panel 和 `uzumeFormatPathPlan` / `uzumeReferencePlan` telemetry。
