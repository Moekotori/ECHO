...
- RPC-002 本轮继续补 same-rate bypass SRC budget visual-state guard：Professional Status 现在把 `same-rate-bypass-reference` realtime summary 与 `same-rate-bypass` SRC budget 标成 good，并显示 exact-bypass null residual；这是 reference-only good state，不表示 production realtime SRC backend 已启用。
