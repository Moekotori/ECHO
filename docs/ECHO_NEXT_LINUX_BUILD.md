...
仓库提供 `flake.nix`、`package.nix` 和 `shell.nix`，方便 Nix 用户在 Linux 上复现开发环境和构建流程：
...
当前 Nix 路径重点覆盖 Electron 主进程、Renderer、`better-sqlite3` 和 `sharp` 的 Linux 构建。`native/audio-host` 仍建议在 `nix develop` 后按本文常规 Linux 流程单独验证，不要把 Nix 构建结果直接当作完整音频链路验收。
...
- Linux shared 输出走 native/miniaudio host，不启用 ASIO。
- CMake 会 `find_package(ALSA REQUIRED)`，并通过系统依赖链接 ALSA 与 libav / FFmpeg 相关库。
- 生产构建不拉取外部音频框架源码。
...
```text
[build:audio-host] Failed to build native audio host.
```
...
```bash
sudo apt install cmake g++ pkg-config libasound2-dev libavcodec-dev libavformat-dev libavutil-dev libswresample-dev libfreetype-dev libfontconfig1-dev
```

如果日志里是 CMake 配置、编译器、pkg-config、ALSA 或系统 libav / FFmpeg 依赖缺失，先补齐本机工具链和系统包，不要先改 audio host 代码。
