...
面板 action 仍按 manifest 中已信任的权限检查。`host:library:getTracks` 必须分页，单页最多 200；API v2 的 settings action 只读写当前插件自己的设置。网络访问和复杂逻辑继续放在 `plugin.js`，由面板通过 `plugin:runCommand` 调用。
...
- 是否让 `panel.html` 直接调用 `echo`。面板应使用 `postMessage` 的受控 host action。
