...
- 2026-06-08：补 Professional Status reference inspectability：assignment 行现在按 `orderedProfileSections` 展示 active/inactive、merge group、split reason 和 latency owner，新增 merge groups / latency owners detail row；这让 Signal Path/状态面板能解释每个 reference UI section 的 engine assignment，而不只显示前几个 active section。
- 2026-06-08：补 expected compiler/assignment visual-state guard：当 schema/telemetry 版本、ordered sections、engine assignment、merge group 与 latency owner 全部对齐时，Professional Status 的 compiler / assignment / merge / latency owner rows 标成 good，Signal Path 节点保持 process/non-warning；缺 assignment、merge group 不对齐或 latency owner 漂移时仍回 warning。这是 compiled reference plan 可审计状态，不表示 runtime backend switch 或 production compiler 已启用。
...
- 2026-06-08：补 expected Continuity / Cache visual-state guard：continuity policy 保持 callback read-committed-only、pre-roll deadline safe、callback ring stable/safe、render-ahead miss 只保留 prior committed output、fallback 为 safe/marginal controlled fallback 且 short bridge 被 underrun protection 拒绝时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；unsafe fallback 仍保留 danger/warning。这是 reference contract 已通过，不表示 production callback ring、render-ahead cache writer 或 GPU fallback runtime 已启用。
...
- 2026-06-08：补 expected Backend Support visual-state guard：当 backend support report 明确为 `reference-backend-only-no-runtime-switch`、CPU float64 deterministic reference 可用、AVX2/GPU runtime backend 标为后续 gate、legacy DSP chain 被 compiler 阻止且 reasons 完整时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；legacy compiler allowed 或 gate 漂移时仍回 warning。这是 reference backend contract 已通过，不表示 runtime backend switch 或 production backend 已启用。
...
- 2026-06-08：补 direct-like device-rate mismatch reference guard：主进程测试锁定 exclusive/direct-like requested rate 与 actual device rate 不一致时报告 `device-rate-mismatch-risk`、`direct-like-rate-mismatch` 和 `inspect-device-rate-mismatch`，同时继续显示 `productionScheduler: not-enabled`，避免把 policy telemetry 误读成 output policy 变更。
- 2026-06-08：补 direct-like device-rate mismatch visual-state guard：Signal Path / Professional Status 在 direct-like/exclusive requested rate 与 actual device rate 不一致时保持 warning，显示 requested/actual rate 与 `inspect-device-rate-mismatch` recommendation，并继续显示 scheduler not-enabled；这是 reference-only policy telemetry，不表示 output policy 或 production scheduler 已启用。
- 2026-06-08：补 expected Output Device Policy visual-state guard：只有 direct-like rate match、requested/actual rate 一致、shared rate 不适用、bit-perfect candidate true、resampling/mismatch false、recommendation none 与 reference-only reasons 全部对齐时才标 good/process；若 telemetry 漂成 direct-like-ready 但仍有 resampling/mismatch 或 inspect recommendation，Signal Path / Professional Status 回 warning。这是 reference-only policy guard，不改变 output device / mixer policy。
- 2026-06-08：补 expected RPC-002 visual-state guard batch：Signal Path 锁定 reference formatPath 六条完整性与旧 placeholder reason 漂移；Signal Path / Professional Status 只有 Realtime Budget Summary same-rate reference-only、SRC budget same-rate exact null residual、SRC formal validation per-check pass/not-applicable、PCM ingress clean path、Block Boundary exact coverage/padding 与 Shared Convolution serial-null zero residual+reason 全部对齐时才标 good/process，漂移统一回 warning；`bitPerfectState: available` 同时覆盖 PCM bit-perfect 与 DSD direct，保持现状，不擅自收窄成 PCM-only。
...
- 2026-06-08：补 expected Latency Budget visual-state guard：只有 `reference-budget-summary-no-runtime-scheduler`、`realtimeBackend: not-enabled`、`schedulerState: reference-only`、callback read-committed-only 与 latency compensation deferred reasons 全部对齐时才标 good/process；若 telemetry 漂成 production scheduler / realtime backend，Signal Path / Professional Status 回 warning。这是 reference-only 语义 guard，不启用 production scheduler 或 latency compensation。
...
- 2026-06-08：补 expected Readiness Contract visual-state guard：只有 readiness contract 仍是 `main-playback-owns-timeline-uzume-reports-readiness`、`productionScheduler: not-enabled`、current-generation-only、stale generation commit blocked、GPU prewarm deferred 与 main playback owns timeline reasons 全部对齐时才标 good/process；若 telemetry 漂成 production scheduler 或丢失 timeline boundary reason，Signal Path / Professional Status 回 warning。这是 reference-only contract guard，不启用 production scheduler，也不让 Renderer 控制 timeline。
...
- 2026-06-08：补 Latency Budget / Generation Cache Key ready visual-state baseline：Professional Status / Signal Path 能显示 ready latency budget 与 generation cache key，并在文案里继续暴露 `reference-only` 与 `renderer inspect-only`；后续 expected guards 已继续收紧，不再只按 `state: ready` 判定 good/process。
- 2026-06-08：补 gapless album-segment generation cache-key guard：gapless reference plan 的 cache key 现在锁定 `gapless-album-segment`、`album-reference:segment-0:index-1`、segment index 与 `renderer inspect-only`，Signal Path / Professional Status 均显示 album segment/index 且保持 good/process；这是只读 keying telemetry，不启用 production cache writer。
- 2026-06-08：补 expected Generation Cache Key visual-state guard：只有 generation cache key 仍是 `generation-safe-cache-key-contract-reference`、cache key 含 generation/timeline/profile/device、gapless album segment keying 一致、stale commit rejected、late current generation only retained for future、renderer inspect-only 与完整 reasons 全部对齐时才标 good/process；若 telemetry 漂成 production cache writer 或 allow-stale commit，Signal Path / Professional Status 回 warning。这是 reference-only keying guard，不启用 production cache writer。
...
- 2026-06-08：补 Artifact Manifest shared formatter helper 测试：`buildUzumeReferenceArtifactManifestSummary` 现在有 helper-level unit test 锁定 deterministic 39/39、planned 项 warning flag、not-applicable 非阻塞、null/undefined fallback，避免 Professional Status 与 Signal Path 的 manifest 文案/状态漂移。
...
- 2026-06-08：补 expected SRC artifact / phase-apodizing visual-state guard：Poly-Sinc formal validation pass、artifact metrics 完整且 exact-silence / seeded-random / passband / stopband / cutoff / transition 有效、phase mode 与 apodizing no-HF-restoration claim 对齐时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；offline-reference-only budget 与 output-risk 仍保留 warning。这是 reference contract 已可审计，不表示 production realtime SRC backend 或 runtime SRC profile 已启用。
- 2026-06-08：补 expected core SRC reference visual-state guard：Poly-Sinc reference family、rate/ratio/family、rational phase accumulator、filter contract、group-delay/lookahead ms、formal validation、artifact metrics 与 phase/apodizing artifacts 对齐时，Professional Status 的 `UZUME SRC reference` 标成 good，Signal Path 节点保持 process/non-warning；phase accumulator 或 telemetry 漂移仍回 warning。这是 reference contract 已可审计，不表示 production realtime SRC backend 已启用。
...
- 2026-06-08：按一次性缺口审计确认 RPC-002 剩余只剩 Headroom / Safety / Limiter UI inspect gap：compiled plan 新增 `uzumeReferencePlan.headroomSafetyLimiter`，Signal Path / Professional Status 改为显示 reference headroom recommendation、stage peak/true-peak attribution、sample-domain limiter trigger/gain-reduction/limited frames，并用 visual-state drift guard 锁定 true-peak/lookahead 漂移回 warning；这仍是只读 reference telemetry，不启用 production limiter 或 runtime telemetry ring。
...
- 2026-06-08：补 expected response resample visual-state guard：same-rate response exact bypass、cross-family / exact-rate mismatch response 使用 windowed-sinc float64 reference 且拒绝 linear interpolation 时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；这是 policy/reference contract 已通过，不表示 production convolver 已启用。
...
- 2026-06-08：补 expected DSD family visual-state guard：D2P reference 报告 decimation profile/internal PCM rate、allowed/disabled controls、fallback none 与 SDM separation 对齐时，Professional Status 标成 good，Signal Path 保持 process/non-warning；D2P fallback 或 unavailable 漂移仍回 warning。这是 DSD family reference contract 已可审计，不表示 D2P/SDM runtime 已启用。
...
- 2026-06-08：补 expected PCM output quantization visual-state guard：fixed-point PCM quantized、deterministic dither seed/LSB present、clip count 为 0 且 residual 有效时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；这是 reference contract 已通过，不表示 production output writer 或 dither gate 已启用。
- 2026-06-08：收紧 PCM output quantization visual-state fallback：Signal Path / Professional Status 不再把“非 expected 但 dither disabled”的 quantization report 误标成 process/good；fixed-point quantized path 若丢失 expected dither/enabled contract 会回 warning，仍不启用 production output writer / dither gate。
...
- 2026-06-08：补 expected DSP reference visual-state guard：gain staging 顺序/clip budget、PEQ/IIR band/order/residual、Channel Scope targeted-only/out-of-scope exact bypass、Stereo Procedural ordered steps、Per-Ear EQ no-reorder placement rule 全部对齐时，Professional Status 标成 good，Signal Path 节点保持 process/non-warning；clip risk、invalid scope、order/reason 漂移或 placement rule 漂移时仍回 warning。这是 deterministic reference contract 已通过，不表示 production PEQ、gain processor、channel router、crossfeed 或 per-ear EQ runtime 已接入。
...
- 2026-06-08：补 Signal Path visual-state 审计覆盖：jsdom 现在锁定 `Reference merge groups`、`Reference latency owners` 的 `data-tone` 为 process/non-warning，并让 `UZUME headroom reference` 在 recommended headroom 缺口存在时保持 warning、`UZUME limiter reference` 在 standby 时保持 process；Safety Meter near-limit 仍是 warning。这是 UI 状态语义测试，不表示新增 runtime 控制或 production limiter。
- 2026-06-08：批量审计 RPC-002 visual-state fallback 缺口，记录并补齐 6 个明确 reference 面：same-rate `UZUME SRC reference` 漂移、SRC rollback family/legacy/short-bridge 边界、SRC output-risk state/tone/recommendation 边界、Gapless SRC same-rate fallback、FIR gapless identity fallback、Equal-Power Crossfade rendered/rejected gate。补强只改变已有 inspect report 的 warning/good/process/muted 判定与测试，不新增 RPC-002 feature 面，不进入 production SRC / convolution / callback runtime。
- 2026-06-08：补 Callback-Safe Urgent Controls inspect 缺口：compiled `callbackSafeControls` 现在同时携带 mute/declick urgent case、volume-only urgent case 与 seek render-state boundary，Signal Path / Professional Status 都能显示 volume 不触发 render cache invalidation，expected visual-state guard 要求三段同时对齐；这仍是 reference inspect surface，不启用 production callback urgent-control path。
