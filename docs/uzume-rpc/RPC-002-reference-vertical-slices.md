...
- 2026-06-08：补 direct-like device-rate mismatch visual-state guard：Professional Status 在 direct-like/exclusive requested rate 与 actual device rate 不一致时保持 warning，显示 requested/actual rate 与 `inspect-device-rate-mismatch` recommendation，并继续显示 scheduler not-enabled；这是 reference-only policy telemetry，不表示 output policy 或 production scheduler 已启用。
