...
- 2026-06-08：补 Signal Path visual-state 审计覆盖：jsdom 现在锁定 `Reference merge groups`、`Reference latency owners` 与 `UZUME headroom reference` 节点的 `data-tone` 为 process/non-warning，并继续让 Safety Meter / limiter risk 保持 warning；这是 UI 状态语义测试，不表示新增 runtime 控制或 production limiter。
