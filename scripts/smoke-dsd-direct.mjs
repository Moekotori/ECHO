...
const packNativeDsdBytes = (channelBlocks, byteOffset, byteCount) => {
  const channels = channelBlocks.length;
  const output = Buffer.alloc(byteCount * channels);
  let outputOffset = 0;
...
const channelBlocks = [Buffer.from([1, 2, 3, 4]), Buffer.from([5, 6, 7, 8])];
const dopPayload = packDop24Le(channelBlocks, 0, 4);
...
const nativePayload = packNativeDsdBytes(channelBlocks, 0, 4);
const expectedNative = [1, 5, 2, 6, 3, 7, 4, 8];
if (JSON.stringify([...nativePayload]) !== JSON.stringify(expectedNative)) {
  fail(`native DSD pack bytes mismatch: got ${JSON.stringify([...nativePayload])}`);
}

console.log('[smoke:dsd-direct] DoP/native DSD byte-pack self-check OK (no DAC playback claimed)');

if (process.platform !== 'win32') {
  console.log('[smoke:dsd-direct] native DoP host skipped on non-Windows; no hardware playback attempted');
  process.exit(0);
}
...
if (result.exitCode === 0) {
  ...
  console.log(`[smoke:dsd-direct] WASAPI Exclusive DoP host accepted test stream (${ready.format ?? 'unknown format'}); this is transport smoke only, not DAC bit-perfect proof`);
} else if (/WASAPI exclusive DoP open failed|unsupported|format|DoP output requires/i.test(result.stderr)) {
  console.log(`[smoke:dsd-direct] DoP host unsupported on this device; explicit no-hardware diagnostic accepted with no DSD success claimed: ${result.stderr.trim()}`);
